...
# ─────────────────────────────────────────────
# CONFIGURATION — reads from hardware_settings.json
# ─────────────────────────────────────────────
_HW_FILE = _Path("app_data/hardware_settings.json")

def _get_label_printer_name() -> str:
    """Read the configured label printer from hardware_settings.json."""
    try:
        if _HW_FILE.exists():
            with open(_HW_FILE, "r", encoding="utf-8") as f:
                return _json.load(f).get("pharmacy_label_printer", "(None)")
    except Exception:
        pass
    return "(None)"
...
def print_pharmacy_label_for_sale(sale_items: list[dict]) -> None:
    ...
    if printed:
        print(f"  ✓ {printed} pharmacy label(s) sent to {_get_label_printer_name()}")
    else:
...
def _print_raw(zpl_bytes: bytes) -> None:
    ...
    printer_name = _get_label_printer_name()
    if printer_name == "(None)":
        print("  ERROR: No Zebra label printer found.")
        return
