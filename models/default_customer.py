#!/usr/bin/env python
"""
Standalone script to create Cash Customer customer on first login.
...
def create_default_customer():
    """Create Cash Customer customer with proper Frappe sync integration"""
    ...
    try:
        # Step 1: Check if Cash Customer customer already exists
        print("1. Checking if Cash Customer customer exists...")
        cursor.execute("SELECT id, frappe_synced FROM [dbo].[customers] WHERE customer_name = 'Cash Customer'")
        ...
        if existing:
            ...
            if frappe_synced:
                ...
                print(f"   [!]  Exists but not synced with Frappe - will trigger robust sync")
        ...
        if not company:
            print("   [!]  No company defaults found! Using defaults...")
            warehouse_name = None
            ...
            print(f"   [OK] Company defaults loaded")
        ...
        if warehouse_name:
            ...
            if wh:
                ...
                print(f"   [OK] Warehouse: {warehouse_name_final} (ID: {warehouse_id})")
            else:
                ...
                if wh:
                    warehouse_id = wh[0]
                    warehouse_name_final = wh[1]
                    print(f"   [OK] Using fallback: {warehouse_name_final}")
        ...
        if cost_center_name:
            ...
            if cc:
                ...
                print(f"   [OK] Cost Center: {cost_center_name_final} (ID: {cost_center_id})")
            else:
                print(f"   [!]  Cost center '{cost_center_name}' not found")
                cursor.execute("SELECT TOP 1 id, name FROM [dbo].[cost_centers] ORDER BY id ASC")
                cc = cursor.fetchone()
                if cc:
                    cost_center_id = cc[0]
                    cost_center_name_final = cc[1]
                    print(f"   [OK] Using fallback: {cost_center_name_final}")
        ...
        if not pl:
            ...
            cursor.execute("SELECT id FROM [dbo].[price_lists] WHERE name = 'Standard'")
            pl = cursor.fetchone()
            
        if not pl:
            # Final fallback: just grab the first one
            cursor.execute("SELECT TOP 1 id FROM [dbo].[price_lists] ORDER BY id ASC")
            pl = cursor.fetchone()
            
        if pl:
            price_list_id = pl[0]
            print(f"   [OK] Price list ID: {price_list_id}")
        else:
            print("   [!]  No price list found in database!")
        ...
        if cg:
            ...
            print(f"   [OK] Customer group ID: {customer_group_id}")
        
        # Step 7: Create Cash Customer customer
        print("\n7. Creating Cash Customer customer...")
        ...
        cursor.execute("""
            ...
            VALUES (?, 'Individual', ?, ?, ?, ?, 
                    ...
                    0, 0, 0, 0, 0)
        """, (
            'Cash Customer', customer_group_id, warehouse_id, cost_center_id, price_list_id,
            phone
        ))
        ...
        print(f"   [OK] Cash Customer customer created! (ID: {customer_id})")
        ...
        print(f"   [OK] Sync scheduled via background retry loop")
        ...
        print("\n" + "="*60)
        print("  [OK] DEFAULT CUSTOMER CREATED SUCCESSFULLY")
        print("="*60)
        print("\n📝 Summary:")
...
def _push_default_to_frappe(cursor, conn, customer_id):
    """Push Cash Customer customer to Frappe using same API as your sync system"""
    ...
    if not api_key or not api_secret:
        print(f"   [!]  No API credentials available")
        return False
    
    if not base_url:
        print(f"   [!]  No Frappe host configured")
        return False
    ...
    payload = {
        "name": "Cash Customer",
        "customer_name": customer[0],  # "Cash Customer"
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if result.get("data"):
                print(f"   [OK] Success! Frappe ID: {result['data'].get('name', 'Unknown')}")
                # Mark as synced
...
def get_default_customer():
    """Get the Cash Customer customer"""
    conn = get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("""
            ...
            FROM customers 
            WHERE customer_name = 'Cash Customer'
        """)
        customer = cursor.fetchone()
