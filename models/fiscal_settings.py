...
class FiscalSettings:
    enabled: bool = False
    provider: str = "frappe"
    base_url: str = "https://erpfiscal.havano.online"
    api_key: str = ""
    api_secret: str = ""
    device_sn: str = ""
    axis_email: str = ""
    ...
    def from_dict(cls, data: dict) -> "FiscalSettings":
        ...
        if not url:
            url = "https://erpfiscal.havano.online"
        return cls(
            id=data.get("id"),
            enabled=bool(data.get("enabled", False)),
            provider=str(data.get("provider", "frappe")),
            base_url=url,
            api_key=str(data.get("api_key", "")),
            api_secret=str(data.get("api_secret", "")),
            device_sn=str(data.get("device_sn", "")),
            axis_email=str(data.get("axis_email", "")),
            axis_password=str(data.get("axis_password", "")),
...
class FiscalSettingsRepository:
    ...
    def _ensure_schema(cursor):
        cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('fiscal_settings') AND name = 'provider')
            BEGIN
                ALTER TABLE fiscal_settings ADD provider NVARCHAR(50) NOT NULL DEFAULT 'frappe';
                ALTER TABLE fiscal_settings ADD axis_email NVARCHAR(200) NOT NULL DEFAULT '';
                ALTER TABLE fiscal_settings ADD axis_password NVARCHAR(200) NOT NULL DEFAULT '';
            END
        """)
    ...
    def get_settings() -> Optional[FiscalSettings]:
        ...
        try:
            ...
            FiscalSettingsRepository._ensure_schema(cursor)
            ...
            if not table_exists:
                ...
                cursor.execute("""
                    CREATE TABLE fiscal_settings (
                        ...
                        provider NVARCHAR(50) NOT NULL DEFAULT 'frappe',
                        ...
                        axis_email NVARCHAR(200) NOT NULL DEFAULT '',
                        axis_password NVARCHAR(200) NOT NULL DEFAULT '',
            ...
            if count == 0:
                cursor.execute("""
                    INSERT INTO fiscal_settings (
                        enabled, provider, base_url, api_key, api_secret, device_sn, axis_email, axis_password,
