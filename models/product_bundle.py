...
def create_bundle(bundle_name, items, description=""):
    ...
    try:
        ...
        conn.commit()
        print(f"[Bundle] [OK] Created bundle '{bundle_name}' with ID {bundle_id}, {inserted_count} item(s)")
...
def update_bundle(bundle_id, bundle_name, items, description=""):
    ...
    try:
        ...
        conn.commit()
        print(f"[Bundle] [OK] Updated bundle '{bundle_name}' (ID: {bundle_id}) with {inserted_count} item(s)")
        return True
...
def delete_bundle(bundle_id):
    ...
    conn.commit()
    conn.close()
    print(f"[Bundle] {'[OK]' if affected > 0 else '❌'} Deleted bundle ID: {bundle_id}")
    return affected > 0
