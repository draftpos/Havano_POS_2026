...
def create_sale(
    ...
    # total_zwd: convert the USD total → native ZWG when currency is ZWG/ZIG.
    # exchange_rate is USD-per-native (e.g. 0.0333 for ZWG), so dividing
    # gives native units: 167 USD / 0.0333 = 5010 ZWG.
    if currency_upper in ("ZWD", "ZWG", "ZIG") and _exch > 0:
...
def _print_receipt(sale: dict, items: list, tendered: float, change: float,
    ...
    # When the sale was paid in a non-USD currency we display tendered/change
    # in that native currency. `sale["total"]` is stored in USD (base
    # currency), so converting to native is required before subtracting the
    # native tendered — otherwise `7000 ZWG − 167 USD` collapses to 6833,
    # the literal cross-currency subtraction that produced the receipt bug.
    _is_zig = (currency or "").upper() in ["ZIG", "ZWG", "ZWD"]

    _exch          = float(sale.get("exchange_rate") or 0)
    _total_usd_val = float(sale.get("total_usd") or 0) or float(total or 0)

    # exchange_rate is USD-per-native (e.g. 0.0333 for ZWG), so dividing
    # the USD total gives native units: 167 USD / 0.0333 = 5010 ZWG.
    if _is_zig and _exch > 0:
        _total_native = round(_total_usd_val / _exch, 4)
    else:
