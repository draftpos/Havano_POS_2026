...
def get_income_by_method_since(shift_id: int) -> dict:
    ...
    log.info(f"Shift {shift_id} income: start={shift_start}, end={shift_end}")

    try:
        # Get regular payments from payment_entries (linked to sales)
        cur.execute("""
            SELECT 
                LTRIM(RTRIM(UPPER(pe.mode_of_payment))) AS payment_method,
                COALESCE(SUM(pe.paid_amount), 0) AS total_amount
            FROM payment_entries pe
            INNER JOIN sales s ON s.id = pe.sale_id
            WHERE s.created_at >= ? AND s.created_at <= ?
              AND pe.payment_type = 'Receive'
            GROUP BY LTRIM(RTRIM(UPPER(pe.mode_of_payment)))
        """, (shift_start, shift_end))

        for method, total in cur.fetchall():
            if method and total > 0:
                clean_method = method.split(" - ")[0].strip()
                result[clean_method] = result.get(clean_method, 0.0) + float(total)

        # Get standalone payments (no sale_id)
        cur.execute("""
            SELECT 
                LTRIM(RTRIM(UPPER(mode_of_payment))) AS payment_method,
                COALESCE(SUM(paid_amount), 0) AS total_amount
            FROM payment_entries
            WHERE created_at >= ? AND created_at <= ?
              AND sale_id IS NULL
              AND payment_type = 'Receive'
            GROUP BY LTRIM(RTRIM(UPPER(mode_of_payment)))
        """, (shift_start, shift_end))
        ...
        # ADD THIS: Get On Account amounts (credit sales with outstanding balance)
        cur.execute("""
            SELECT 
                'ON ACCOUNT' AS payment_method,
                COALESCE(SUM(s.total - s.tendered), 0) AS total_amount
            FROM sales s
            WHERE s.created_at >= ? AND s.created_at <= ?
              AND s.is_on_account = 1
              AND s.total > s.tendered
        """, (shift_start, shift_end))

        for method, total in cur.fetchall():
            if total > 0:
                result["ON ACCOUNT"] = result.get("ON ACCOUNT", 0.0) + float(total)
                log.info(f"On Account total for shift {shift_id}: ${total:.2f}")

    except Exception as e:
        log.error(f"Error fetching payment_entries: {e}")

    conn.close()
    log.info(f"Income for shift {shift_id}: {result}")
    return result
def refresh_income(shift_id: int) -> dict:
    ...
    shift_methods = [row[0] for row in cur.fetchall()]
    ...
    # Check if we need to add ON ACCOUNT row
    on_account_amount = income_by_method.get("ON ACCOUNT", 0.0)
    has_on_account = any(m.upper() == "ON ACCOUNT" for m in shift_methods)
    
    # ALWAYS ensure ON ACCOUNT exists if there are credit sales, regardless of other methods
    if on_account_amount > 0 and not has_on_account:
        # Insert ON ACCOUNT row if it doesn't exist
        cur.execute("""
            INSERT INTO shift_rows (shift_id, method, start_float, income, counted)
            VALUES (?, ?, 0, ?, 0)
        """, (shift_id, "ON ACCOUNT", float(on_account_amount)))
        ...
        # Update existing ON ACCOUNT row to zero if no credit sales
        cur.execute("""
            UPDATE shift_rows SET income = 0 WHERE shift_id = ? AND method = 'ON ACCOUNT'
        """, (shift_id,))
        print(f"[INFO] Updated ON ACCOUNT row to 0")
    elif has_on_account and on_account_amount > 0:
        # Update existing ON ACCOUNT row with current amount
        cur.execute("""
            UPDATE shift_rows SET income = ? WHERE shift_id = ? AND method = 'ON ACCOUNT'
        """, (float(on_account_amount), shift_id))
        print(f"[INFO] Updated ON ACCOUNT row with income: {on_account_amount}")

    for method in shift_methods:
        base_method = method.split(" - ")[0].strip().upper()

        income = 0.0
        for income_method, amount in income_by_method.items():
            if income_method.upper() == base_method:
                income = amount
                break
        
        # Special handling for ON ACCOUNT - ensure it gets the correct amount
        if method.upper() == "ON ACCOUNT":
            income = on_account_amount

        updated_income[method] = income

        if not is_closed:
            cur.execute("""
                UPDATE shift_rows SET income = ?
                WHERE shift_id = ? AND method = ?
            """, (float(income), shift_id, method))
...
def get_cashier_sales_for_shift(shift_id: int) -> list[dict]:
    ...
    # Get all sales during this shift with payment entries AND cashier names from users table
    cur.execute("""
        SELECT 
            s.id as sale_id,
            s.invoice_no,
...
def get_print_ready_cashiers(shift_id: int) -> list[dict]:
    ...
    for cashier in cashier_sales:
        ...
        for method_key, amount_collected in cashier_payment_methods.items():
            ...
            if total_method_collected > 0:
                proportion = amount_collected / total_method_collected
            else:
                proportion = 0.0
...
def start_shift(station: int, shift_number: int, cashier_id: int,
    ...
    for method, start_float in opening_floats.items():
        cur.execute("""
            INSERT INTO shift_rows (shift_id, method, start_float, income, counted)
            VALUES (?, ?, ?, 0, 0)
        """, (shift_id, method, float(start_float)))
...
def migrate():
    ...
    # Drop and recreate shift_reconciliations table with correct schema
    cur.execute("""
        IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'shift_reconciliations')
        DROP TABLE shift_reconciliations
    """)
...
def _get_shift_rows(shift_id: int, cur) -> list[dict]:
    ...
    for r in rows:
        ...
        # Special handling for ON ACCOUNT - counted should always be 0
...
def diagnose_income(shift_id: int = None):
    ...
    cur.execute("""
        SELECT method, start_float, income, counted
        FROM shift_rows
        WHERE shift_id = ?
    """, (shift_id,))
    ...
    rows = cur.fetchall()
    if rows:
        for method, start_float, income, counted in rows:
            total = start_float + income
            variance = counted - total
            print(f"    {method}: start_float=${start_float:.2f}, income=${income:.2f}, total=${total:.2f}, counted=${counted:.2f}, variance=${variance:.2f}")
    else:
...
def save_shift_reconciliation(shift_id: int, reconciliation_data: dict) -> int:
    """
    Save the complete shift reconciliation data to database.
    ...
    try:
        ...
        # Handle None values for closing_cashier_id
