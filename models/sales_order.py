...
def ensure_tables():
    ...
    _add_column_if_missing(cur, conn, "sales_order", "waiter_name",   "NVARCHAR(120)  NULL")
    _add_column_if_missing(cur, conn, "sales_order", "sync_error",    "NVARCHAR(MAX)  NULL")
...
def create_sales_order(
    ...
    cur.execute("""
        INSERT INTO sales_order
            (order_no, customer_id, customer_name, company, order_date,
             ...
             status, synced, frappe_ref, created_at, waiter_name)
        OUTPUT INSERTED.id
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        order_no,
    ...
    for item in cart_items:
        ...
        # === NEW: Deduct Stock immediately for Laybyes / Sales Orders ===
        try:
            from models.sale import adjust_stock, get_product_by_part_no
            prod_id = item.get("id") or item.get("product_id")
            if not prod_id and item_code:
                p_row = get_product_by_part_no(item_code)
                if p_row:
                    prod_id = p_row["id"]
            if prod_id:
                # Deduct stock based on the warehouse of the item
                adjust_stock(prod_id, -qty, warehouse_id=item.get("warehouse_id"))
        except Exception as e:
            log.error("Failed to deduct laybye stock for %s: %s", item_code, e)
...
def post_payment_to_frappe(order: dict) -> str | None:
    ...
    try:
        ...
        log.info("[OK] Payment Entry created in Frappe: %s for order %s", pe_name, order_no)
        return pe_name
...
def update_order_status(order_id: int, status: str):
    ...
    # Return stock if order is being cancelled
    if status.lower() == 'cancelled':
        cur = conn.cursor()
        cur.execute("SELECT status FROM sales_order WHERE id = ?", (order_id,))
        row = cur.fetchone()
        if row and row[0].lower() != 'cancelled':
            cur.execute("SELECT item_code, qty FROM sales_order_item WHERE sales_order_id = ?", (order_id,))
            for r in cur.fetchall():
                try:
                    from models.sale import adjust_stock, get_product_by_part_no
                    item_code, qty = r[0], r[1]
                    if item_code:
                        p_row = get_product_by_part_no(item_code)
                        if p_row and p_row.get("id"):
                            adjust_stock(p_row["id"], float(qty)) # positive qty to return stock
                except Exception as e:
                    log.error("Failed to return stock for cancelled order item %s: %s", r[0], e)
...
def convert_order_to_sale(order_id: int, cashier_id=None, cashier_name: str = "") -> dict | None:
    ...
    sale = create_sale(
        ...
        skip_stock=True,                # Prevent double deduction since stock was deducted at SO creation
    )
