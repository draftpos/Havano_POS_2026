...
def create_credit_note(
    ...
    try:
        ...
        if has_exch_rate:
            cur.execute("""
                SELECT invoice_no, frappe_ref, currency, customer_name, total, exchange_rate, warehouse_id
                FROM sales 
                WHERE id = ?
            """, (original_sale_id,))
        else:
            cur.execute("""
                SELECT invoice_no, frappe_ref, currency, customer_name, total, warehouse_id
                FROM sales 
                WHERE id = ?
            """, (original_sale_id,))
        ...
        if has_exch_rate and len(sale_row) > 5:
            ...
            original_warehouse_id = sale_row[6] if len(sale_row) > 6 else None
        else:
            original_warehouse_id = sale_row[5] if len(sale_row) > 5 else None
        ...
        for return_item in items_to_return:
            ...
            try:
                ...
                if prod_row:
                    adjust_stock(prod_row[0], return_qty, warehouse_id=original_warehouse_id)
            except Exception as e:
        ...
        print(f"[OK] Created credit note: {cn_number}")
...
def get_all_credit_notes() -> list[dict]:
    """Fetch all credit notes ordered by date descending."""
    conn = get_connection()
    cur = conn.cursor()
    ...
    if has_exchange_rate:
        cur.execute("""
            SELECT id, cn_number, original_sale_id, original_invoice_no,
                   frappe_ref, frappe_cn_ref, total, currency, exchange_rate, cashier_name,
                   customer_name, cn_status, created_at, fiscal_status
            FROM credit_notes
            ORDER BY created_at DESC
        """)
    else:
    ...
    for row in rows:
        ...
        if row.get("created_at"):
            if hasattr(row["created_at"], "strftime"):
                ...
                row["date"] = str(row["created_at"]).split()[0]
                row["time"] = str(row["created_at"]).split()[1][:8] if len(str(row["created_at"]).split()) > 1 else ""
        else:
            row["date"] = ""
            row["time"] = ""
