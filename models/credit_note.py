...
def create_credit_note(
    ...
    try:
        ...
        # ── Auto-create the refund payment entry ──────────────────────────────
        # cn_payment_entry_service is a standalone file — nothing else is touched.
        # frappe_invoice_ref stays NULL until credit_note_sync_service links it
        # after the CN is confirmed on Frappe, at which point the existing
        # payment sync daemon pushes it to Frappe automatically.
        try:
            from services.cn_payment_entry_service import create_cn_payment_entry
            create_cn_payment_entry(cn_result)
        except Exception as pe_err:
            log.warning("CN %s created OK but payment entry failed: %s", cn_number, pe_err)
        # ─────────────────────────────────────────────────────────────────────
