...
def migrate():
    ...
    if not _table_exists(cur, "restaurant_settings"):
        cur.execute("""
            CREATE TABLE [dbo].[restaurant_settings] (
                ...
                [kot_order_num_size]      INT DEFAULT 16,
                [kot_header_size]         INT DEFAULT 12,
                [kot_header_text]         NVARCHAR(200) DEFAULT 'KITCHEN ORDER',
                [default_view_restaurant] BIT DEFAULT 0
            )
        """)
        conn.commit()
    ...
    # ── 8. restaurant_predefined_notes ───────────────────────────────────────
    if not _table_exists(cur, "restaurant_predefined_notes"):
        cur.execute("""
            CREATE TABLE [dbo].[restaurant_predefined_notes] (
                [id]   INT IDENTITY(1,1) PRIMARY KEY,
                [note] NVARCHAR(300) NOT NULL
            )
        """)
        conn.commit()
        print("[Migrate] Created table: restaurant_predefined_notes")

    # Prepopulate restaurant_predefined_notes if empty
    cur.execute("SELECT COUNT(*) FROM restaurant_predefined_notes")
    ...
    if notes_count == 0:
        ...
        for note in default_notes:
            cur.execute("INSERT INTO restaurant_predefined_notes (note) VALUES (?)", (note,))
        conn.commit()
        print("[Migrate] Prepopulated default restaurant predefined notes")

    # ── 9. restaurant_cancel_reasons ─────────────────────────────────────────
    if not _table_exists(cur, "restaurant_cancel_reasons"):
        cur.execute("""
            CREATE TABLE [dbo].[restaurant_cancel_reasons] (
                [id]     INT IDENTITY(1,1) PRIMARY KEY,
                [reason] NVARCHAR(300) NOT NULL
            )
        """)
        conn.commit()
        print("[Migrate] Created table: restaurant_cancel_reasons")

    # Prepopulate restaurant_cancel_reasons if empty
    cur.execute("SELECT COUNT(*) FROM restaurant_cancel_reasons")
    reasons_count = cur.fetchone()[0]
    if reasons_count == 0:
        default_reasons = [
            "Customer changed mind"
        ]
        for reason in default_reasons:
            cur.execute("INSERT INTO restaurant_cancel_reasons (reason) VALUES (?)", (reason,))
        conn.commit()
        print("[Migrate] Prepopulated default restaurant cancel/modify reasons")
...
def get_restaurant_settings() -> dict:
    ...
    defaults = {
        ...
        "kot_order_num_size":      16,
        "kot_header_size":         12,
        "kot_header_text":         "KITCHEN ORDER",
        "default_view_restaurant": False,
    }
    # Fields that are NOT boolean - must preserve their original type
    _int_fields  = {"kot_order_num_size", "kot_header_size"}
    _text_fields = {"kot_header_text"}
    try:
        conn = get_connection(); cur = conn.cursor()
        ...
        if row:
            ...
            for k in defaults:
                ...
                if k in _int_fields:
                    try:
                        defaults[k] = int(raw) if int(raw) > 0 else defaults[k]
                    except (ValueError, TypeError):
                        pass  # keep the default
                elif k in _text_fields:
                    sv = str(raw).strip()
                    # Guard against corrupted "True"/"1" values from old bug
                    if sv.lower() not in ("true", "false", "0", "1", ""):
                        defaults[k] = sv
                else:
                    defaults[k] = bool(raw)
        return defaults
...
def save_restaurant_settings(settings: dict):
    ...
    try:
        ...
        bit_fields = [
            ...
            "enabled",
            "default_view_restaurant",
        ]
        # Non-BIT fields (keep their real value)
        int_fields  = ["kot_order_num_size", "kot_header_size"]
        text_fields = ["kot_header_text"]

        fields = bit_fields + int_fields + text_fields
    ...
    try:
        ...
        cur.execute("""
            SELECT o.id, o.table_id, t.name as table_name, t.table_number, t.floor,
                   ...
                   o.bill_notes, o.opened_at, o.shift_order_number
...
def get_order_by_table(table_id: int) -> dict | None:
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("""
            SELECT id, table_id, customer_name, status, created_at, waiter_id, bill_notes, opened_at, shift_order_number
...
def get_orders_for_table(table_id: int) -> list[dict]:
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("""
            SELECT id, table_id, customer_name, status, created_at, waiter_id, shift_order_number
            FROM restaurant_orders
            WHERE table_id = ? AND status IN ('Open', 'Ordered')
...
def get_predefined_notes() -> list[str]:
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("SELECT note FROM restaurant_predefined_notes ORDER BY id DESC")
...
def add_predefined_note(note: str):
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("INSERT INTO restaurant_predefined_notes (note) VALUES (?)", (note,))
        conn.commit(); conn.close()
...
def delete_predefined_note(note: str):
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("DELETE FROM restaurant_predefined_notes WHERE note = ?", (note,))
        conn.commit(); conn.close()
    except Exception:
        pass


def get_cancel_reasons() -> list[str]:
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("SELECT reason FROM restaurant_cancel_reasons ORDER BY id DESC")
        rows = [r[0] for r in cur.fetchall()]
        conn.close()
        return rows
    except Exception:
...
def add_cancel_reason(reason: str):
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("INSERT INTO restaurant_cancel_reasons (reason) VALUES (?)", (reason,))
        conn.commit(); conn.close()
...
def delete_cancel_reason(reason: str):
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("DELETE FROM restaurant_cancel_reasons WHERE reason = ?", (reason,))
        conn.commit(); conn.close()
    except Exception:
        pass
...
def get_sales_by_waiter(waiter_name: str | None = None, start_date: str | None = None, end_date: str | None = None) -> list[dict]:
    """
    Fetch sales from the main sales table filtered by waiter and date range.
    date format: 'YYYY-MM-DD'
    """
    try:
        from database.db import get_connection, fetchall_dict
        conn = get_connection()
        cur = conn.cursor()
        ...
        if waiter_name and waiter_name != "All Waiters":
            query += " AND waiter_name = ?"
        ...
        print(f"[Model] Error in get_sales_by_waiter: {e}")
        return []

def get_all_waiter_names() -> list[str]:
    """Return a list of all unique waiter names found in the users table."""
    try:
        from database.db import get_connection
        conn = get_connection()
        cur = conn.cursor()
        
        # Get users who are likely waiters (e.g. have a certain role or just all users for now)
        cur.execute("SELECT DISTINCT username FROM users WHERE active = 1 ORDER BY username")
