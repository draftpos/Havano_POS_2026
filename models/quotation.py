...
class Quotation:
    ...
    # opened by a different user later still shows the original creator even if a different
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Quotation':
        ...
        return cls(
            ...
            waiter_name=data.get("waiter_name") or data.get("custom_waiter"),
        )
...
def create_quotations_table():
    ...
    # Ensure waiter_name column exists if table already existed
    try:
        cur.execute("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'quotations' AND COLUMN_NAME = 'waiter_name'")
        if not cur.fetchone():
            print("[Quotation] Adding waiter_name column to quotations table...")
            cur.execute("ALTER TABLE quotations ADD waiter_name NVARCHAR(255) NULL")
            conn.commit()
    except Exception as e:
        print(f"[Quotation] Error adding waiter_name column: {e}")
    ...
    conn.commit()
    conn.close()
    print("[Quotation] [OK] Tables created/verified.")


def save_quotation(quotation: Quotation, is_sync: bool = False) -> int:
    """Save a quotation to local database with product linking. If is_sync=True, avoids auto-printing."""
    conn = get_connection()
    cur = conn.cursor()
    ...
    print(f"\n[DEBUG] save_quotation: START for name='{quotation.name}'")
    print(f"[DEBUG]   waiter_name received: '{quotation.waiter_name}'")
...
def convert_quotation_to_cart(quotation: Quotation) -> list[dict]:
    ...
    for item in quotation.items:
        ...
        # Resolve pharmacy flag dynamically from products table if missing
        is_pharm = bool(item.is_pharmacy)
        if not is_pharm and part_no:
            try:
                conn = get_connection()
                cur = conn.cursor()
                cur.execute("SELECT COALESCE(is_pharmacy_product, 0) FROM products WHERE part_no = ?", (part_no,))
                row = cur.fetchone()
                conn.close()
                if row and row[0]:
                    is_pharm = True
