...
def _get_api_url() -> str:
    ...
    if not host:
        return ""
    return f"{host.rstrip('/')}/api/method/havano_pos_integration.api.get_sales_invoice"
...
def _get_credentials() -> tuple[str, str]:
    ...
    try:
        ...
        s = get_session()
        if s.get("api_key"):
            return s["api_key"], s.get("api_secret", "")
    except Exception as exc:
        log.debug("Could not read from auth session: %s", exc)
...
def _auth_header(api_key: str, api_secret: str) -> dict:
    ...
    hdr = build_auth_header(api_key, api_secret)
    return {"Authorization": hdr} if hdr else {}
...
def _upsert_invoices(invoices: list[dict]) -> tuple[int, int]:
    ...
    Now properly handles is_on_account flag from Frappe custom field.
    """
    try:
        from database.db import get_connection
    except ImportError:
    ...
    try:
        for inv in invoices:
            ...
            # ── NEW: Read is_on_account from Frappe custom field ──────────────
            # Check if the invoice is marked as On Account
            is_on_account = False
            custom_fields = inv.get("custom_fields", {})
            if custom_fields:
                is_on_account = bool(custom_fields.get("is_on_account", False))
            else:
                # Fallback: check if method is "On Account"
                is_on_account = inv.get("method", "") == "On Account"
            ...
            if existing:
                # UPDATE header (preserve is_on_account flag)
                cur.execute("""
                    UPDATE sales
                    SET    customer_name = ?,
                           company_name  = ?,
                ...
                _insert_items(cur, sale_id, inv.get("items", []))
                updated += 1
                log.debug("Updated invoice %s (on_account=%s)", invoice_no, is_on_account)

            else:
                # INSERT header with is_on_account flag
                cur.execute("""
                    INSERT INTO sales (
                        invoice_no, invoice_date, customer_name, company_name,
                        total, subtotal, total_items,
                        ...
                        kot, receipt_type, footer,
                        total_vat, discount_amount, change_amount,
                        synced, created_at, invoice_number, is_on_account
                    ) VALUES (
                        ?, ?, ?, ?,
                        ?, ?, ?,
                        0, ?, ?,
                ...
                """, (
                    invoice_no, posting_date, customer, company,
                    grand_total, grand_total, total_qty,
                    inv.get("method", "API"), inv.get("currency", "USD"),
                    created_at,
                    1 if is_on_account else 0,
                ))
                ...
                _insert_items(cur, sale_id, inv.get("items", []))
                inserted += 1
                log.info("Inserted invoice %s (total=%.2f, on_account=%s)", 
                        invoice_no, grand_total, is_on_account)
