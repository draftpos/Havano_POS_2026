...
def _is_zpl_printer(printer_name: str) -> bool:
    """Determine if a printer uses ZPL by checking its Windows driver name."""
    try:
        import win32print
        hprinter = win32print.OpenPrinter(printer_name)
        try:
            info = win32print.GetPrinter(hprinter, 2)
            driver_name = info['pDriverName'].lower()
        finally:
            win32print.ClosePrinter(hprinter)
        if any(kw in driver_name for kw in ["zebra", "zdesigner", "xprinter", "420", "tspl", "zpl"]):
            return True
        return False
    except Exception as e:
        print(f"[ZPL] Could not detect driver for {printer_name}: {e}")
        return False
...
def auto_print_pharmacy_labels_for_quotation(
    ...
    is_zpl = _is_zpl_printer(printer_name)
    printed = 0
    
    if is_zpl:
        print(f"[Pharmacy Print] Using native RAW ZPL for printer: {printer_name}")
        for label in labels:
            ...
            if _send_to_printer(zpl):
                printed += 1
                print(f"  ✓ Printed (ZPL): {label['product_name']} (x{label['qty']})")
            else:
                print(f"  ✗ Failed: {label['product_name']}")
    else:
        print(f"[Pharmacy Print] Using Windows Driver (HTML) for printer: {printer_name}")
        try:
            from services.pharmacy_label_print import _build_document, _make_label_printer
            ...
            printed = len(labels)
            for label in labels:
                print(f"  ✓ Printed (Driver): {label['product_name']} (x{label['qty']})")
        except Exception as e:
            print(f"[ERROR] Print via Windows driver failed: {e}")
...
^FO10,55^A0N,18,18^FDIf you see this, ZPL printer works!^FS
^XZ"""
        success = _send_to_printer(test_zpl.encode("utf-8"))
    else:
        try:
            from services.pharmacy_label_print import _make_label_printer
            from PySide6.QtGui import QTextDocument
            printer = _make_label_printer()
            printer.setPrinterName(printer_name)
            html = (
                "<html><body style=\"font-family: Arial; padding: 2mm;\">"
                "<h2 style=\"margin:0;\">TEST PRINT</h2>"
                "<hr>"
                "<p>If you see this, your label printer is working properly!</p>"
                "</body></html>"
            )
            doc = QTextDocument()
            doc.setHtml(html)
    ...
    if parent:
        if success:
            mode = "ZPL mode" if is_zpl else "Windows Driver mode"
            QMessageBox.information(parent, "Printer Test", 
                                   f"Test sent to {printer_name} successfully via {mode}!")
        else:
            QMessageBox.warning(parent, "Printer Test", 
