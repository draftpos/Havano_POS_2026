...
def print_quotation(ref) -> bool:
    ...
    # Check paper size setting first before requiring thermal printer selection
    paper_size = "80mm"
    try:
        hw_file = Path("app_data/hardware_settings.json")
        if hw_file.exists():
            hw = json.loads(hw_file.read_text(encoding="utf-8"))
            paper_size = hw.get("paper_size", "80mm") or "80mm"
    except Exception as e:
        log.warning("print_quotation: error reading paper_size: %s", e)

    receipt = _build_receipt(quotation)

    if str(paper_size).upper() == "A4":
        log.info("Quotation printing in A4 paper size mode - launching preview dialog.")
    ...
    for printer_name in printers:
        try:
            success = ps.print_receipt(receipt, printer_name=printer_name)
            if success:
                log.info("Quotation printed -> %s", printer_name)
                ok = True
            else:
