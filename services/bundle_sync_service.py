...
def debug_print(msg: str, data: any = None):
    try:
        if sys and hasattr(sys, 'is_finalizing') and sys.is_finalizing():
            return
        if "Exception" in msg or "Failed" in msg:
            log.warning(f"[BUNDLE SYNC] {msg}")
        else:
            log.info(f"[BUNDLE SYNC] {msg}")
    except Exception:
        pass
...
def fetch_bundles_from_frappe(api_key: str, api_secret: str, host: str) -> List[Dict]:
    ...
    try:
        ...
        debug_print(f"Notice: Network bundle fetch unavailable ({e})")
        return []
def push_bundle_to_frappe(bundle: Dict, api_key: str, api_secret: str, host: str) -> bool:
    ...
    debug_print(f"Pushing bundle to Frappe via /api/resource/Product%20Bundle")
    ...
    try:
        with safe_urlopen(req, timeout=30) as resp:
            ...
            if success:
                debug_print(f"[OK] Successfully pushed bundle: {payload['new_item_code']}")
            else:
        ...
        for remote in remote_bundles:
            ...
            try:
                ...
                if existing:
                    ...
                    skipped += 1
                    debug_print(f"  [OK] Updated bundle: {name}")
                else:
                    ...
                    saved += 1
                    debug_print(f"  [OK] Created bundle: {name}")
            except Exception as e:
...
# =============================================================================
# BUNDLE SYNC DAEMON (runs every 60 seconds in background thread)
# =============================================================================

class BundleSyncWorker:
    """Background thread worker that syncs bundles every 60 seconds"""
    
    def __init__(self):
        self._running = True
        self._interval = 60  # Increased to 60 seconds to reduce server load
        self._last_sync = 0
    
    def stop(self):
        """Stop the sync worker"""
        self._running = False
        log.info("[BundleSync] Worker stopped")
    
    def run(self):
        """Main sync loop"""
        log.info(f"[BundleSync] Worker started, syncing every {self._interval} seconds")
        ...
        while self._running:
            try:
                ...
                log.warning(f"[BundleSync] Error in loop: {e}")
                time.sleep(5)  # Back off on error
    
    def _do_sync(self):
        """Perform the actual sync (pull and push)"""
        try:
            log.info("[BundleSync] Running sync cycle...")
            
            # 1. Pull bundles from Frappe
            pull_result = pull_all_bundles()
            log.info(f"[BundleSync] Pull: {pull_result.get('saved', 0)} new, {pull_result.get('skipped', 0)} updated")
            
            # 2. Push pending bundles to Frappe
            push_result = sync_pending_bundles()
            log.info(f"[BundleSync] Push: {push_result.get('synced', 0)} synced, {push_result.get('errors', 0)} errors")
            
            log.info(f"[BundleSync] Sync cycle complete")
            
        except Exception as e:
            log.warning(f"[BundleSync] Sync failed: {e}")
...
def start_bundle_sync_daemon():
    ...
    log.info(f"[BundleSync] Daemon started (runs every {worker._interval}s)")
...
def stop_bundle_sync_daemon(tracking: dict):
    """Stop the bundle sync daemon gracefully."""
    if not tracking:
        return
    ...
    # Since it's a daemon=True threading.Thread, it will die with the process,
    # but worker.stop() breaks the loop early.