...
_LABEL_TEMPLATE = (
    "<table border='0' cellspacing='0' cellpadding='0' width='100%'><tr><td style='padding-top:15px; padding-left:30px;'>"
    "<div style=\"font-family:'Segoe UI',Arial,sans-serif; margin:0px; padding:0px; line-height:0.8; max-height:48mm; overflow:hidden; page-break-inside:avoid;\">"

    
    # Product Name (massive)
    "<div style=\"font-weight:bold; font-size:20pt; margin:0px; overflow:hidden;\">{item_name}</div>"
    
    # Separator
    "<div style=\"border-bottom:2px solid #000; margin:1px 0;\"></div>"
    
    # QTY Row (no tables)
    "<div>"
    "  <div style='font-size:16pt; line-height:0.8;'>QTY</div>"
    "  <div style='font-size:20pt; font-weight:bold; line-height:0.8;'>{qty_str}</div>"
    "</div>"
    
    # Batch / Expiry Row (using table for perfect horizontal alignment)
    "<table width='100%' cellpadding='0' cellspacing='0' style='margin-top:1px; border:0;'>"
    "  <tr>"
    "    <td width='50%' align='left' valign='top'>"
    "      <div style='font-size:16pt; line-height:0.8;'>BATCH</div>"
    "      <div style='font-size:18pt; font-weight:bold; line-height:0.8;'>{batch_no}</div>"
    "    </td>"
    "    <td width='50%' align='right' valign='top'>"
    "      <div style='font-size:16pt; line-height:0.8;'>EXPIRY</div>"
    "      <div style='font-size:18pt; font-weight:bold; line-height:0.8;'>{expiry_date}</div>"
    "    </td>"
    "  </tr>"
    "</table>"
    
    # Dosage Block (optional)
    "{dosage_block}"
...
def render_label_html(label: dict) -> str:
    ...
    item_name_raw   = label.get("item_name") or label.get("product_name") or ""
    # Hard truncate the product name to 28 characters to prevent it from wrapping
    # onto a second line, which would break the physical 30mm height constraint.
    if len(item_name_raw) > 28:
        item_name_raw = item_name_raw[:26] + ".."
        
    item_name       = html.escape(_clean(item_name_raw)).upper()
    qty             = html.escape(_fmt_qty(label.get("qty")))
    uom             = html.escape(_clean(label.get("uom"), ""))
    dosage          = html.escape(_clean(label.get("dosage")))
    batch_no        = html.escape(_clean(label.get("batch_no"), "N/A"))
    ...
    pharmacy_phone  = html.escape(_clean(label.get("pharmacy_phone"), ""))

    qty_str = f"{qty} {uom}".strip() if uom and uom != "-" else qty
    
    company_block = ""
    if pharmacy_name and pharmacy_name != "-":
        company_block += f"<div style=\"font-size:17pt; font-weight:bold; text-align:left; line-height:0.9; margin-top:2px;\">{pharmacy_name}</div>"
    if pharmacy_addr and pharmacy_addr != "-":
        company_block += f"<div style=\"font-size:15pt; text-align:left; line-height:0.9; margin-top:1px;\">{pharmacy_addr}</div>"
    
    dosage_block = ""
    if dosage and dosage != "-":
        dosage_block = (
            "<div style=\"border-top:1px solid #000; margin-top:1px; padding-top:1px;\"></div>"
            "<div style=\"font-size:16pt; line-height:0.8;\">DOSAGE</div>"
            f"<div style=\"font-size:18pt; font-weight:bold; margin:0px; line-height:0.8;\">{dosage}</div>"
        )
        
    pharmacist_block = ""
    if pharmacist and pharmacist != "-":
        pharmacist_block = (
            "<div style=\"border-top:1px solid #000; margin-top:1px; padding-top:1px;\">"
            f"<div style=\"font-size:16pt; line-height:0.8;\">Pharmacist: {pharmacist}</div>"
            "</div>"
        )
        
    phone_block = ""
    if pharmacy_phone and pharmacy_phone != "-":
        phone_block = f"<div style=\"font-size:15pt; text-align:left; margin-top:1px; line-height:0.9;\">Tel: {pharmacy_phone}</div>"
    ...
    return _LABEL_TEMPLATE.format(
        ...
        pharmacist_block=pharmacist_block,
...
def _get_pharmacy_context() -> dict:
    ...
    phone = (d.get("phone") or d.get("mobile") or d.get("contact") or "").strip()
...
def _fetch_pharmacy_items_for_quotation(quotation_id: int) -> list[dict]:
    ...
    try:
        try:
            cur.execute("""
                SELECT qi.item_code, qi.item_name, qi.qty, qi.uom,
                       COALESCE(qi.is_pharmacy, 0) AS is_pharmacy,
                       qi.dosage,
                       qi.batch_no, qi.expiry_date
...
def _build_document(labels: list[dict], printer: QPrinter) -> QTextDocument:
    ...
    for idx, lbl in enumerate(labels):
        ...
        # In QTextDocument, appending an empty div with page-break-after often fails
        # causing the next label's header to print on the bottom of the current label.
        # The most reliable fix is to apply page-break-before: always directly to the 
        # root block element of the next label.
        if idx > 0:
            body = body.replace("<table border='0'", "<table style=\"page-break-before: always;\" border='0'", 1)
    ...
    try:
        # Use paperRect (full physical size) rather than pageRect (printable size)
        # to prevent Qt from wrapping text when it hits the invisible margins configured
        # inside the Windows Generic Printer driver.
