...
def create_cn_payment_entry(cn: dict) -> int | None:
    ...
    try:
        ...
        if get_system_mode() == "saas":
            log.info("[create_cn_PE] SaaS mode detected, skipping CN payment entry.")
            return None
    except Exception:
        pass
    ...
    customer = (cn.get("customer_name") or "Cash Customer").strip() or "Cash Customer"
...
def _build_cn_payload(pe: dict, defaults: dict,
    ...
    party = pe.get("party") or "Cash Customer"
    
    # For refund: paid_to = Receivables/Debtors account (destination)
    try:
        from services.payment_entry_sync_service import _fetch_receivable_account
        paid_to = _fetch_receivable_account(party, company, api_key, api_secret, host)
    except ImportError:
        paid_to = ""
        
    if not paid_to:
        paid_to = defaults.get("server_receivable_account", "").strip()
...
def push_cn_payment_entry(pe: dict) -> str | None:
    ...
    """
    from services.credentials import check_credentials
    api_key, api_secret = _get_credentials()
    if not check_credentials(api_key, api_secret):
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            log.info(f"[OK] CN Payment Entry pushed -> Frappe {name}")
