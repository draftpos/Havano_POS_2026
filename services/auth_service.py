# =============================================================================
# services/auth_service.py  —  Online/Offline Authentication + Auto Sync
# =============================================================================
import json
...
_session = {
    ...
    "api_secret":     None,
    "source":         None,
    "raw_login_data": None,   # full login response, used by sync_service
}
...
def login(username: str, password: str) -> dict:
    """
    Tries online login first, falls back to offline.
    ...
    Returns:
        {
            ...
            "sync_result": { products_synced, inserted, updated, ... } | None,
            "error":       "reason string",
        }
    """
    ...
    if online["success"]:
        ...
        _session["api_key"]        = online.get("api_key")
        _session["api_secret"]     = online.get("api_secret")
        ...
        # ── Save server defaults (company, warehouse, etc.) to local DB ──────
        ...
        try:
            ...
            existing["api_key"]                   = online.get("api_key") or ""
            existing["api_secret"]                = online.get("api_secret") or ""
            save_defaults(existing)
            print("[auth] ✅ Server defaults saved to company_defaults.")
        except Exception as e:
            print(f"[auth] ⚠️  Could not save server defaults: {e}")

        # ── Auto-sync products into local SQL Server DB ───────────────────────
        sync_result = None
...
def _try_online_login(username: str, password: str) -> dict:
    payload = json.dumps({
        "usr": username, "pwd": password, "timezone": TIMEZONE,
    }).encode("utf-8")
    ...
    try:
        ...
        status = e.code
        try:
            msg = json.loads(e.read().decode()).get("message", f"HTTP {status}")
        except Exception:
            msg = f"HTTP {status}"
        if status in (401, 403, 417):
            return {"success": False, "auth_failed": True, "error": "Wrong username or password."}
        return {"success": False, "auth_failed": False, "error": f"Server error {status}"}
...
def _parse_online_success(data: dict, username: str) -> dict:
    """
    Parse the Havano login response.

    Actual shape from your API:
    {
      "message": { "warehouse_items": [...], "user": {...}, "company": {...},
                   "time_zone": "Africa/Harare" },
      "token_string": "102ebeac2df589c:887a7dd54dc1980",
      "token": "MTAy..."    ← base64
    }
    """
    # Token
    ...
    # User info — lives inside data["user"], NOT data["message"]
    # data["message"] is just the string "Logged In"
    ...
    return {
        ...
        "raw_data":   data,       # full response for sync_service
    }
