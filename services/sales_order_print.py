...
def print_sales_order(order_id: int) -> bool:
    ...
    for printer_name in printers:
        try:
            ...
            if success:
                log.info("[OK] Sales Order %s printed -> %s", order.get("order_no"), printer_name)
                ok = True
            else:
                log.warning("[!]  Sales Order print failed on %s", printer_name)
        except Exception as exc:
...
def print_laybye_deposit(order_id: int) -> bool:
    ...
    for printer_name in printers:
        try:
            success = printing_service.print_receipt(receipt, printer_name=printer_name)
            if success:
                log.info("[OK] Laybye deposit slip %s printed -> %s",
                         order.get("order_no"), printer_name)
                ok = True
            else:
                log.warning("[!]  Deposit slip print failed on %s", printer_name)
        except Exception as exc:
            log.error("❌ Printer error on %s: %s", printer_name, exc)
    return ok