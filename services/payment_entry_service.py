...
def _resolve_amounts(sale: dict, override_rate: float = None) -> dict:
    ...
    if override_rate is not None:
        ...
        if exch_rate > 0 and exch_rate < 1 and currency in ("ZWD", "ZWG", "ZIG","ZAR","AMD","CAD" "EUR"):
            exch_rate = exch_rate
    elif currency == "USD":
        exch_rate = 1.0
    else:
    ...
    if currency == "USD":
        ...
        amount_zwg = 0.0
    elif currency in ("ZWD", "ZWG", "ZIG","ZAR","AMD","CAD" "EUR"):
        amount_usd = amount          # paid_amount IS the USD basis
        amount_zwd = 0.0
...
def create_payment_entry(sale: dict, override_rate: float = None,
    ...
    _walk_in = _defaults.get("server_walk_in_customer", "").strip() or "Cash Customer"
    ...
    try:
        # Prevent duplicate payment entry creation for the exact same sale & mode
        if not _is_split and sale.get("id"):
            cur.execute("""
                SELECT TOP 1 id FROM payment_entries 
                WHERE sale_id = ? AND mode_of_payment = ?
            """, (sale["id"], mop_name))
            existing_pe = cur.fetchone()
            if existing_pe:
                log.info("[create_PE] Payment entry already exists for sale_id=%s mop=%s (PE id=%s). Skipping duplicate.",
                         sale["id"], mop_name, existing_pe[0])
                conn.close()
                return existing_pe[0]
        ...
        # Trigger an immediate sync so this entry doesn't wait for the next daemon tick (if not in saas mode)
        if not is_saas:
            _trigger_sync_now()
        return new_id
...
def create_split_payment_entries(sale: dict, splits: list[dict], shift_id: int = None) -> list[int]:
    ...
    for idx, split in enumerate(splits):
        ...
        if currency in ("ZWD", "ZWG", "ZIG","ZAR","AMD","CAD" "EUR") and rate <= 0:
            try:
                from models.exchange_rate import get_rate
        ...
        if 0 < rate < 1 and currency != _base_curr:
            rate = 1.0 / rate
            log.info("[create_split_PE] Inverted to local per USD for %s: %.6f", currency, rate)

        if currency == _base_curr:
            rate = 1.0

        # ------------------------------------------------------------------ #
        # Convert local amount -> USD basis for foreign splits
        # For USD splits amount_local is already USD, no conversion needed.
        # ------------------------------------------------------------------ #
        if currency != _base_curr and rate > 0:
...
def _build_payload(pe: dict, defaults: dict,
    ...
    if currency in ("ZWD", "ZWG", "ZIG", "RAND", "AMD","CAD"):
    ...
    if currency.startswith("USD") and paid_to_currency in ("ZWD", "ZWG", "ZIG", "AMD", "CAD"):
    ...
    # --- Party ---
    _walk_in         = defaults.get("server_walk_in_customer", "").strip() or "Cash Customer"
...
def stop_payment_sync_daemon() -> None:
    global _sync_thread_running
    _sync_thread_running = False
    log.info("Payment entry sync daemon stop requested.")
