...
def fetch_quotations_from_external_site(page: int = 1, limit: int = 100) -> dict:
    """Fetch one page of quotations from the external site (or main Odoo site if in Odoo mode)."""
    print(f"\n[ExternalQtn] ── fetch page={page} limit={limit} ──")

    from services.credentials import get_system_mode
    is_odoo = get_system_mode() == "odoo"
    ...
    if is_odoo:
        ...
        url = defaults.get("server_api_host") or _get_host()
        url = url.strip().rstrip("/")
        odoo_token = defaults.get("odoo_token") or creds.get("odoo_token")
        
        if not url or not odoo_token:
            return _err(page, "Odoo URL or Token is missing. Please log in again.")
            
        endpoint = f"{url}/saas_api/get_quotations?page={page}&limit={limit}"
        print(f"[ExternalQtn] Odoo Endpoint: {endpoint}")
        
        req = urllib.request.Request(endpoint)
        req.add_header("Authorization", odoo_token)
    ...
    try:
        with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            ...
            print(f"[ExternalQtn] [OK] Quotations on page : {len(quotations)}")
...
def save_external_quotations_locally(quotations_data: list) -> dict:
    ...
    for i, qtn_data in enumerate(quotations_data):
        ...
        try:
            ...
            result["saved"] += 1
            print(f"[ExternalQtn]   [{i+1}] [OK] Saved as '{prefixed_name}'")
