...
def _build_quotation_payload(quotation: dict, defaults: dict) -> dict:
    ...
    if not customer:
        customer = defaults.get("server_walk_in_customer", "Cash Customer")
    ...
    payload = {
        ...
        "custom_waiter": quotation.get("waiter_name") or "",
    }
...
def _push_quotation_to_frappe(quotation: dict, api_key: str, api_secret: str, 
    ...
    try:
        ...
        with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            ...
            if frappe_name:
                log.info("[OK] Quotation %s -> Frappe as %s (customer: %s, ref: %s, cost_center: %s)", 
                         qtn_name, frappe_name, customer, payload.get("reference_number"), payload.get("cost_center"))
                return frappe_name
            else:
                log.info("[OK] Quotation %s pushed successfully to Frappe (customer: %s)", 
                         qtn_name, customer)
                return "success"
        ...
        log.warning("[!] Quotation %s - HTTP %s: %s", qtn_name, e.code, msg[:500])
        return None
        
    except urllib.error.URLError as e:
        if "SSL" in str(e):
            log.warning("Network: SSL error pushing quotation %s (skipping)", qtn_name)
        else:
...
def push_unsynced_quotations() -> dict:
    ...
    if get_system_mode() == "odoo":
        # log.debug("Odoo mode - skipping Frappe quotation sync.")
        return result
    ...
    host = _get_host()
    if not host:
        # Offline mode or not configured
        return result
    ...
    if not api_key:
        log.warning("Missing API credentials - skipping quotation upload cycle.")
        return result
    ...
    for idx, quotation in enumerate(quotations):
        ...
        if result_val:
            try:
                ...
                result["pushed"] += 1
                log.info(f"[OK] Quotation {quotation.name} marked as synced")
            except Exception as e:
    ...
    log.info("Quotation upload done - [OK] %d pushed  ❌ %d failed  (of %d)",
...
def fetch_quotations_from_frappe(page: int = 1, limit: int = 100) -> dict:
    ...
    if not api_key:
        log.warning("Missing API credentials - cannot fetch quotations.")
        return {"quotations": [], "total": 0, "page": page, "has_next": False}
    ...
    try:
        ...
        if "SSL" in str(e):
            log.warning("Network: SSL error fetching quotations (skipping)")
        else:
        ...
        while has_next:
            ...
            for qtn_data in quotations_data:
                try:
                    ...
                    save_quotation(quotation, is_sync=True)
                    result["synced"] += 1
                except Exception as e:
...
def cancel_quotation_in_frappe(quotation_name: str) -> bool:
    ...
    try:
        ...
        with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            ...
            if success:
                log.info(f"[OK] Quotation {quotation_name} cancelled in Frappe")
                return True
            else:
                log.warning(f"[!] Failed to cancel quotation {quotation_name}: {data.get('message', 'Unknown error')}")
                return False
...
def sync_quotation_on_create(quotation_id: int, max_retries: int = 3) -> bool:
    ...
    for attempt in range(max_retries):
        ...
        if result:
            ...
            mark_quotation_synced(quotation.local_id, frappe_ref)
            log.info(f"[OK] Quotation {quotation.name} synced successfully")
            return True
        ...
        def run(self) -> None:
            ...
            while self._running:
                try:
                    ...
                    # Fetch invoices from Odoo as quotations if rule is enabled
                    ...
                    if get_system_mode() == "odoo":
                        try:
                            ...
                            log.error("Failed to check fetch_invoices_as_quotations rule: %s", e)
...
def start_quotation_sync_thread() -> dict:
    ...
    t.start()
    log.info("Quotation sync daemon Thread started.")
    return {"thread": t, "worker": None}


def stop_quotation_sync_thread(tracking: dict):
    """Stop the quotation sync thread gracefully."""
    if not tracking:
        return
    worker = tracking.get("worker")
    thread = tracking.get("thread")
...
# =============================================================================
# FETCH INVOICES AS QUOTATIONS (ODOO)
# =============================================================================

def sync_invoices_as_quotations_from_odoo() -> dict:
    """
    Fetch quotations (draft sales orders) from Odoo using saas_api and save them as local quotations if the POS rule is enabled.
    """
    result = {"synced": 0, "errors": 0, "total": 0}
    
    from services.credentials import get_odoo_token
    odoo_token = get_odoo_token()
    if not odoo_token:
        print("[OdooFetch] No odoo token found, skipping sync.")
        return result
    
    host = _get_host()
    if not host:
        print("[OdooFetch] No host URL configured, skipping sync.")
        return result

    try:
        print("[OdooFetch] Starting fetch from Odoo using saas_api...")
        from models.company_defaults import get_defaults
        defaults = get_defaults() or {}
        db_name = defaults.get("server_database", "")
        
        url = f"{host.rstrip('/')}/saas_api/get_quotations"
        if db_name:
            url += f"?db={db_name}"
            
        req = urllib.request.Request(url)
        req.add_header("Authorization", f"Bearer {odoo_token}")
        req.add_header("Accept", "application/json")
        
        with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            data = json.loads(resp.read().decode())
            message = data.get("message", {})
            orders = message.get("quotations", []) if isinstance(message, dict) else []
            
            print(f"[OdooFetch] Received {len(orders)} sales orders/quotations from Odoo")
            if not orders:
                return result
                
            from models.quotation import Quotation, save_quotation, get_all_quotations
            
            # Get existing quotations to avoid duplicates
            existing_quotes = get_all_quotations()
            existing_names = {q.name for q in existing_quotes}
            
            for ord_data in orders:
                ord_name = ord_data.get("name")
                if not ord_name:
                    continue
                    
                if ord_name in existing_names:
                    continue
                    
                print(f"[OdooFetch] Processing new order {ord_name}")
                result["total"] += 1
                
                # Directly map Odoo's saas_api quotation structure
                quotation = Quotation.from_dict(ord_data)
                quotation.synced = True
                
                save_quotation(quotation, is_sync=True)
                print(f"[OdooFetch] Saved local quotation {ord_name}")
                result["synced"] += 1
                
    except Exception as e:
        print(f"[OdooFetch] Error in Odoo sync: {e}")
        log.error("Failed to sync invoices as quotations: %s", e)
        result["errors"] += 1

    if result["synced"] > 0:
        log.info("Synced %d invoices as quotations from Odoo.", result["synced"])
