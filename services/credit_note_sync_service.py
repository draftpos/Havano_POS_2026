...
def stop_credit_note_sync_daemon() -> None:
    global _sync_thread_running
    _sync_thread_running = False
    log.info("Credit note sync daemon stop requested.")
...
def _base_cn_payload_fields(
    ...
    In SaaS mode, includes reference_number and SaaS terminal metadata.
    """
    ...
    ref_num = str(cn.get("cn_number") or cn.get("reference_number") or cn.get("frappe_ref") or "").strip()

    payload: dict = {
        "is_return":           True if is_saas else 1,
        "return_against":      cn.get("frappe_ref") or "",
        ...
        "is_pos":              1 if is_saas else 0,
        "update_stock":        1,
        "docstatus":           1,
        "custom_cn_reference": ref_num,
    }

    if is_saas:
        cashier_email = str(cn.get("cashier_email") or cn.get("owner") or defaults.get("server_email") or defaults.get("user_email") or "").strip()
        if cashier_email:
            payload["owner"] = cashier_email
            payload["cashier"] = cashier_email
            payload["sales_person"] = cashier_email
        pos_profile = defaults.get("pos_profile") or defaults.get("server_profile") or ""
        if pos_profile:
            payload["pos_profile"] = pos_profile
        term_id = defaults.get("server_terminal_id") or defaults.get("terminal_id")
        if term_id:
            try:
                payload["terminal_id"] = int(term_id) if str(term_id).isdigit() else term_id
            except Exception:
                payload["terminal_id"] = term_id
...
def _build_cn_payload_usd(
    ...
    for item in items:
        ...
        row: dict = {
            "item_code": item_code,
            "qty":       abs(qty) if is_saas else -abs(qty),
            "rate":      rate,
...
def _push_cn(
    ...
    # ── DEBUG LOGGING FOR CREDIT NOTE UPLOAD PAYLOAD ──────────────────────────
    try:
        print("\n" + "=" * 80)
        print(f"[DEBUG] CREDIT NOTE UPLOAD PAYLOAD for {cn_num} (Frappe Ref: {cn.get('frappe_ref')}):")
        print(json.dumps(payload, indent=2, default=str))
        print("=" * 80 + "\n")
        log.info("[DEBUG] CN Payload for %s: %s", cn_num, json.dumps(payload, default=str))
    except Exception as _ex_dbg:
        print(f"[DEBUG] Could not print CN payload: {_ex_dbg}")
    ...
    if get_system_mode().lower() == "saas":
        target_urls = [
            f"{host}/api/method/saas_api.www.api.create_invoice",
            f"{host}/api/method/saas_api.www.api.create_sales_invoice",
            f"{host}/api/method/saas_api.www.api.make_sale",
            f"{host}/api/resource/Sales%20Invoice"
        ]
    else:
        target_urls = [f"{host}/api/resource/Sales%20Invoice"]
    ...
    try:
        for target_url in target_urls:
            ...
            try:
                with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
                    ...
                    name = (resp_json.get("data") or {}).get("name", "") or resp_json.get("name") or True
                    log.info("[OK] CN %s -> Server %s  (return_against=%s  currency=%s)",
                ...
                try:
                    ...
                    msg = (err.get("exception") or err.get("message") or err.get("error") or
                ...
                if e.code == 409:
                    log.info("[cn-sync] CN %s already exists on server (409).", cn_num)
                    return True
                ...
                log.warning("[cn-sync] CN %s endpoint %s failed (HTTP %s): %s", cn_num, target_url, e.code, msg)

        if last_msg:
            log.error("❌ CN %s  Failed across all endpoints: %s", cn_num, last_msg)
            try:
                from database.db import get_connection
                conn = get_connection()
                cur = conn.cursor()
                cur.execute("UPDATE credit_notes SET sync_error = ? WHERE id = ?", (last_msg, cn["id"]))
                conn.commit()
                conn.close()

                from services.sync_errors_service import record_error
                customer_name = cn.get("customer_name") or defaults.get("server_walk_in_customer") or "Cash Customer"
                amount = float(cn.get("total") or 0)
                record_error("CN", cn_num, last_msg, customer=customer_name, amount=amount)
            except Exception as e:
                log.warning("[cn-sync] Could not record sync error: %s", e)
            return False
        ...
        if frappe_ref:
            try:
                ...
                hdrs_check = {"Accept": "application/json"}
                if auth_hdr:
                    hdrs_check["Authorization"] = auth_hdr
                check_req = urllib.request.Request(
                    check_url,
                    headers=hdrs_check,
                )
                with safe_urlopen(check_req, timeout=15) as check_resp:
...
def push_unsynced_credit_notes(force: bool = False) -> dict:
            ...
            if val:
                ...
                try:
                    ...
                    try:
                        ...
                        resolve("CN", cn.get("cn_number", str(cn["id"])))
                    except Exception:
                        pass
                    result["pushed"] += 1
                except Exception as e:
    ...
    log.info("[cn-sync] Done - [OK] %d pushed  ❌ %d failed  ⏭ %d skipped",
...
def _sync_loop():
    global _sync_thread_running
    log.info("Credit note sync daemon started (interval=%ds).", SYNC_INTERVAL)
...
def start_credit_note_sync_daemon() -> dict:
    """Call once from MainWindow.__init__ alongside the other daemons."""
    global _thread
    if _thread and _thread.is_alive():
        return {"thread": _thread, "worker": None}
    _thread = threading.Thread(target=_sync_loop, daemon=True, name="CreditNoteSyncDaemon")
    _thread.start()
