...
def _build_so_payload(order: dict, items: list[dict], defaults: dict) -> dict | None:
    ...
    walk_in  = defaults.get("server_walk_in_customer", "Cash Customer").strip() or "Cash Customer"
    ...
    payload = {
        ...
        "conversion_rate":  1.0,
        "items":            frappe_items,
        "custom_waiter":    str(order.get("waiter_name") or ""),
    }
...
def _push_order(order: dict, api_key: str, api_secret: str,
    ...
    try:
        with safe_urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            ...
            log.info("[OK] Order %s synced -> %s", order_no, name)
...
def push_unsynced_orders() -> dict:
    ...
    log.info(
        "Sales Order upload done - [OK] %d pushed  ❌ %d failed  (of %d)",
...
def start_so_upload_thread() -> dict:
    """
    Start the Sales Order upload background thread.
    Returns a dict containing {"thread": Thread, "worker": None}
    """
    global _so_upload_running
    _so_upload_running = True
    def _loop():
        global _so_upload_running
        while _so_upload_running:
            try:
                push_unsynced_orders()
            except Exception as exc:
    ...
    t.start()
    log.info("Sales Order upload daemon Thread started.")
    return {"thread": t, "worker": None}

def stop_so_upload_thread():
    global _so_upload_running
    _so_upload_running = False
    log.info("Sales Order upload daemon stop requested.")