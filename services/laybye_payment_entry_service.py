...
def _push_single(pe: dict, api_key: str, api_secret: str,
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            conn.commit()
            log.info("[OK] Laybye PE %d synced -> %s", pe_id, frappe_name)
            print(f"[OK] Laybye PE {pe_id} ({pe.get('order_no')}) synced to Frappe: {frappe_name}")
...
def link_laybye_payment_to_frappe(so_number: str, frappe_so_ref: str) -> None:
    ...
    try:
        ...
        conn.commit()
        conn.close()
        log.info("[OK] Linked laybye payments for %s to %s", so_number, frappe_so_ref)
        print(f"[OK] Linked laybye payments for {so_number} to Frappe SO: {frappe_so_ref}")
    except Exception as e:
...
def stop_laybye_pe_sync_daemon() -> None:
    global _sync_thread_running
    _sync_thread_running = False
    log.info("Laybye PE sync daemon stop requested.")