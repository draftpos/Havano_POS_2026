...
def clear_all_errors():
    """Completely wipe all errors from the sync_errors table."""
    try:
        ensure_table()
        c = _conn()
        c.cursor().execute("DELETE FROM sync_errors")
        c.commit()
        log.info("All sync errors have been wiped.")
    except Exception as e:
        log.warning("sync_errors clear_all_errors failed: %s", e)
...
def _auto_wipe_loop():
    while True:
        try:
            # We no longer wipe errors every 60 seconds so the user can read them in the UI!
            pass
        except Exception as e:
            pass
        time.sleep(60)

_auto_wipe_thread = None

def start_auto_wipe_daemon():
    global _auto_wipe_thread
    if _auto_wipe_thread and _auto_wipe_thread.is_alive():
        return
    _auto_wipe_thread = threading.Thread(target=_auto_wipe_loop, daemon=True, name="AutoWipeDaemon")
    _auto_wipe_thread.start()