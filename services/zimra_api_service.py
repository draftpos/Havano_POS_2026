...
class ZimraApiService:
    ...
    def ping_zimra(self, settings) -> ApiResult:
        """
        Pings ZIMRA using the token endpoint to check connectivity.
        """
        session = requests.Session()
        ok, result = self._fetch_token(settings, session)
        if not ok:
        ...
        @dataclass
        class ZimraPingResponse:
            device_sn: str
            reporting_frequency: int
            operation_id: str

        return ApiResult.success(ZimraPingResponse(
            device_sn=settings.device_sn or "Unknown",
    ...
    def send_invoice(self, settings, invoice_number: str, currency: str,
        ...
        with _send_lock:
            ...
            data = {
                ...
                "customer_vat_number":      str(kwargs.get("buyer_vat", "")) or "000000000",
                "customer_address":         str(kwargs.get("buyer_street", "")) + (" " + str(kwargs.get("buyer_city", "")) if kwargs.get("buyer_city") else ""),
                "customer_telephone_number": str(kwargs.get("buyer_phone", "")) or "",
                "customer_tin":             str(kwargs.get("buyer_tin", "")) or "111111111",
                "customer_province":        str(kwargs.get("buyer_province", "")) or "",
                "customer_street":          str(kwargs.get("buyer_street", "")) or "",
                "customer_houseNo":         str(kwargs.get("buyer_house_no", "")) or "",
                "customer_city":            str(kwargs.get("buyer_city", "")) or "",
                "customer_email":           str(kwargs.get("buyer_email", "")) or "",
                "invoice_comment":          "",
            ...
            try:
                ...
                if isinstance(msg, dict):
                    ...
                    if "success" in msg.lower():
                        print(f"[ZIMRA] [OK] Success string received")
                        return ApiResult.success(FiscalInvoiceResponse.from_dict(resp_data))
                    else:
                ...
                if resp_data.get("QRcode"):
                    print(f"[ZIMRA] [OK] Success! QR Code at top level")
                    return ApiResult.success(FiscalInvoiceResponse.from_dict(resp_data))
