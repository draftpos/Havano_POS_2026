...
def _get_products_endpoint() -> str:
    host = _get_host()
    if not host:
        return ""
...
PRODUCT_SYNC_INTERVAL_SECONDS = 15     # highly aggressive product stock and price sync
GL_MOP_SYNC_INTERVAL_SECONDS  = 120    # GL / MOP / exchange rates every 2 mins
...
def _has_local_products() -> bool:
    ...
    Returns True ONLY when the products table exists AND contains at least
    one row. An empty table (e.g. after a wipe or failed first sync) is
    treated as a first-time install so we always run the full inline sync
    at login instead of deferring to the background worker and leaving the
    POS grid blank.
    """
    try:
        ...
        conn.close()
        if n == 0:
            log.info("[sync] products table is empty - forcing inline sync on login")
        return n > 0
    except Exception as e:
        log.debug("_has_local_products check failed (table may not exist yet): %s", e)
        return False
...
def sync_from_login_response(login_data: dict) -> dict:
    ...
    try:
        ...
        print(
            f"[sync] [OK] Doctors synced: {result['doctors_synced']} | "
    ...
    try:
        ...
        print(
            f"[sync] [OK] Doctors pushed: {result['doctors_pushed']} "
    ...
    try:
        ...
        print(
            f"[sync] [OK] Dosages pushed: {result['dosages_pushed']} "
...
def _ensure_product_schema(cur) -> None:
    ...
    try:
        cur.execute("""
            ...
            CREATE TABLE products (
                ...
                is_pharmacy_product INT           DEFAULT 0,
                track_stock         INT           DEFAULT 1,
                is_product_bundle   INT           DEFAULT 0,
                active              INT           DEFAULT 1,
                order_1             INT           DEFAULT 0,
                order_2             INT           DEFAULT 0,
                order_3             INT           DEFAULT 0,
                order_4             INT           DEFAULT 0,
                order_5             INT           DEFAULT 0,
                order_6             INT           DEFAULT 0,
                updated_at          DATETIME2     DEFAULT SYSDATETIME()
            )
        """)
    except Exception as e:
        log.warning("[schema] products table create: %s", e)
    ...
    # ── products - add tax/hs_code columns if this is an older DB ─────────────
...
def _extract_cost_price(prices: list, stock_uom: str = "Nos") -> float:
    """
    Best cost (buying) price from the prices array.
    Priority:
      1. Buying price that matches the stock UOM
      2. Any 'Standard Buying' price list entry
      3. First buying-type entry
    """
...
def _parse_product(p: dict) -> dict | None:
    ...
    for rp in (p.get("prices") or []):
        pl = str(rp.get("priceName") or "").strip()
        uom_name = str(rp.get("uom") or "Nos").strip()
        try:
            rp_price = float(rp.get("price") or 0)
        except (TypeError, ValueError):
            rp_price = 0.0
        if pl and uom_name and rp_price > 0:
            item_prices.append({"price_list": pl, "uom": uom_name, "price": rp_price})
    ...
    result = {
        ...
        "raw_warehouses":      p.get("warehouses") or [],
...
class ProductSyncNotifier(QObject):
    progress = Signal(int, int, str)
    finished = Signal()

product_sync_notifier = ProductSyncNotifier()
...
def _fetch_page(page_num: int, headers: dict) -> tuple[int, list, dict | None]:
    ...
    """
    endpoint = _get_products_endpoint()
    if not endpoint:
        raise RuntimeError("No host URL configured for product sync")
        
    fallback_endpoint = endpoint.replace("saas_api.www.api.get_my_products", "havano_pos_integration.api.get_products")
    
    url = f"{endpoint}?page={page_num}&limit={PAGE_SIZE}"
    fallback_url = f"{fallback_endpoint}?page={page_num}&limit={PAGE_SIZE}"
    last_error = None
 
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(f"[sync] 🌐 Fetching Product Catalogue URL (Page {page_num}): {url}", flush=True)
            ...
            # Try fallback URL if the first one failed (like 404)
            if url != fallback_url:
                log.info(f"[sync] Endpoint {url} failed with {e.code}, trying fallback {fallback_url}")
                url = fallback_url
...
def sync_products(api_key: str = "", api_secret: str = "",
    ...
    if result["total_api"] > 0:
        try:
            product_sync_notifier.progress.emit(0, result["total_api"], f"Found {result['total_api']} products. Starting sync...")
        except Exception:
            pass
    ...
    for idx, raw in enumerate(all_raw, start=1):
        ...
        # Live progress counter for login loader & UI progress bar
        total_raw = len(all_raw)
        if total_raw <= 50 or idx % max(1, total_raw // 50) == 0 or idx == total_raw or idx == 1:
            try:
                product_sync_notifier.progress.emit(idx, total_raw, f"Syncing products: {idx}/{total_raw}")
            except Exception:
                pass
    ...
    try:
        ...
        init_stock_cache()
        log.info("[sync] Stock cache refreshed after product sync")
    except Exception as e:
        log.warning("[sync] Failed to refresh stock cache: %s", e)

    try:
        product_sync_notifier.finished.emit()
    except Exception:
        pass
...
def _upsert_parsed_product(cur, conn, p: dict, local_part_nos: set) -> bool:
    ...
    # ── Item prices ─────────────────────────────────────────────────────
    for ip in (p.get("item_prices") or []):
        try:
            cur.execute(
                """
                MERGE item_prices AS target
                USING (SELECT ? AS part_no, ? AS price_list, ? AS uom) AS src
                ...
                WHEN NOT MATCHED THEN
                    INSERT (part_no, price_list, uom, price) VALUES (?, ?, ?, ?);
                """,
                (
                    part_no, ip["price_list"], ip["uom"], ip["price"],
                    part_no, ip["price_list"], ip["uom"], ip["price"],
                ),
            )
        except Exception as e:
            log.warning("[sync] Item price MERGE failed for %s/%s/%s: %s", part_no, ip.get("price_list"), ip.get("uom"), e)
    try:
        conn.commit()
    except Exception as e:
        log.warning("[sync] Item prices commit failed for %s: %s", part_no, e)
    ...
    # ── product_warehouse_stock & warehouses auto-registration ──────
    raw_warehouses = p.get("raw_warehouses") or []
    if raw_warehouses:
        try:
            cur.execute(
                "DELETE FROM product_warehouse_stock "
                "WHERE product_id = (SELECT id FROM products WHERE part_no = ?)",
                (part_no,)
            )
            for w in raw_warehouses:
                w_name = str(w.get("warehouse") or "").strip()
                if not w_name:
                    continue
                try:
                    qty = float(w.get("qtyOnHand") or 0)
                ...
                cur.execute("SELECT id FROM warehouses WHERE LTRIM(RTRIM(UPPER(name))) = ?", (w_name.upper(),))
                r = cur.fetchone()
                if r:
                    wh_id = r[0]
                else:
                    ...
                    cur.execute("INSERT INTO warehouses (name, company_id) VALUES (?, ?)", (w_name, comp_id))
                    cur.execute("SELECT @@IDENTITY")
                    r_new = cur.fetchone()
                    wh_id = r_new[0] if r_new else None
                ...
                if wh_id:
                    ...
                    """, (part_no, wh_id, qty))
            conn.commit()
            log.debug("[sync] %s - warehouse stock synced for %d warehouse(s)", part_no, len(raw_warehouses))
        except Exception as e:
            log.warning("[sync] product_warehouse_stock upsert failed for %s: %s", part_no, e)
...
def sync_gl_accounts(api_key: str, api_secret: str,
    ...
    try:
        ...
        if get_system_mode().lower() == "saas":
            log.info("[sync] SaaS mode active - skipping /api/resource/Account GL account fetch.")
            print("[sync] ℹ️ SaaS mode active - skipping /api/resource/Account GL account fetch.", flush=True)
            return 0
    except Exception:
        pass
    ...
    for account_type in account_types_to_sync:
        ...
        filters_list = [["account_type", "=", account_type]]
        eff_company = (company or "").strip()
        if eff_company:
            filters_list.append(["company", "=", eff_company])
        ...
        print(f"[sync] 🌐 Fetching GL Accounts ({account_type}) URL: {url}", flush=True)
    ...
    print(f"[sync] [OK] GL accounts synced (Cash/Bank only): {count}")
    return count
...
def sync_exchange_rates(api_key: str, api_secret: str,
    ...
    for curr in sorted(currencies):
        ...
        try:
            ...
            print(f"[sync] 🌐 Fetching Exchange Rate ({curr} -> {base_curr}) URL: {url}", flush=True)
            req = urllib.request.Request(url)
            auth_hdr = build_auth_header(api_key, api_secret)
            if auth_hdr:
                req.add_header("Authorization", auth_hdr)
            ...
            if rate > 0:
                ...
                print(
                    f"[sync] [OK] Rate synced: 1 {curr} = {rate:.8f} {base_curr}  "
            ...
            try:
                ...
                pass
            log.warning(
                "[rates] HTTP %d fetching rate for %s -> %s",
                e.code, curr, base_curr,
            )
            print(f"[sync] ⚠ Exchange rate for {curr} -> {base_curr} not configured on server (HTTP {e.code})")
    ...
    log.info(
        "[rates] [OK] %d exchange rate pair(s) synced, %d error(s) (base=%s).",
        count, errors, base_curr,
    )
    print(f"[sync] [OK] Exchange rates: {count} synced, {errors} failed (base={base_curr})")
    return count
...
def sync_modes_of_payment(api_key: str, api_secret: str,
                           host: str, company: str) -> int:
    from models.gl_account import upsert_mop, upsert_account

    auth_hdr = build_auth_header(api_key, api_secret)
    eff_company = (company or "").strip()

    candidate_urls = [
        f"{host}/api/method/havano_pos_integration.api.get_account",
        f"{host}/api/method/saas_api.www.api.get_modes_of_payment?company={urllib.parse.quote(eff_company)}",
        f"{host}/api/method/saas_api.www.api.get_account",
    ]

    payload = None
    for url in candidate_urls:
        print(f"[sync] 🌐 Fetching Payment Methods / GL Accounts URL: {url}", flush=True)
        req = urllib.request.Request(url)
        if auth_hdr:
            req.add_header("Authorization", auth_hdr)
        try:
            with safe_urlopen(req, timeout=30) as r:
                data = json.loads(r.read().decode())
                # Check if data contains a valid non-empty list in data or message
                test_list = []
            ...
            log.warning("Payment method fetch failed for %s: %s", url, e)
    ...
    if isinstance(payload, list):
        ...
        if isinstance(msg, dict):
            mop_list = msg.get("data") or []
        else:
            mop_list = msg if isinstance(msg, list) else []

    # ── Fallback 1: Try standard Frappe resource endpoints if candidate endpoints returned no MOPs ──
    if not mop_list:
        try:
            frappe_mop_url = f"{host}/api/resource/Mode%20of%20Payment?fields=%5B%22name%22%2C%20%22type%22%5D&limit_page_length=500"
            print(f"[sync] 🌐 Fetching Frappe Mode of Payment URL: {frappe_mop_url}", flush=True)
            f_req = urllib.request.Request(frappe_mop_url)
            if auth_hdr:
                f_req.add_header("Authorization", auth_hdr)
            with safe_urlopen(f_req, timeout=30) as r:
                f_mops = json.loads(r.read().decode()).get("data", [])

            acct_map = {}
            try:
                f_accts_url = f"{host}/api/resource/Mode%20of%20Payment%20Account?fields=%5B%22parent%22%2C%20%22company%22%2C%20%22default_account%22%5D&limit_page_length=500"
                f_acct_req = urllib.request.Request(f_accts_url)
                if auth_hdr:
                    f_acct_req.add_header("Authorization", auth_hdr)
                with safe_urlopen(f_acct_req, timeout=30) as r_acct:
                    for arow in json.loads(r_acct.read().decode()).get("data", []):
                        p = arow.get("parent")
                        if p and arow.get("default_account"):
                            acct_map[p] = arow.get("default_account")
            except Exception:
                pass
            ...
            for fmop in f_mops:
                ...
                if mname:
                    mop_list.append({
                        "name": mname,
                        "account_name": mname,
                        "type": fmop.get("type") or "Cash",
                        "default_account": acct_map.get(mname, ""),
                    })
        except Exception as e_f:
            log.warning("Frappe standard MOP resource fetch failed: %s", e_f)

    count = 0
    synced_names = []
    for mop in mop_list:
        # Support havano_pos_integration.api.get_account, standard Frappe structure, and saas_api
        mop_name = (mop.get("account_name") or mop.get("name") or "").strip()
        gl_acct  = (mop.get("name") or mop.get("default_account") or mop_name).strip()
        if not mop_name:
            continue
        try:
            base_ccy = ""
            try:
                ...
                base_ccy = str(defs.get("server_company_currency") or "").strip()
            except Exception:
                base_ccy = ""

            raw_currency = (mop.get("account_currency") or mop.get("currency") or "").strip()
            currency = raw_currency or base_ccy
            mop_type = (mop.get("account_type") or mop.get("type") or "Cash").strip()

            # Upsert GL account as well so payment_dialog join can resolve it
            try:
                upsert_account({
                    "name":             gl_acct,
                    "account_name":     mop_name,
                    "company":          mop.get("company") or company,
                    "parent_account":   mop.get("parent_account", ""),
                    "account_type":     mop_type,
                    "account_currency": currency,
                })
            except Exception as _ex_gl:
                log.debug("GL account upsert during MOP sync: %s", _ex_gl)
            ...
            synced_names.append(mop_name.lower())
            log.debug("MOP synced: %s -> GL: %s (%s)", mop_name, gl_acct, currency)
            count += 1
        except Exception as e:
            log.warning("Failed to upsert MOP '%s': %s", mop_name, e)

    # In cloud / SaaS mode, purge local modes of payment that do not exist on the backoffice
    if synced_names:
        try:
            from database.db import get_connection
            conn_mop = get_connection()
            cur_mop = conn_mop.cursor()
            placeholders = ",".join(["?"] * len(synced_names))
            cur_mop.execute(f"DELETE FROM modes_of_payment WHERE LOWER(name) NOT IN ({placeholders})", synced_names)
            cur_mop.execute(f"""
            ...
            conn_mop.commit()
            conn_mop.close()
            log.info("Purged obsolete local modes_of_payment not present in backoffice API.")
        except Exception as purge_err:
            log.warning("Failed to purge obsolete modes_of_payment: %s", purge_err)

    log.info("Modes of Payment synced: %d", count)
    print(f"[sync] [OK] Modes of Payment synced: {count}")
    return count
...
def format_sync_result(result: dict) -> str:
    ...
    lines = [
        f"[OK] Sync complete  ({result.get('total_api', 0)} products on server)",
...
try:
    ...
    class SyncWorker(QObject):
        ...
        def run(self) -> None:
            ...
            log.info(
                "[sync] SyncWorker started (initial product/MOP sync)"
            )
                ...
                try:
                    ...
                    if get_system_mode() == "odoo":
                        try:
                            ...
                            log.error("[sync] Odoo background sync failed: %s", e)
                    ...
                    if now - last_product_sync >= PRODUCT_SYNC_INTERVAL_SECONDS:
                        # ── 1. Product + tax sync via windows-service script (FRAPPE) ──
                        try:
                        ...
                        try:
                            ...
                            if not has_credentials():
                                ...
                                if api_key:
                                    ...
                                    # Sync customers and users on the same interval
                                    cust_res = sync_customers()
                                    if cust_res and cust_res.get("inserted", 0) > 0:
                                        log.info(f"[sync] {cust_res['inserted']} new customers synced from Frappe")
                                        
                                    user_res = sync_users()
                                    if user_res and user_res.get("synced", 0) > 0:
                                        log.info(f"[sync] {user_res['synced']} users synced from Frappe")

                                    log.info("[sync] Periodic GL/MOP/Rates/Customers/Users sync complete")
                        except Exception as e:
                    ...
                    try:
                        ...
                        if res.success_count > 0:
                            log.info(f"[sync] [OK] Fiscalized {res.success_count} pending sales")
                    except Exception as e:
...
def _get_products_endpoint():
    ...
    method = "havano_pos_integration.api.get_products" if mode == "frappe" else "saas_api.www.api.get_my_products"

    try:
        from services.site_config import get_host
        host = get_host()
        if host:
            return f"{host}/api/method/{method}"
    except Exception:
        pass
    
    # Fallback to checking DB directly
    try:
        from database.db import get_connection
        conn = get_connection()
        cur = conn.cursor()
