...
class _InlineSettingsDialog(QDialog):
    ...
    def _page_pos_rules(self) -> QWidget:
        ...
        _rule_row("block_zero_stock",  "Block Zero / Negative Stock Sales",
                  ...
                  default=False)
        _rule_row("use_pricing_rules", "Apply Pricing Rules",
                  "Automatically apply discounts from pricing rules when adding items.",
                  default=False)
...
class AdminDashboard(QWidget):
    ...
    def _build(self):
        ...
        # No separate nav bar — the QMainWindow menu bar + white POSView bar
        # are the single unified header. The dashboard just starts with tabs.
        ...
        self._tabs.addTab(self._build_shift_history_tab(), qta.icon("fa5s.calendar-alt"), "Shift Reconciliations")
    ...
    def _build_shift_history_tab(self):
        ...
        # Top header panel
        hdr = QHBoxLayout()
        title_lbl = self._section_label("Shift Reconciliations History")
        ...
        mod_logs_btn.setFixedHeight(34)
        mod_logs_btn.setCursor(Qt.PointingHandCursor)
        mod_logs_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: #f57c00; color: white; border: none;
        ...
        view_details_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background-color: {ACCENT_H if 'ACCENT_H' in globals() else '#1b4a82'}; }}
        """)
        view_details_btn.clicked.connect(self._on_view_details_clicked)
        hdr.addWidget(view_details_btn)
        p1_lay.addLayout(hdr)

        self.shift_history_table = QTableWidget(0, 5)
        self.shift_history_table.setHorizontalHeaderLabels(
            ["Date", "Shift #", "Expected Amount", "Actual Amount", "Variance"]
        )
        ...
        self.shift_history_table.setSelectionMode(QAbstractItemView.SingleSelection)
        ...
        """)
        self.shift_history_table.setItemDelegate(delegate)
        p1_lay.addWidget(self.shift_history_table, 1)
        ...
        # Detail header row
        hdr2 = QHBoxLayout()
        back_btn = QPushButton("  Back to Shifts")
        ...
        # Mode header row
        hdr3 = QHBoxLayout()
        back_btn3 = QPushButton("  Back to Cashiers")
        back_btn3.setIcon(qta.icon("fa5s.arrow-left", color=WHITE))
        ...
        self.cashier_mode_title.setStyleSheet(f"""
            ...
            background:transparent; padding-left:12px;
        """)
        hdr3.addWidget(self.cashier_mode_title, 1)
        ...
        # Header row
        hdr4 = QHBoxLayout()
        back_btn4 = QPushButton("  Back to Shifts")
        back_btn4.setIcon(qta.icon("fa5s.arrow-left", color=WHITE))
...
class POSView(QWidget):
    ...
    def _build_nav(self):
        ...
        # ── Sales — two always-visible buttons ────────────────────────────────
        _sales_btn = _nb("Sales", self._open_sales_list, color=ACCENT, hov=ACCENT_H)
        _sales_btn.setIcon(qta.icon("fa5s.clipboard", color="white"))
        layout.addWidget(_sales_btn)
        
       

        # ── Maintenance ───────────────────────────────────────────────────────
        layout.addWidget(_nb("Maintenance", self._open_settings))
        ...
        laybye_btn.setFixedHeight(32); laybye_btn.setMinimumWidth(100)
        ...
        _payments_btn = _nb("Payments", self._open_customer_payment_entry, color=ACCENT, hov=ACCENT_H)
        _payments_btn.setIcon(qta.icon("fa5s.money-bill", color="white"))
        layout.addWidget(_payments_btn)
        layout.addSpacing(4)

        # ── Quotation button ───────────────────────────────────────────────────
        quote_btn = QPushButton("Quote")
        quote_btn.setIcon(qta.icon("fa5s.file", color="white"))
        quote_btn.setFixedHeight(32); quote_btn.setMinimumWidth(90)
        quote_btn.setCursor(Qt.PointingHandCursor)
        quote_btn.setStyleSheet(f"""
            QPushButton {{
        ...
        """)
        quote_btn.setToolTip("Create a Quotation (clears cart; select customer first)")
        quote_btn.clicked.connect(self._on_quotation)
        layout.addWidget(quote_btn); layout.addSpacing(4)

        # ── Return mode indicator (hidden unless in CN flow) ───────────────────
        self._return_btn = _nb("Return", self._process_return, color=DANGER, hov=DANGER_H)
        self._return_btn.setIcon(qta.icon("fa5s.undo", color="white"))
        ...
        # Refresh queue badge on startup and every 5 s
        ...
        # ── Dashboard button (admin only, right side) ──────────────────────────
        try:
    ...
    def _get_currency_symbol(self) -> str:
        """Returns active currency symbol directly from server/company defaults."""
        try:
    ...
    def _build_invoice_table(self):
        ...
        dynamic_labels = ["Item No.", "Item Details", f"Amount ({sym})", "Qty", "Disc", "TAX", f"Total ({sym})"]
...
""")
        ...
        for r in range(ROWS):
            for c in range(COLS):
                ...
                if flat < len(page_products):
                    ...
                    # Prevent double-fire on touchscreens (touch + synth mouse)
                    btn.setAttribute(Qt.WA_AcceptTouchEvents, False)
    ...
    def _load_credit_note_into_table(self, cn):
        ...
        if cust:
            ...
            self._cust_btn.setIcon(qta.icon("fa5s.undo", color="white"))
    ...
    def _on_laybye(self):
        """
        Full Laybye flow:
          1. Customer must be selected FIRST. If cart has items but no customer,
             ask whether to clear the cart so they can select customer first.
          2. If no customer selected, open customer picker immediately.
          3. Collect cart items.
          4. Show LaybyeConfirmDialog — forces cashier to confirm customer.
          5. Show LaybyePaymentDialog — deposit (optional) + delivery date + order type.
          6. Save to sales_order table locally (synced=0).
          7. Refresh unsynced badge → sync picks it up in background.
        """
        if not _HAS_LAYBYE:
        ...
        # ── Permission check ─────────────────────────────────────────────────
        if not self._check_permission("allow_laybye", "Create Laybye"):
            return

        # ── Check if customer is selected ────────────────────────────────────
        if not self._selected_customer:
            # If cart already has items, ask user whether to clear cart
            existing_items = self._collect_invoice_items()
            if existing_items:
                reply = QMessageBox.question(
                    self, "Customer Required for Laybye",
                    "A Laybye requires a customer to be selected FIRST.\n\n"
                    "Your cart currently has items. To start a Laybye you must "
                    "select a customer first, which will clear the cart.\n\n"
                    "Clear the cart and select a customer now?",
                    QMessageBox.Yes | QMessageBox.No, QMessageBox.No
                )
                if reply != QMessageBox.Yes:
                    return
            ...
            if dlg.exec() != QDialog.Accepted or not dlg.selected_customer:
                QMessageBox.information(self, "Customer Required",
                                        "Please select a customer before creating a Laybye.")
                return
            self._selected_customer = dlg.selected_customer
    ...
    def _save_quotation(self):
        ...
        now = QDateTime.currentDateTime().toString("dd/MM/yyyy  hh:mm")
        W = 40
        lines = ["=" * W, "         HAVANO POS — QUOTATION",
                 f"  Date:      {now}", f"  Customer:  {cname}", "-" * W]
        for it in items:
            name = it["product_name"][:24]; qty = it["qty"]; price = it["price"]
            disc = it.get("discount", 0.0); line_tot = it["total"]
            qty_str = f"{int(qty)}" if qty == int(qty) else f"{qty:.2f}"
            lines.append(f"{name:<24} {qty_str:>3}x ${price:.2f}")
            if disc:
                lines.append(f"  Disc              -${qty*price*(disc/100):.2f}")
            lines.append(f"  {'─'*20}  ${line_tot:.2f}")
        lines += ["-" * W, f"  TOTAL:             ${total:.2f}", "=" * W,
                  "  This quotation is valid for 30 days.", "=" * W]

        from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QHBoxLayout
        dlg = QDialog(self); dlg.setWindowTitle("Quotation Preview"); dlg.setMinimumSize(400, 500)
        dlg.setStyleSheet(f"QDialog {{ background:{WHITE}; }}")
        lay = QVBoxLayout(dlg); lay.setContentsMargins(16, 16, 16, 16); lay.setSpacing(10)
        txt = QTextEdit(); txt.setReadOnly(True)
        txt.setFont(__import__("PySide6.QtGui", fromlist=["QFont"]).QFont("Courier New", 10))
        ...
        lay.addWidget(txt, 1)
        br = QHBoxLayout(); br.setSpacing(8)
        print_btn = QPushButton("Print"); print_btn.setFixedHeight(36); print_btn.setCursor(Qt.PointingHandCursor)
        ...
        close_btn = QPushButton("Close"); close_btn.setFixedHeight(36); close_btn.setCursor(Qt.PointingHandCursor)
        close_btn.setStyleSheet(f"QPushButton {{ background:{NAVY}; color:{WHITE}; border:none; border-radius:5px; font-size:13px; font-weight:bold; padding:0 20px; }} QPushButton:hover {{ background:{NAVY_2}; }}")
        print_btn.clicked.connect(lambda: (self._send_quotation_to_printer("\n".join(lines)), dlg.accept()))
        close_btn.clicked.connect(dlg.accept)
        br.addStretch(); br.addWidget(print_btn); br.addWidget(close_btn)
        lay.addLayout(br)
        dlg.exec()
...
class CustomerPaymentDialog(QDialog):
    ...
    - Saves to local DB + queues for cloud sync
    ...
    def _print_payment_slip(self, payment_id, sync_status: str = "queued"):
...
class CreditNoteManagerDialog(QDialog):
    ...
    def _build(self):
        ...
        s = QLabel("Select credit notes and push to server.")
        ...
        self._tbl.setHorizontalHeaderLabels(
            ["", "CN NUMBER", "INVOICE", "CUSTOMER", "TOTAL", "STATUS", "SYNC REF"])
        hh = self._tbl.horizontalHeader()
    ...
    def _sync_selected(self):
        ...
        if no_ref:
            QMessageBox.information(self, "Some Skipped",
                f"{no_ref} credit note(s) skipped — original sale not yet synced.\n"
