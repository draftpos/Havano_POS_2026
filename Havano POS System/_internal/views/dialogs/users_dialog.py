...
class _UserFormDialog(QDialog):
    ...
    def _build(self):
        ...
        self._f_disc_expiry.setFixedHeight(FIELD_H)
        self._f_disc_expiry.setStyleSheet(f"""
            ...
            QDateEdit:focus {{ border:2px solid {ACCENT}; }}
            QDateEdit::drop-down {{ border:none; width:22px; }}
        """)
        fl.addLayout(_field_row("Disc. Expires", self._f_disc_expiry))

        disc_note = QLabel("Discount blocked after this date even if % is set.")
        disc_note.setStyleSheet(
            f"font-size:10px;color:{MUTED};font-style:italic;background:transparent;"
        )
        disc_note.setWordWrap(True)
        fl.addWidget(disc_note)
    ...
    def _populate(self, u: dict):
        ...
        # Discount expiry date
        expiry_str = u.get("discount_expiry_date", "")
        if expiry_str:
            try:
                d = QDate.fromString(expiry_str, "yyyy-MM-dd")
                if not d.isValid():
                    d = QDate.fromString(expiry_str, "dd/MM/yyyy")
                if d.isValid():
                    self._f_disc_expiry.setDate(d)
            except Exception:
                pass
    ...
    def _save(self):
        ...
        try:
            ...
            data = {
                ...
                "discount_expiry_date":  self._f_disc_expiry.date().toString("yyyy-MM-dd"),
