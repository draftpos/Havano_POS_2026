...
def _push_sale(sale: dict, api_key: str, api_secret: str,
    ...
    # Errors that are fixable on the Frappe/stock side — record them and keep
    # retrying so they remain visible in the Unsynced popup until resolved.
    _RETRYABLE_ERRORS = (
        "negativestockerror",        # stock shortage — fix stock on Frappe
        "not enough stock",
    )

    # Errors that are true permanent config problems — record AND stop retrying.
    _PERMANENT_ERRORS = (
        "not marked as sales item",
        "is not a sales item",
        "account is required",
    ...
    def _record_sync_error(code: str, raw_msg: str):
        """Save error to sync_errors table and push to the UI error bus."""
        ...
        try:
            ...
            sync_error_bus.post_error("POSUpload", inv_no, f"{code}: {raw_msg}")
        except Exception:
            pass
    ...
    for i, p in enumerate(attempts):
        ...
        try:
            with urllib.request.urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
                ...
                log.info("✅ %s → Frappe %s  customer=%s%s",
                         inv_no, name, p["customer"], suffix)
                # Clear any previous recorded error now that it succeeded
                try:
            ...
            # 409 — already exists on Frappe, treat as success
            if e.code == 409:
                log.info("Sale %s already exists on Frappe (409) — marking synced.", inv_no)
                try:
                    from services.sync_errors_service import resolve
            ...
            # Retryable errors (e.g. NegativeStockError) — record the raw Frappe
            # message so it appears in the Unsynced popup, but return False so the
            # sale stays in the queue and retries next cycle.
            if e.code == 417 and any(kw in msg.lower() for kw in _RETRYABLE_ERRORS):
                log.warning(
                    "⚠️  Sale %s — retryable Frappe error (keeping in queue).\n  %s",
                    inv_no, msg,
                )
                _record_sync_error(f"HTTP {e.code}", msg)
                return False   # ← stay in queue, show in UI, retry next cycle

            # Permanent config errors — record once then stop retrying
            if e.code == 417 and any(kw in msg.lower() for kw in _PERMANENT_ERRORS):
                log.warning(
                    "⚠️  Sale %s — permanent Frappe data error (recorded, marked synced).\n  %s",
                    inv_no, msg,
                )
                _record_sync_error(f"HTTP {e.code}", msg)
                return True    # ← mark synced to stop loop; error visible in UI
            ...
            # All other HTTP errors — record and retry next cycle
            _record_sync_error(f"HTTP {e.code}", msg)
            return False

        except urllib.error.URLError as e:
            reason = str(e.reason)
            log.warning("Network error pushing %s: %s", inv_no, reason)
            _record_sync_error("NETWORK", f"Cannot reach server: {reason} — check site URL in Company Defaults.")
            return False
