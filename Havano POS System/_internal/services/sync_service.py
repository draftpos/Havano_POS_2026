...
try:
    ...
    class SyncWorker(QObject):
        """
        Runs one full product + GL account + MOP sync cycle, then exits.
        main_window.py moves this onto a QThread — it runs once on login.
        """
        ...
        def run(self) -> None:
            ...
            # Product + tax sync via existing windows service script
            ...
            if not os.path.exists(script):
                ...
                log.info("[sync] Running product + tax sync: %s debug", script)
                print(f"[sync] Running product + tax sync...")
                try:
                    ...
                    if result.returncode != 0 and result.stderr:
                        log.error("[sync] sync error: %s", result.stderr.strip())
                except subprocess.TimeoutExpired:
                    log.error("[sync] product+tax sync timed out after 120s")
                except Exception as e:
                    log.error("[sync] product+tax sync error: %s", e)

            # GL accounts + MOP sync — runs directly (no subprocess needed)
            try:
                ...
                if api_key and api_secret:
                    ...
                    mop_count  = sync_modes_of_payment(api_key, api_secret, host, company)
                    rate_count = sync_exchange_rates(api_key, api_secret, host)
                    print(f"[sync] ✅ GL accounts: {gl_count}  |  MOPs: {mop_count}  |  Rates: {rate_count}")
                else:
                    log.warning("[sync] No credentials — skipping GL/MOP/rates sync.")
            except Exception as e:
                log.error("[sync] GL/MOP/rates sync error in SyncWorker: %s", e)
