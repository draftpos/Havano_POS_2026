...
class UnsyncedPopup(QDialog):
    ...
    def _fetch_rows(self, kind: str):
        ...
        # ── Primary: sync_errors table (SI / CN / SO only) ───────────────────
        if kind in ("SI", "CN", "SO"):
            try:
                from services.sync_errors_service import get_unresolved
                for e in get_unresolved(kind):
                    rows.append((
                        e.get("doc_ref")   or "—",
                        e.get("customer")  or "—",
                        f"${float(e.get('amount', 0)):.2f}",
                        # raw_error: prefer raw_message, fall back to error_msg
                        e.get("raw_message") or e.get("error_msg") or "—",
                    ))
                if rows:
                    return rows
            except Exception:
        ...
        # ── Fallback / direct fetch per type ─────────────────────────────────
        if kind == "SI":
            try:
                from models.sale import get_all_sales
            ...
            try:
                from database.db import get_connection
                conn = get_connection(); cur = conn.cursor()
                # Added sync_error to the fetch
                cur.execute("""
                    SELECT cn_number, customer_name, total, sync_error
                    FROM   credit_notes
                    WHERE  cn_status IN ('ready','pending_sync')
                    ORDER  BY id DESC
                """)
                for cn_no, cust, amt, err in cur.fetchall():
                    rows.append((
                        cn_no or "—", cust or "—",
                        f"${float(amt or 0):.2f}",
                        err or "Pending",
            ...
            try:
                from models.sales_order import get_unsynced_orders
                for o in get_unsynced_orders():
                    rows.append((
                        o.get("order_no") or f"#{o.get('id', '')}",
            ...
            try:
                # 1. Fetch from customer_payments
                from models.payment import get_unsynced_payments
                std_payments = (get_unsynced_payments() or [])
                for p in std_payments:
                    rows.append((
                        p.get("reference") or f"PAY-CP-{p.get('id', 0):07d}",
                        p.get("customer_name") or "Walk-in",
                        f"${float(p.get('amount', 0)):.2f}",
                        p.get("sync_error") or "Pending...",
                    ))

                # 2. Fetch from payment_entries (Refunds / PE 32 / etc.)
                from database.db import get_connection, fetchall_dicts
                conn = get_connection(); cur = conn.cursor()
                cur.execute("""
                    SELECT id, reference_no, party, paid_amount, sync_error, last_error
                    FROM payment_entries 
                    WHERE synced = 0
                """)
                pe_rows = fetchall_dicts(cur); conn.close()
                for pe in pe_rows:
                    err = pe.get("sync_error") or pe.get("last_error") or "Pending..."
                    rows.append((
                        pe.get("reference_no") or f"PAY-PE-{pe.get('id', 0):07d}",
                        pe.get("party") or "Walk-in",
                        f"${float(pe.get('paid_amount', 0)):.2f}",
                        err,
                    ))
            except Exception as e:
                rows.append(("—", "—", "—", f"DB error: {e}"))
            ...
            try:
                ...
                for cid, cname, phone in cur.fetchall():
                    rows.append((
                        f"CUST-{cid}",
                        cname or "—",
                        phone or "—",          # reuses the "amount" column for phone
                        "Not yet synced",
                    ))
                conn.close()
            except Exception as e:
            ...
            try:
                if kind == "SI":
                    ...
                    try:
                        ...
                        print(f"Failed to trigger payment entry sync: {e}")
                elif kind == "CUST":
                    from models.customer import retry_unsynced_customers
                    retry_unsynced_customers()
            except Exception as e:
                print(f"Retry error for {kind}: {e}")
...
class POSView(QWidget):
    ...
    def _build_nav(self):
        ...
        self._unsynced_timer.setInterval(3000)  # 3 seconds — worker is async so no UI blocking
    ...
    def _refresh_unsynced_badge(self):
        ...
        # If a worker is already running, skip this tick to avoid pile-up
        ...
        class _BadgeWorker(QThread):
            done = _Signal(int, int, int, int, int)   # si, cn, so, pay, cust
            ...
            def run(self):
                ...
                try:
                    ...
                    # CN — credit notes not yet synced (only those that still need it)
                    try:
                        conn = get_connection()
                        cur = conn.cursor()
                        cur.execute(
                            "SELECT COUNT(*) FROM credit_notes "
                            "WHERE cn_status != 'synced'"
                        )
                    ...
                    # SO — unsynced sales orders / laybyes
                    try:
                        conn = get_connection()
                        cur = conn.cursor()
                        cur.execute(
                    ...
                    # PAY — unsynced payment entries whose sale is already synced
                    # (entries linked to an unsynced sale are already counted under SI)
                    try:
                        conn = get_connection()
                        cur = conn.cursor()
                        cur.execute(
                            ...
                            " AND (pe.sale_id IS NULL OR EXISTS ("
                            "   SELECT 1 FROM sales s "
                            "   WHERE s.id = pe.sale_id AND s.synced = 1"
                            "))"
                        )
                    ...
                    # CUST — locally-created customers not yet pushed to Frappe
                    try:
                        conn = get_connection()
                        cur = conn.cursor()
                        cur.execute(
                ...
                self.done.emit(si, cn, so, pay, cust)

        worker = _BadgeWorker(self)

        def _on_done(si, cn, so, pay, cust):
            self._badge_worker_running = False
            self._apply_badge_counts(si, cn, so, pay, cust)
    ...
    def _apply_badge_counts(self, si_count, cn_count, so_count, pay_count, cust_count):
        """Update the Q-badge UI — always called on the main thread via signal."""
        if not hasattr(self, "_q_badge"):
            return

        total = si_count + cn_count + so_count + pay_count + cust_count
        btn = self._q_badge
        ...
        if total == 0:
            ...
            bg, hov = (AMBER, ORANGE) if total < 5 else (DANGER, DANGER_H)
            suffix = f"{total} !" if total >= 5 else str(total)
            btn.setText(f"Q : {suffix}")
            btn.setToolTip(
                f"{total} unsynced record(s) — click to view\n"
    ...
    def _open_company_defaults_nav(self):
        """Open company defaults from nav-bar hover menu."""
        try:
    ...
    def _require_active_shift(self) -> bool:
        """
        Return True when a shift is running.
        If no shift is active, show a styled prompt with a 'Start Shift' button
        that opens the existing ShiftChooserDialog — exactly as the right-panel
        button does.  Nothing else in the UI is changed.
        """
        try:
            ...
            return True   # can't check → don't block (fail open)

        # ── Build a custom dialog so we can wire a real action button ─────────
        dlg = QDialog(self)
        ...
        def _launch_shift():
            dlg.accept()
            # Reuse the exact same flow as the right-panel Start/Close Shift button
            self._open_shift_chooser()
            # Refresh the right-panel shift button label after the dialog closes
        ...
        dlg.exec()
        return False   # always block the original action; user restarts it after shift begins
...
""")
    ...
    def _open_payment(self):
        """
        Router: Redirects to Laybye flow if toggled ON (Orange state), 
        otherwise proceeds with standard POS payment.
        """
        # ── 0. SHIFT GUARD ───────────────────────────────────────────────────────────────
        if not self._require_active_shift():
            return
        ...
        if hasattr(self, 'laybye_btn') and self.laybye_btn.isChecked():
            ...
            # Ensure your separate logic function is named exactly this:
        ...
        # If none is selected, open the customer picker now.
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            try:
                ...
                # Store local payment entry for sync
                try:
                    ...
                    _lg.getLogger("POS").warning("Could not create local payment entry: %s", _pe_err)

                # ── 5. UI FEEDBACK & CLEANUP ────────────────────────────────
    ...
    def _on_laybye(self):
        ...
        if self.laybye_btn.isChecked():
            ...
            # 2. Change Main Action Button (The Big Pay Button) to Deposit
            # Note: We keep the styling here because btn_pay is usually elsewhere,
            # but we removed the self.laybye_btn.setStyleSheet line.
            ...
            # 3. Reset Main Action Button to Standard Green
    ...
    def _execute_laybye_transaction(self):
        ...
        DIRECT FLOW: Skips the confirmation dialog and goes straight to Payment.
        """
        # ── Shift guard (Laybye is also a transaction) ───────────────────────────
        if not self._require_active_shift():
            return
        ...
        # ── 2. CUSTOMER VALIDATION (Replaces Confirm Dialog) ──────────────────
        # Laybye requires a named customer (not walk-in)
        ...
        if not self._selected_customer or _is_walk_in(self._selected_customer):
            QMessageBox.warning(self, "Customer Required", 
                                "A named customer is required for Laybye.\n"
                                "Please select a customer now.")
            self._select_customer()
            # Check again after picker closes
            if not self._selected_customer or _is_walk_in(self._selected_customer):
                return # User cancelled or didn't select a valid customer
        ...
        deposit_dlg = LaybyePaymentDialog(
            parent=self, total=cart_total, customer=confirmed_customer,
        )
        ...
        so_items = [{"item_code": it.get("part_no", ""), "item_name": it.get("product_name", ""),
                     "qty": it.get("qty", 1), "rate": it.get("price", 0.0), 
                     "amount": it.get("total", 0.0)} for it in cart_items]

        try:
            order_id = _create_sales_order(
                cart_items=so_items, total=cart_total, deposit_amount=deposit_amount,
                deposit_method=deposit_method, customer=confirmed_customer,
                company=company_name, delivery_date=delivery_date, order_type=order_type,
        ...
        # ── 4b. Print ────────────────────────────────────────────────
        # Split payment entries are now handled automatically inside create_sales_order

        if _HAS_SO_PRINT:
            try:
                from services.printing_service import print_laybye_deposit as _print_laybye_deposit
        ...
        if hasattr(self, 'laybye_btn'):
            self.laybye_btn.setChecked(False)
            # Manually trigger switcher to revert colors/text to Green/Standard
            self._on_laybye() 
    ...
    def _save_quotation(self):
        ...
        lines += ["=" * W, "  This quotation is valid for 30 days.", "=" * W]

        from PySide6.QtWidgets import QDialog, QVBoxLayout, QTextEdit, QPushButton, QHBoxLayout
        dlg = QDialog(self); dlg.setWindowTitle("Quotation Preview"); dlg.setMinimumSize(400, 540)
        dlg.setStyleSheet(f"QDialog {{ background:{WHITE}; }}")
        lay = QVBoxLayout(dlg); lay.setContentsMargins(16, 16, 16, 16); lay.setSpacing(10)
        txt = QTextEdit(); txt.setReadOnly(True)
        txt.setFont(__import__("PySide6.QtGui", fromlist=["QFont"]).QFont("Courier New", 10))
...
# Clean UI — no icons, tight layout, simple dropdown, no extra fields
#
class CustomerPaymentDialog(QDialog):
    ...
    def _update_balance_highlights(self):
        ...
        # Outstanding — bold + colored when active, muted when not
        if self._payment_type == "outstanding":
            self._card_outstanding_title.setStyleSheet(
                f"font-size:12px; color:{self._NAVY}; font-weight:600; background:transparent;"
            )
            self._lbl_outstanding_bal.setStyleSheet(
                f"font-size:13px; font-weight:800; color:{self._NAVY}; background:transparent;"
                f" font-family:'Courier New',monospace;"
            )
        else:
            self._card_outstanding_title.setStyleSheet(
                f"font-size:12px; color:{self._MUTED}; background:transparent;"
            )
            self._lbl_outstanding_bal.setStyleSheet(
                f"font-size:13px; font-weight:500; color:{self._MUTED}; background:transparent;"
                f" font-family:'Courier New',monospace;"
            )
    ...
    def _build_header(self):
        """Slim navy header — customer name + badge only."""
        hdr = QWidget()
        hdr.setFixedHeight(44)
        ...
        name_lbl.setStyleSheet(
            f"color:{self._WHITE}; font-size:14px; font-weight:700; letter-spacing:0.3px;"
        )
        hl.addWidget(name_lbl)

        phone = (self._customer or {}).get("custom_telephone_number", "")
        group = (self._customer or {}).get("customer_group_name", "")
        if phone or group:
        ...
        badge.setStyleSheet(
            ...
            f" font-size:8px; font-weight:700; letter-spacing:1.2px; padding:3px 9px;"
        )
        hl.addWidget(badge)
        return hdr
    ...
    def _build_top_bar(self):
        """
        Two-row bar — plain, no borders, no drama.
          Row 1: Payment type selector (Outstanding / Laybye) + balances
          Row 2: Date  |  Account  |  Ref
        """
        bar = QWidget()
        ...
        # ── Row 1: type tabs + balances ───────────────────────────────────
        row1 = QHBoxLayout()
        row1.setSpacing(0)

        # Tab-style toggle — Outstanding
        ...
        # Outstanding balance — plain label pair
        self._card_outstanding = QWidget()  # kept as attribute for compat
        lout = QHBoxLayout(self._card_outstanding)
        lout.setContentsMargins(0, 0, 0, 0)
        lout.setSpacing(6)
        self._card_outstanding_title = QLabel("Outstanding:")
        self._card_outstanding_title.setStyleSheet(
            f"font-size:12px; color:{self._MUTED}; background:transparent;"
        )
        self._lbl_outstanding_bal = QLabel("$0.00")
        self._lbl_outstanding_bal.setStyleSheet(
            f"font-size:13px; font-weight:700; color:{self._NAVY}; background:transparent;"
            f" font-family:'Courier New',monospace;"
        )
        lout.addWidget(self._card_outstanding_title)
        lout.addWidget(self._lbl_outstanding_bal)
        row1.addWidget(self._card_outstanding)
        row1.addSpacing(20)

        # Laybye balance — plain label pair
        self._card_laybye = QWidget()
        llay = QHBoxLayout(self._card_laybye)
        llay.setContentsMargins(0, 0, 0, 0)
        llay.setSpacing(6)
        self._card_laybye_title = QLabel("Laybye:")
        self._card_laybye_title.setStyleSheet(
            f"font-size:12px; color:{self._MUTED}; background:transparent;"
        )
        ...
        llay.addWidget(self._card_laybye_title)
        llay.addWidget(self._lbl_laybye_bal)
        row1.addWidget(self._card_laybye)
        row1.addStretch()
        bl.addLayout(row1)
        ...
        self._acct_combo.setFixedHeight(28)
        self._acct_combo.setFixedWidth(190)
        for m in self._methods:
            self._acct_combo.addItem(f"{m['label']}  ({m['currency']})", userData=m)
        self._acct_combo.setStyleSheet(
            f"QComboBox {{ background:{self._OFF_WHITE}; color:{self._NAVY};"
        ...
        self._ref_input.setFixedWidth(160)
        self._ref_input.setPlaceholderText("Ref / Note")
        self._ref_input.setStyleSheet(self._input_style())

        row2.addWidget(self._date_edit)
        row2.addWidget(self._acct_combo)
    ...
    def _input_style(self) -> str:
        return (
            f"background:{self._OFF_WHITE}; color:{self._NAVY};"
            f" border:1px solid {self._BORDER}; border-radius:5px;"
            f" font-size:12px; padding:0 9px;"
        )
    ...
    def _refresh_tab_styles(self):
        ...
        self._btn_outstanding.setStyleSheet(
            active_style if self._payment_type == "outstanding" else inactive_style
        )
        self._btn_laybye.setStyleSheet(
            active_style if self._payment_type == "laybye" else inactive_style
        )
    ...
    def _build_left(self):
        ...
        amt_card = QFrame()
        amt_card.setFixedHeight(56)
        amt_card.setStyleSheet(
            f"QFrame {{ background:{self._WHITE}; border:1.5px solid {self._BORDER}; border-radius:8px; }}"
        )
        acl = QVBoxLayout(amt_card)
        acl.setContentsMargins(16, 6, 16, 6)
        acl.setSpacing(1)
        cap = QLabel("AMOUNT  (USD)")
        cap.setAlignment(Qt.AlignCenter)
        cap.setStyleSheet(
            f"color:{self._MUTED}; font-size:8px; font-weight:700; letter-spacing:1.2px;"
        )
        self._total_lbl = QLabel("0.00")
        self._total_lbl.setAlignment(Qt.AlignCenter)
        self._total_lbl.setStyleSheet(
            f"color:{self._NAVY}; font-size:22px; font-weight:800;"
            f" font-family:'Courier New',monospace;"
        )
        acl.addWidget(cap)
        ...
        for txt, st, al in [
            ("METHOD",  4, Qt.AlignLeft),
            ("CCY",     1, Qt.AlignCenter),
            ("AMOUNT",  3, Qt.AlignRight),
        ]:
            lh = QLabel(txt)
            lh.setStyleSheet(
                f"color:{self._MUTED}; font-size:8px; font-weight:700; letter-spacing:0.8px;"
            )
            lh.setAlignment(al)
            chl.addWidget(lh, st)
        vbox.addWidget(ch)
    ...
    def _build_right(self):
        ...
        def _nb(text, kind="digit"):
            ...
            styles = {
                ...
                "clear": ("#8c1515",    "#a81c1c",     self._WHITE),
            }
            bg, hov, fg = styles.get(kind, styles["digit"])
            btn.setStyleSheet(
        ...
        bclr  = _nb("CLR", "clear");  bclr.clicked.connect(self._nb_clear);  grid.addWidget(bclr,  0, 1)
        bcan  = _nb("Cancel", "clear"); bcan.setIcon(qta.icon("fa5s.times", color="white"))
        bcan.clicked.connect(self.reject); grid.addWidget(bcan, 0, 2, 1, 2)

        for ri, (digs, qa) in enumerate([("789", 10), ("456", 20), ("123", 50), ("0.", 100)], 1):
        ...
        bsave.setStyleSheet(
            ...
            f" border-radius:6px; font-size:13px; font-weight:700; letter-spacing:0.4px; }}"
            f"QPushButton:hover {{ background:{self._SUCCESS_H}; }}"
            f"QPushButton:pressed {{ background:#165f30; }}"
        )
    ...
    def _method_style(self, active: bool) -> str:
        ...
        return (
            f"QPushButton {{ background:{self._WHITE}; color:{self._NAVY};"
            f" border:1px solid {self._BORDER}; border-radius:5px;"
    ...
    def _field_style(self, active: bool) -> str:
        border = f"1.5px solid {self._NAVY}" if active else f"1px solid {self._BORDER}"
        bg     = self._WHITE
        return (
            f"QLineEdit {{ background:{bg}; color:{self._NAVY};"
            f" border:{border}; border-radius:5px;"
            f" font-size:14px; font-weight:700; padding:0 8px; }}"
        )
        ...
        if self._payment_type == "laybye":
            ...
            if total_usd > laybye:
                QMessageBox.warning(self, "Amount Exceeds Balance",
                                    f"Payment (${total_usd:.2f}) exceeds laybye balance (${laybye:.2f}).")
                return
        else:
            if total_usd > outstanding and outstanding > 0:
                reply = QMessageBox.question(
                    self, "Amount Exceeds Balance",
                    f"Payment (${total_usd:.2f}) exceeds outstanding balance (${outstanding:.2f}).\n"
                    f"This will create a credit. Continue?",
                    QMessageBox.Yes | QMessageBox.No, QMessageBox.No
                )
                if reply != QMessageBox.Yes:
                    return
    ...
    def _print_payment_slip(self, payment_id, sync_status: str = "queued"):
        ...
        if self._payment_type == "laybye":
            lines += [f"  Type       : LAYBYE PAYMENT",
                      f"  Balance    : ${laybye:.2f}"]
        else:
            lines += [f"  Type       : OUTSTANDING BALANCE",
        ...
        close_btn.setStyleSheet(
            f"QPushButton {{ background:{self._NAVY}; color:{self._WHITE}; border:none;"
            f" border-radius:5px; font-size:12px; font-weight:700; padding:0 18px; }}"
            f"QPushButton:hover {{ background:{self._NAVY_2}; }}"
        )
        close_btn.clicked.connect(dlg.accept)
        br = QHBoxLayout()
...
class AdvanceSettingsDialog(QDialog):
    ...
    def _reset_to_defaults(self):
        ...
        self.sb_content_size.setValue(max(6, int(default.contentFontSize or 0)))
...
# Self-contained — all colours, helpers, and logic in one file.
# =============================================================================
...
# =============================================================================
# COLOUR PALETTE  (self-contained — no dependency on main_window)
# =============================================================================
NAVY      = "#0d1f3c"
NAVY_2    = "#162d52"
NAVY_3    = "#1e3d6e"
...
def _btn(label: str, color: str, hover: str, height: int = 40) -> QPushButton:
    """Reusable styled button — replaces navy_btn from main_window."""
    b = QPushButton(label)
    ...
    b.setStyleSheet(f"""
        ...
        QPushButton:disabled {{
            background-color: {LIGHT}; color: {MUTED};
        }}
    """)
    return b
...
def _load_payment_rows_from_shift() -> list[str]:
    """
    Priority order:
      1. Active shift's own shift_rows  (most reliable — exactly what was opened)
      2. GL accounts filtered by payment type
    ...
    try:
        ...
        if active and active.get("rows"):
            methods = sorted({r["method"] for r in active["rows"] if r.get("method")})
            if methods:
                return methods
    except Exception as e:
        print(f"[Recon] Could not load shift rows: {e}")

    # 2 — GL accounts
    try:
        from models.gl_account import get_all_accounts
        accounts = get_all_accounts() or []
        payment_types = {"Cash", "Bank", "Payment Account", "Receivable"}
        methods = sorted(
            acc.get("name", "")
            for acc in accounts
            if any(pt in acc.get("account_type", "") for pt in payment_types)
            or "cash" in acc.get("name", "").lower()
            if acc.get("name")
        )
        if methods:
            return methods
    except Exception as e:
        print(f"[Recon] Could not load GL accounts: {e}")

    # 3 — fallback
    return ["CASH", "CHECK", "C / CARD", "AMEX", "DINERS", "EFTPOS"]
...
class ShiftReconciliationDialog(QDialog):
    """
    Shows each payment method with:
      • Expected  = start_float + income  (from shift_rows, refreshed live)
      • Actual    = cashier enters this
      • Variance  = Actual − Expected
    Clicking "Finalise" calls end_shift(), which itself refreshes income one
    final time before writing the counted values.
    """
    ...
    def __init__(self, parent=None, cashier_id=None):
        ...
        # Refresh DB income before we read anything
        self._active_shift = None
        self._refresh_shift_income()

        self.PAYMENT_ROWS = _load_payment_rows_from_shift()
        self._build_ui()
        self._load_expected_data()

    # ── Income refresh ────────────────────────────────────────────────────────

    def _refresh_shift_income(self):
        """
        Call refresh_income() so shift_rows.income reflects all sales made
        during this shift before we display the Expected column.
        """
        try:
            from models.shift import get_active_shift, refresh_income
            self._active_shift = get_active_shift()
            if self._active_shift:
                income = refresh_income(self._active_shift["id"])
                print(f"[Recon] Income refreshed for shift "
                      f"{self._active_shift['id']}: {income}")
                # Re-fetch so the rows contain the updated income values
                from models.shift import get_shift_by_id
                self._active_shift = get_shift_by_id(self._active_shift["id"])
            else:
                print("[Recon] No active shift found.")
        except Exception as e:
            print(f"[Recon] Error refreshing income: {e}")
    ...
    def _build_ui(self):
        ...
        # Shift info line
        self._info_lbl = QLabel("Loading shift info…")
        self._info_lbl.setStyleSheet(f"color: {MUTED}; font-size: 11px;")
        lay.addWidget(self._info_lbl)

        instr = QLabel("Count your drawer and enter the actual amounts below:")
        instr.setStyleSheet(f"color: {MUTED}; font-size: 11px;")
        lay.addWidget(instr)

        # Main table: Method | Expected | Actual (editable) | Variance
        self.table = QTableWidget(0, 4)
        self.table.setHorizontalHeaderLabels(
            ["Method", "Expected", "Actual (Counted)", "Variance"])
        hh = self.table.horizontalHeader()
        hh.setSectionResizeMode(0, QHeaderView.Stretch)
        hh.setSectionResizeMode(1, QHeaderView.Fixed);  self.table.setColumnWidth(1, 110)
        hh.setSectionResizeMode(2, QHeaderView.Fixed);  self.table.setColumnWidth(2, 130)
        hh.setSectionResizeMode(3, QHeaderView.Fixed);  self.table.setColumnWidth(3, 100)
        self.table.verticalHeader().setVisible(False)
        self.table.setAlternatingRowColors(True)
        self.table.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        ...
        self.lbl_summary.setStyleSheet(f"""
            font-weight: bold; font-size: 15px; color: {NAVY};
            padding: 10px; background: {LIGHT}; border-radius: 5px;
        """)
        ...
        self.close_btn = _btn("Finalise & Close Shift", SUCCESS, SUCCESS_H, height=45)
        self.close_btn.setIcon(qta.icon("fa5s.check", color="white"))
        self.close_btn.clicked.connect(self._on_finalize)

        cancel_btn = _btn("↩  Back to POS", DANGER, DANGER_H, height=45)
        cancel_btn.clicked.connect(self.reject)
    ...
    def _load_expected_data(self):
        """
        Populate the table from the (already-refreshed) active shift rows.
        Uses a case-insensitive, whitespace-trimmed lookup so GL account name
        variations never cause a miss.
        """
        if not self._active_shift:
            QMessageBox.warning(self, "No Active Shift",
                                "No open shift was found.\n"
                                "Please start a shift before reconciling.")
            self.close_btn.setEnabled(False)
            return

        # Shift info label
        snum = self._active_shift.get("shift_number", "?")
        sdate = self._active_shift.get("date", "")
        stime = self._active_shift.get("start_time", "")
        user  = self._active_shift.get("username", "")
        self._info_lbl.setText(
            f"Shift #{snum}  ·  {sdate}  ·  Started {stime}  ·  Cashier: {user}")

        # Build a normalised expected map: lower-stripped-name -> total
        expected_map: dict[str, float] = {}
        raw_map: dict[str, float] = {}       # original-case key -> total
        for sr in self._active_shift.get("rows", []):
            method = sr.get("method", "")
            total  = float(sr.get("total", 0.0))   # start_float + income
            if method:
                raw_map[method] = total
                expected_map[method.strip().lower()] = total

        self.table.setRowCount(len(self.PAYMENT_ROWS))

        for i, method in enumerate(self.PAYMENT_ROWS):
            # Try exact match first, then normalised
            exp = raw_map.get(method,
                  expected_map.get(method.strip().lower(), 0.0))

            # Col 0 — method name
            m_item = QTableWidgetItem(method)
            m_item.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
            self.table.setItem(i, 0, m_item)

            # Col 1 — expected (read-only)
            e_item = QTableWidgetItem(f"{exp:.2f}")
            e_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
            e_item.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
            if exp > 0:
                e_item.setForeground(QColor(SUCCESS))
            self.table.setItem(i, 1, e_item)

            # Col 2 — actual input (editable QLineEdit in cell)
            actual_input = QLineEdit("0.00")
            actual_input.setAlignment(Qt.AlignRight)
            actual_input.textChanged.connect(
                lambda _, row=i: self._update_variance(row))
            self.table.setCellWidget(i, 2, actual_input)

            # Col 3 — variance (read-only, coloured)
            v_item = QTableWidgetItem("0.00")
        ...
        # Focus first actual-input cell
        if self.table.rowCount() > 0:
            first_input = self.table.cellWidget(0, 2)
            if first_input:
    ...
    def _update_variance(self, row: int):
        try:
            exp_text = (self.table.item(row, 1) or QTableWidgetItem("0")).text()
            act_widget = self.table.cellWidget(row, 2)
            act_text = act_widget.text().strip() if act_widget else "0"

            expected = float(exp_text or "0")
            actual   = float(act_text  or "0")
            variance = actual - expected
            ...
            v_item.setText(f"{variance:.2f}")
            v_item.setForeground(QColor(DANGER if variance < 0 else SUCCESS))
        except ValueError:
            pass
    ...
    def _update_total_summary(self):
        ...
        for r in range(self.table.rowCount()):
            try:
                total_var += float((self.table.item(r, 3) or QTableWidgetItem("0")).text())
            except (ValueError, AttributeError):
                pass
    ...
    def _on_finalize(self):
        ...
        for r in range(self.table.rowCount()):
            try:
                ...
                act_w    = self.table.cellWidget(r, 2)
                actual   = float(act_w.text().strip() or "0") if act_w else 0.0
                totals.append({"method": method, "expected": expected, "actual": actual})
            except (ValueError, AttributeError) as e:
                QMessageBox.warning(self, "Invalid Data",
                                    f"Row {r + 1} has invalid data: {e}")
                return

        try:
            from models.shift import end_shift, get_active_shift
            active = get_active_shift()
            if not active:
            ...
            # end_shift() internally calls refresh_income() one final time
            ...
            try:
                ...
                print(f"⚠️  Reconciliation print failed: {print_err}")
