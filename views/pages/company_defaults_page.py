...
class _ToggleSwitch(QWidget):
    ...
    def mousePressEvent(self, ev):
        self._pill.setChecked(not self.isChecked(), animated=True)
...
class CompanyDefaultsPage(QWidget):
    ...
    def _build(self):
        ...
        # ── CLOSE BUTTON - closes the parent QDialog, not the whole app ──────
        close_page_btn = QPushButton()
        close_page_btn.setIcon(qta.icon("fa5s.times", color="white"))
        close_page_btn.setFixedSize(38, 38)
        close_page_btn.setCursor(Qt.PointingHandCursor)
        close_page_btn.setToolTip("Close")
        close_page_btn.setStyleSheet(f"""
            QPushButton {{
                background:transparent; border:none; border-radius:4px;
            }}
            QPushButton:hover   {{ background:rgba(176,32,32,0.85); }}
            QPushButton:pressed {{ background:rgba(176,32,32,1.0);  }}
        """)
        close_page_btn.clicked.connect(self._close_parent)
        ...
        # Butchery Settings
        self._butchery_mode_chk = _ToggleSwitch("Butchery Settings", size=20)
        butchery_hint = QLabel(
            "When enabled, products marked as butchery will prompt for an amount "
            "when added to the cart, automatically calculating quantity based on unit price."
        )
        butchery_hint.setWordWrap(True)
        butchery_hint.setStyleSheet(
            f"color:{MUTED}; font-size:11px; background:{LIGHT};"
            f" border:1px solid {BORDER}; border-radius:6px; padding:10px 12px;"
        )
        pcl.addSpacing(12)
        pcl.addWidget(self._butchery_mode_chk)
        pcl.addSpacing(8)
        pcl.addWidget(butchery_hint)
        ...
        bc = _card()
        bcl = QVBoxLayout(bc)
        bcl.setContentsMargins(28, 20, 28, 24)
        bcl.setSpacing(ROW_SP)
        _section_header(bcl, "Banking Details  (printed on A4 Tax Invoices)", top_margin=0)

        self._bank_name = _inp()
        self._bank_name.setPlaceholderText("e.g. Stanbic Bank / CBZ / FBC")
        bcl.addLayout(_field_row("Bank Name", self._bank_name, lw=100))

        self._bank_acc_name = _inp()
        self._bank_acc_name.setPlaceholderText("e.g. HAVANO POS (PVT) LTD")
        bcl.addLayout(_field_row("Account Name", self._bank_acc_name, lw=100))

        self._bank_acc_no = _inp()
        self._bank_acc_no.setPlaceholderText("e.g. 91400012345678")
        bcl.addLayout(_field_row("Account No", self._bank_acc_no, lw=100))

        self._bank_branch = _inp()
        self._bank_branch.setPlaceholderText("e.g. Avondale / STICZWHX")
        bcl.addLayout(_field_row("Branch / SWIFT", self._bank_branch, lw=100))

        _section_header(bcl, "Additional Notes / Multi-Currency Accounts", top_margin=6)
        self._banking = QTextEdit()
        self._banking.setMinimumHeight(80)
        self._banking.setPlaceholderText(
            "Enter extra banking details or secondary currency accounts.\n"
            "Printed on full-page A4 tax invoices / quotations."
        )
        self._banking.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self._banking.setStyleSheet(f"""
            QTextEdit {{
                background:{WHITE}; color:{DARK_TEXT};
                border:1px solid {BORDER}; border-radius:6px;
                padding:10px 12px; font-size:13px;
        ...
        # ═════════════════════════════════════════════════════════════════════
        # ROW 3: ZIMRA Settings & Login Defaults
        # ═════════════════════════════════════════════════════════════════════
        row3 = QHBoxLayout()
        row3.setSpacing(16)
        row3.addWidget(zc,  1)
    ...
    def _close_parent(self):
        # Case 1: we're inside a QDialog -> close just that dialog
        p = self.parent()
        while p is not None:
            if isinstance(p, QDialog):
                ...
                return
            p = p.parent()
        # Case 2: we're embedded in a QStackedWidget (AdminDashboard stack index 8)
        # -> navigate back to the app-grid (index 0)
        p = self.parent()
        while p is not None:
            ...
            if isinstance(p, QStackedWidget):
                p.setCurrentIndex(0)
                return
            # Also check if the parent object has a .stack attribute
            if hasattr(p, "stack") and isinstance(getattr(p, "stack"), QStackedWidget):
    ...
    def _update_preview(self):
        ...
        text   = f"{prefix}{num:04d}" if prefix else f"{num:04d}"
    ...
    def _open_fiscalization_dialog(self):
        ...
        # Title
        title = QLabel("Fiscalization Configuration")
        ...
        self._fiscal_provider.setStyleSheet("""
            QComboBox {
                background: white;
        ...
        """)
        conn_layout.addLayout(_field_row("Provider:", self._fiscal_provider, lw=120))
        
        # Base URL
        self._fiscal_base_url = _inp(placeholder="https://erpfiscal.havano.online")
        conn_layout.addLayout(_field_row("Base URL:", self._fiscal_base_url, lw=120))
        
        # Frappe fields
        self._frappe_lbl_key = QLabel("API Key:")
        self._frappe_lbl_key.setFixedWidth(120)
        self._fiscal_api_key = _inp(pwd=True)
        self._frappe_row_key = QHBoxLayout()
        self._frappe_row_key.addWidget(self._frappe_lbl_key)
        self._frappe_row_key.addWidget(self._fiscal_api_key)
        conn_layout.addLayout(self._frappe_row_key)
        
        self._frappe_lbl_sec = QLabel("API Secret:")
        self._frappe_lbl_sec.setFixedWidth(120)
        self._fiscal_api_secret = _inp(pwd=True)
        self._frappe_row_sec = QHBoxLayout()
        self._frappe_row_sec.addWidget(self._frappe_lbl_sec)
        self._frappe_row_sec.addWidget(self._fiscal_api_secret)
        conn_layout.addLayout(self._frappe_row_sec)
        
        # Axis fields
        self._axis_lbl_email = QLabel("Axis Email:")
        self._axis_lbl_email.setFixedWidth(120)
        self._fiscal_axis_email = _inp()
        self._axis_row_email = QHBoxLayout()
        self._axis_row_email.addWidget(self._axis_lbl_email)
        self._axis_row_email.addWidget(self._fiscal_axis_email)
        conn_layout.addLayout(self._axis_row_email)
        
        self._axis_lbl_pass = QLabel("Axis Password:")
        self._axis_lbl_pass.setFixedWidth(120)
        self._fiscal_axis_password = _inp(pwd=True)
        self._axis_row_pass = QHBoxLayout()
        self._axis_row_pass.addWidget(self._axis_lbl_pass)
        self._axis_row_pass.addWidget(self._fiscal_axis_password)
        conn_layout.addLayout(self._axis_row_pass)
        
        # Device SN
        self._frappe_lbl_sn = QLabel("Device SN:")
        self._frappe_lbl_sn.setFixedWidth(120)
        self._fiscal_device_sn = _inp(placeholder="Device serial number")
        self._frappe_row_sn = QHBoxLayout()
        self._frappe_row_sn.addWidget(self._frappe_lbl_sn)
        self._frappe_row_sn.addWidget(self._fiscal_device_sn)
        conn_layout.addLayout(self._frappe_row_sn)
        ...
        def _on_provider_change():
            ...
            self._frappe_lbl_sec.setVisible(is_zimra)
            self._fiscal_api_secret.setVisible(is_zimra)
            
            self._frappe_lbl_sn.setVisible(is_zimra)
            self._fiscal_device_sn.setVisible(is_zimra)
            
            self._axis_lbl_email.setVisible(is_axis)
            self._fiscal_axis_email.setVisible(is_axis)
            
            self._axis_lbl_pass.setVisible(is_axis)
            ...
            if is_zimra:
                if not self._fiscal_api_key.text().strip():
                    self._fiscal_api_key.setText("51565a611a2cdb7")
                if not self._fiscal_api_secret.text().strip():
                    self._fiscal_api_secret.setText("425105201835d2c")
            
        self._fiscal_provider.currentIndexChanged.connect(_on_provider_change)
    ...
    def _load_fiscal_settings_into_dialog(self):
        ...
        try:
            ...
            if settings:
                ...
                provider_val = "havano_zimra" if settings.provider == "frappe" else settings.provider
                idx = self._fiscal_provider.findData(provider_val)
                if idx >= 0:
                    self._fiscal_provider.setCurrentIndex(idx)
                    
                base_url = (settings.base_url or "").strip()
                if not base_url or base_url == "https://erpfiscal.havano.online":
                    base_url = "https://erpfiscal.havano.online"
                self._fiscal_base_url.setText(base_url)
                ...
                self._fiscal_axis_email.setText(settings.axis_email)
                self._fiscal_axis_password.setText(settings.axis_password)
    ...
    def _test_fiscal_connection(self):
        ...
        axis_email = self._fiscal_axis_email.text().strip()
        ...
        if provider in ("frappe", "havano_zimra"):
            if not api_key:
        ...
        from views.dialogs.fiscal_settings_dialog import TestConnectionThread
    ...
    def _load(self):
        ...
        for line in bank_text.splitlines():
            sline = line.strip()
            if sline.startswith("Bank Name:"):
                self._bank_name.setText(sline.split(":", 1)[1].strip())
            elif sline.startswith("Account Name:"):
                self._bank_acc_name.setText(sline.split(":", 1)[1].strip())
            elif sline.startswith("Account Number:"):
                self._bank_acc_no.setText(sline.split(":", 1)[1].strip())
            elif sline.startswith("Branch / SWIFT:"):
                self._bank_branch.setText(sline.split(":", 1)[1].strip())
            else:
                extra_lines.append(line)

        if (self._bank_name.text().strip() or self._bank_acc_no.text().strip()):
            self._banking.setPlainText("\n".join(extra_lines).strip())

        for key, lbl in self._ro_labels.items():
            val = data.get(key, "")
            lbl.setText(val if val else "-")

        prefix_val = data.get("invoice_prefix", "").strip()
        if not prefix_val:
            import random, string
            prefix_val = ''.join(random.choices(string.ascii_uppercase, k=3))
        self._prefix_inp.setText(prefix_val)
        try:
        ...
        self._butchery_mode_chk.setChecked(
            str(data.get("butchery_mode", "0")).strip() == "1"
        )
    ...
    def _save(self):
        ...
        data.update({k: i.text().strip() for k, i in self._inputs.items()})
        ...
        b_acc_no = self._bank_acc_no.text().strip()
        b_branch = self._bank_branch.text().strip()
        extra_notes = self._banking.toPlainText().strip()

        banking_parts = []
        if b_name: banking_parts.append(f"Bank Name: {b_name}")
        if b_acc_name: banking_parts.append(f"Account Name: {b_acc_name}")
        if b_acc_no: banking_parts.append(f"Account Number: {b_acc_no}")
        if b_branch: banking_parts.append(f"Branch / SWIFT: {b_branch}")
        ...
        data["pharmacy_mode"]        = "1" if self._pharmacy_mode_chk.isChecked() else "0"

        data["butchery_mode"]        = "1" if self._butchery_mode_chk.isChecked() else "0"
        ...
        try:
            ...
            QTimer.singleShot(600, lambda: (
                shiboken6.isValid(self) and self._close_parent()
            ))
        except Exception as e:
