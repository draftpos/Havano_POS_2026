...
from PySide6.QtWidgets import (
    ...
    QDoubleSpinBox, QComboBox, QCompleter, QListWidget, QListWidgetItem
)
# pyrefly: ignore [missing-import]
...
def _show_order_note_dialog(parent_widget, table_id, order_id=None):
    """Open sleek single-line order note dialog for restaurant view."""
    # 1. Fetch current note
    from models.restaurant_order import get_order_by_table, get_order_by_id, update_bill_notes, get_predefined_notes
    existing = ""
    if order_id:
        order = get_order_by_id(order_id)
        if order: existing = order.get("bill_notes") or ""
    else:
        order = get_order_by_table(table_id)
        if order: 
            existing = order.get("bill_notes") or ""
            order_id = order["id"]

    if not order_id:
        QMessageBox.warning(parent_widget, "No Order", "No active order found for this table.")
        return

    predefined = get_predefined_notes()

    # 2. Build Dialog (matching MainWindow style)
    dlg = QDialog(parent_widget)
    dlg.setWindowTitle("Order Note")
    dlg.setFixedWidth(1100)
    dlg.setStyleSheet(f"QDialog {{ background: #ffffff; border: 1px solid {HDR_BG}; border-radius: 12px; }}")
    ...
    header = QLabel("GLOBAL ORDER NOTE")
    header.setStyleSheet(f"font-size: 11px; color: {TEXT_SEC}; font-weight: bold; letter-spacing: 1px;")
    v.addWidget(header)
    ...
    edit.setPlaceholderText("Type order note...")
    edit.setFixedHeight(44)
    edit.setStyleSheet(f"""
        QLineEdit {{
            border: 1px solid {BORDER}; border-radius: 8px;
            padding: 0 15px; font-size: 15px; color: {TEXT};
            background: {ROW_ALT};
        }}
    ...
    if predefined:
        ...
        completer.popup().setStyleSheet(f"""
            QListView {{
                ...
                border-radius: 6px; font-size: 14px; color: {TEXT};
                padding: 4px;
            }}
    ...
    ok_btn.setStyleSheet(f"QPushButton {{ background: {SUCCESS}; color: #ffffff; border: none; border-radius: 8px; font-size: 20px; font-weight: bold; }} QPushButton:hover {{ background: #15803d; }}")
    ...
    clr_btn.setStyleSheet(f"QPushButton {{ background: {DANGER}; color: #ffffff; border: none; border-radius: 8px; font-size: 18px; font-weight: bold; }} QPushButton:hover {{ background: #b91c1c; }}")
    ...
    osk_btn = QPushButton("⌨")
    osk_btn.setFixedSize(36, 36)
    osk_btn.setCursor(Qt.PointingHandCursor)
    osk_btn.setStyleSheet(f"background: {SURFACE}; border-radius: 4px; font-size: 16px;")
    ...
    if dlg.exec() == QDialog.Accepted:
        ...
        if update_bill_notes(order_id, new_note):
            if hasattr(parent_widget, "refresh"): parent_widget.refresh()
            elif hasattr(parent_widget, "parent_window") and hasattr(parent_widget.parent_window, "_set_status"):
                parent_widget.parent_window._set_status(f"Order note updated: {new_note[:30]}")
...
class TableCard(QFrame):
    ...
    def __init__(self, table_data: dict, parent=None):
        ...
        if occupied:
            ...
            if waiter_name:
                w_lbl = _tag(f"[User] {waiter_name}", ACCENT_SOFT, ACCENT, bold=True)
...
class WaiterSelectionDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowFlags(Qt.Popup | Qt.FramelessWindowHint)
        self.setStyleSheet(_GLOBAL_STYLE)
        self.setMinimumWidth(250)
        self.selected_waiter_id = None
        self.selected_waiter_name = None
        ...
        container.setObjectName("waiter_dialog_container")
        container.setStyleSheet(
            "#waiter_dialog_container { background-color: #ffffff; border: 1px solid #d1d5db; border-radius: 8px; }"
            "QLabel { color: #111827; font-size: 15px; font-weight: bold; padding-bottom: 5px; }"
        )
        ...
        lbl = QLabel("Assign Waiter", container)
        c_layout.addWidget(lbl)

        self.list_widget = QListWidget(container)
        self.list_widget.setStyleSheet(
            "QListWidget { background-color: transparent; border: none; outline: none; }"
            "QListWidget::item { color: #111827; padding: 12px 10px; border-bottom: 1px solid #f3f4f6; }"
    ...
    def _load_waiters(self):
        try:
            ...
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("SELECT id, username FROM users WHERE active=1 AND role != 'Pharmacist' ORDER BY username ASC")
            ...
            print(f"[WaiterSelectionDialog] Error loading waiters: {e}")

    def _on_item_clicked(self, item):
        self.selected_waiter_id = item.data(Qt.UserRole)
        self.selected_waiter_name = item.text()
        self.accept()
...
class TableActionDialog(QDialog):
    def __init__(self, table_data: dict, parent=None, settings: dict | None = None):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton#btnPay:hover {{ background: #dcfce7; color: #166534; }}
            QPushButton#btnPay {{ color: {SUCCESS}; font-weight: 700; }}
            
            QPushButton#btnAdd:hover {{ background: {ACCENT_SOFT}; color: {ACCENT}; }}
            QPushButton#btnAdd {{ color: {ACCENT}; font-weight: 700; }}
            
            QPushButton#btnDismiss:hover {{ background: #fee2e2; color: #991b1b; }}
        """)
        ...
        btn_add = QPushButton("Add New Order")
        ...
        self._pay_restricted = not _is_admin and not bool(self.user.get("allow_close_table", True))
        self._prebill_restricted = not _is_admin and not bool(self.user.get("allow_prebill", True))

        btn_pay = QPushButton("Close Table" + (" [ADMIN]" if self._pay_restricted else ""))
        ...
        btn_prebill = QPushButton("Print Pre-Bill" + (" [ADMIN]" if self._prebill_restricted else ""))
        btn_prebill.setCursor(Qt.PointingHandCursor)
        btn_prebill.clicked.connect(lambda: self._gated_action('prebill', self._prebill_restricted))
        ...
        btn_cancel.setObjectName("btnDismiss")
    ...
    def _on_order_note(self):
        """Open single-line order note dialog with predefined autocomplete."""
        _show_order_note_dialog(self, self.table_data["id"])
...
class KOTActionDialog(QDialog):
    def __init__(self, order_data, parent=None, settings=None, user=None):
        super().__init__(parent)
        self.order_data = order_data
        self.settings = settings or {}
        self.user = user or getattr(parent, "user", {}) or {}
        ...
        self._modify_restricted = not _is_admin and not bool(self.user.get("allow_edit_kot", True))
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton#btnNote:hover {{ background: {ACCENT_SOFT}; }}
            QLabel#Title {{
                font-size: 13px;
                font-weight: 800;
                color: {TEXT};
        ...
        display_num = order_data.get('shift_order_number') or order_data['id']
        title = QLabel(f"Order #ORD-{display_num}")
        title.setObjectName("Title")
        ...
        # # ORDER NOTE - global bill note
        # btn_note = QPushButton("ORDER NOTE")
        # btn_note.setObjectName("btnNote")
        # btn_note.setCursor(Qt.PointingHandCursor)
        # btn_note.clicked.connect(self._on_order_note)
    ...
    def _on_order_note(self):
        """Open single-line order note dialog with predefined autocomplete."""
        table_id = self.order_data.get("table_id")
        if table_id:
            _show_order_note_dialog(self, table_id, order_id=self.order_data.get("id"))
...
class _OrdersPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.user: dict = {}                          # set by OrderView.refresh()
        ...
        self._scroll.setWidget(self._orders_container)
    ...
    def _apply_filter(self):
        ...
        for o in self._all_orders:
            ...
            o["_eff"] = eff
            self._filtered_orders.append(o)
    ...
    def _load_next_batch(self, batch_size: int):
        if self._loaded_count >= len(self._filtered_orders):
            return
            
        batch = self._filtered_orders[self._loaded_count : self._loaded_count + batch_size]
        
        # Bulk fetch items to prevent N+1 query connection exhaustion crashes
        ...
        if order_ids:
            try:
                ...
                conn = get_connection()
                cur = conn.cursor()
                # SQL Server supports up to 2100 parameters in IN clause
                placeholders = ",".join(["?"] * len(order_ids))
                cur.execute(f"""
                    SELECT order_id, product_id, item_code, item_name, quantity as qty, rate as price
                ...
                for row in cur.fetchall():
                    ...
                    if oid not in items_by_order:
                        items_by_order[oid] = []
                    items_by_order[oid].append(d)
                conn.close()
            except Exception as e:
                print(f"[OrderPanel] Bulk item fetch error: {e}")

        for o in batch:
            is_open = o.get("_eff") == "Open"
            o["_items"] = items_by_order.get(o["id"], [])
            row = self._make_order_row(o, is_open, self._loaded_count)
    ...
    def _make_order_row(self, o: dict, is_open: bool, idx: int) -> QFrame:
        ...
        display_num = o.get('shift_order_number') or o['id']
        ord_id_lbl = QLabel(f"#ORD-{display_num}")
    ...
    def _on_row_clicked(self, event, order_data: dict):
        if event.button() == Qt.LeftButton:
            ...
            try:
                ...
                pass
            dlg = KOTActionDialog(order_data, self, settings=rs, user=self.user)
...
class OrderView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._current_waiter_id: int | None = None
    ...
    def clear_session_state(self):
        """Reset internal filters and refresh data."""
    ...
    def _handle_table_click(self, td: dict):
        def _position_dialog(dlg):
            ...
            # Clamp to screen to prevent overlapping the edge
            ...
            if screen:
                ...
                x = max(screen_geom.left(), min(dlg.x(), screen_geom.right() - dlg.width()))
                y = max(screen_geom.top(), min(dlg.y(), screen_geom.bottom() - dlg.height()))
                dlg.move(x, y)
        ...
        if td.get("status") == "Occupied":
            ...
            user = getattr(self.window(), "user", {}) or getattr(self.parent(), "user", {}) or {}
            if user.get("allow_assign_waiter", False):
                dlg = WaiterSelectionDialog(self)
                _position_dialog(dlg)
                if dlg.exec() == QDialog.Accepted:
                    td_assigned = dict(td)
                    td_assigned["assigned_waiter_id"] = dlg.selected_waiter_id
                    td_assigned["assigned_waiter_name"] = dlg.selected_waiter_name
                    self.table_selected.emit(td_assigned)
            else:
    ...
    def _print_prebill(self, table_data: dict):
        ...
        receipt.footer = "Havano Version 1.1.8"
        ...
        if self.parent():
            try:
                # Find MainWindow
                p = self.parent()
                while p and not hasattr(p, "_restaurant_auto_logout"):
                    p = p.parent()
                if p:
                    p._restaurant_auto_logout()
            except Exception:
                pass
    ...
    # ── Main Sales Order Print ────────────────────────────────────────────

    def print_main_sales_order(self, order_id: int) -> bool:
        """
        Print the full main sales order receipt (not just KOT) for the given
        restaurant order ID.  Call this from main_window right after
        save_order() succeeds so it fires automatically on order creation.
        ...
        try:
            ...
            # Attempt to fetch the order header for table/guest details
            order_data: dict = {}
            try:
                from models.restaurant_order import get_order_by_id
                order_data = get_order_by_id(order_id) or {}
            except Exception:
                pass

            table_lbl = (
                order_data.get("table_name")
                or f"Table {order_data.get('table_number', '')}"
            ).strip()

            receipt.receiptHeader  = "*** SALES ORDER ***"
            receipt.invoiceNo      = f"{table_lbl} / ORD-{order_id}"
            receipt.invoiceDate    = datetime.now().strftime("%Y-%m-%d")
            receipt.customerName   = order_data.get("customer_name") or "Guest"
            ...
            ok = ps.print_invoice_receipt(receipt, printer_name=printer_name)
            if not ok:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(
                    self, "Print Failed",
                    f"Could not print the main sales order for ORD-{order_id}.\n"
            ...
            import traceback
            print(f"[OrderView] print_main_sales_order error: {e}\n{traceback.format_exc()}")
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(
                self, "Print Error",
                f"Failed to print main sales order:\n{e}"
            )
            return False
    ...
    def _build_toolbar(self) -> QFrame:
        ...
        # Hide Monitors button if KDS is disabled in settings
        try:
            from database.db import get_connection
            conn = get_connection()
            cur = conn.cursor()
    ...
    def _show_kot_log(self):
        """Open the full-page KOT Activity dialog (Cancelled + Modified orders)."""
        from views.dialogs.kot_activity_dialog import KOTActivityDialog
        dlg = KOTActivityDialog(self)
        dlg.exec()
    ...
    def refresh(self):
        ...
        waiter_id        = getattr(self, "_current_waiter_id", None)
        ...
        tables = get_all_tables(
            waiter_id=waiter_id, 
            waiter_isolation=effective_isolation
        )
        ...
        self._panel.user               = getattr(self, "user", {}) or {}
