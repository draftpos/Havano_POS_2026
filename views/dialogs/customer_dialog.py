...
# #         self._btn_confirm = QPushButton("[OK]  Issue Credit Note")
...
# #             group_id       = _find_id("customer_groups", "Individual")
# #             conn.close()
...
from PySide6.QtWidgets import (
    QGridLayout,
    QDialog, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QGridLayout,
    QLabel, QLineEdit, QFrame, QTableWidget, QTableWidgetItem,
    QHeaderView, QAbstractItemView, QMessageBox, QComboBox,
    QSizePolicy, QDoubleSpinBox, QListWidget, QListWidgetItem,
)
...
class QuickAddCustomerDialog(QDialog):
    ...
    def _build(self):
        ...
        # ── buttons bar (moved to top) ──────────────────────────────────────
        top_btns = QWidget()
        top_btns.setStyleSheet(f"background: {OFF_WHITE}; border-bottom: 1px solid {BORDER};")
        bl = QHBoxLayout(top_btns)
        ...
        scroll_area.setStyleSheet(f"QScrollArea {{ border: none; background: {WHITE}; }}")
        ...
        self._f_trade_name = self._field("Trade Name")
        self._f_phone      = self._field("Phone number")
        self._f_email      = self._field("Email address")
        self._f_province   = self._field("Province")
        self._f_city       = self._field("City")
        self._f_street     = self._field("Street")
        self._f_address    = self._field("House Number")
        self._f_price_list = QComboBox()
        self._f_price_list.setFixedHeight(38)
        self._f_price_list.setStyleSheet(f"""
            QComboBox {{
        ...
        # Populate Price List Combo
        try:
            from models.price_list import get_all_price_lists
            from models.company_defaults import get_defaults
            defs = get_defaults()
            for pl in get_all_price_lists():
                self._f_price_list.addItem(pl.get("name"), pl.get("id"))
            idx = self._f_price_list.findData(defs.get("default_price_list_id"))
            if idx >= 0: self._f_price_list.setCurrentIndex(idx)
        except Exception as e:
            print(f"[QuickAddCustomer] Price list load error: {e}")

        self._f_tin        = self._field("TIN Number")
        self._f_vat        = self._field("VAT Number")

        fields = [
            ("FULL NAME",    self._f_name),
            ("TRADE NAME",   self._f_trade_name),
            ("PHONE NUMBER", self._f_phone),
            ("EMAIL ADDRESS",self._f_email),
            ("PROVINCE",     self._f_province),
            ("CITY",         self._f_city),
            ("STREET",       self._f_street),
            ("HOUSE NUMBER", self._f_address),
            ("PRICE LIST",   self._f_price_list),
            ("TIN NUMBER",   self._f_tin),
            ("VAT NUMBER",   self._f_vat),
        ]
        ...
        for i, (lbl_txt, widget) in enumerate(fields):
            ...
            lbl = QLabel(lbl_txt)
            lbl.setObjectName("section")
            fl.addWidget(lbl, row, col)
            fl.addWidget(widget, row + 1, col)
        ...
        fl.addWidget(self._doctor_lbl, 4, 2)
        fl.addLayout(_docrow, 5, 2)
        ...
        self._status.setAlignment(Qt.AlignCenter)
        fl.addWidget(self._status, 6, 0, 1, 3)
    ...
    def _on_add_doctor(self):
        ...
        try:
            ...
            print(f"[QuickAddCustomer] [OK] Doctor created locally id={new_id} "
    ...
    def _save(self):
        ...
        trade_name= self._f_trade_name.text().strip()
        phone     = self._f_phone.text().strip()
        email     = self._f_email.text().strip()
        province  = self._f_province.text().strip()
        city      = self._f_city.text().strip()
        street    = self._f_street.text().strip()
        address   = self._f_address.text().strip()
        tin       = self._f_tin.text().strip()
        vat       = self._f_vat.text().strip()
        ...
        if tin and not (tin.isdigit() and len(tin) == 10):
            self._status.setText("TIN Number must be exactly 10 digits.")
            self._f_tin.setFocus()
            return
            
        if vat and not (vat.isdigit() and len(vat) == 9):
            self._status.setText("VAT Number must be exactly 9 digits.")
            self._f_vat.setFocus()
            return
        ...
        try:
            ...
            def _find_id(table: str, name_val: str) -> int | None:
                if name_val and name_val.strip():
                    ...
                    row = cur.fetchone()
                    if row:
                        print(f"  [_find_id] {table} '{name_str}' -> id={row[0]}")
                        return row[0]
                    # Auto-insert using a dedicated connection so commit is guaranteed
                    try:
                        _ins_conn = get_connection()
                        _ins_cur  = _ins_conn.cursor()
                        if table in ("warehouses", "cost_centers"):
                            _ins_cur.execute("SELECT TOP 1 id FROM companies ORDER BY id ASC")
                            _c = _ins_cur.fetchone()
                            _c_id = _c[0] if _c else 1
                            _ins_cur.execute(f"INSERT INTO {table} (name, company_id) VALUES (?, ?)", (name_str, _c_id))
                        else:
                            _ins_cur.execute(f"INSERT INTO {table} (name) VALUES (?)", (name_str,))
                        _ins_conn.commit()
                        _ins_conn.close()
                        # Now re-query on main cursor to get the committed id
                        cur.execute(f"SELECT id FROM {table} WHERE LTRIM(RTRIM(name)) = ?", (name_str,))
                        row_new = cur.fetchone()
                        if row_new:
                            print(f"  [_find_id] {table} '{name_str}' AUTO-CREATED -> id={row_new[0]}")
                            return row_new[0]
                    except Exception as _e:
                        print(f"  [_find_id] auto-create error for {table} '{name_str}': {_e}")
                return None

            # Query the database for the most recent already-synced customer to copy defaults from
            cur.execute("""
                SELECT TOP 1 
                       custom_warehouse_id, 
                ...
                FROM customers
                WHERE frappe_synced = 1
                  AND custom_warehouse_id IS NOT NULL
                  AND custom_cost_center_id IS NOT NULL
                ORDER BY id DESC
            """)
            ref_row = cur.fetchone()
            if ref_row:
                warehouse_id   = ref_row[0]
                cost_center_id = ref_row[1]
                group_id       = ref_row[2] or _find_id("customer_groups", "Individual")
                price_list_id  = ref_row[3] or self._f_price_list.currentData()
                print(f"[QuickAddCustomer] Found reference synced customer defaults -> warehouse={warehouse_id}, cost_center={cost_center_id}")
            else:
                # Resolve warehouse name from defs or users table
                wh_name = defs.get("server_warehouse", "").strip()
                if not wh_name:
                    cur.execute("SELECT TOP 1 warehouse FROM users WHERE warehouse IS NOT NULL AND LTRIM(RTRIM(warehouse)) <> '' ORDER BY id ASC")
                    _uwh = cur.fetchone()
                    if _uwh and _uwh[0]:
                        wh_name = str(_uwh[0]).strip()

                # Resolve cost center name from defs or users table
                cc_name = defs.get("server_cost_center", "").strip()
                if not cc_name:
                    cur.execute("SELECT TOP 1 cost_center FROM users WHERE cost_center IS NOT NULL AND LTRIM(RTRIM(cost_center)) <> '' ORDER BY id ASC")
                    _ucc = cur.fetchone()
                    if _ucc and _ucc[0]:
                        cc_name = str(_ucc[0]).strip()

                warehouse_id   = _find_id("warehouses",      wh_name)
                cost_center_id = _find_id("cost_centers",    cc_name)
                price_list_id  = self._f_price_list.currentData()
                group_id       = _find_id("customer_groups", "Individual")
            conn.commit()
            conn.close()
            ...
            cur2.execute("""
                INSERT INTO customers (
                    customer_name, customer_group_id, customer_type,
                    custom_trade_name, custom_telephone_number, custom_email_address,
                    custom_city, custom_street, custom_province, custom_house_no,
                    custom_warehouse_id, custom_cost_center_id, default_price_list_id,
                    doctor_id, doctor_frappe_name, custom_customer_tin, custom_customer_vat
                ) OUTPUT INSERTED.id VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                full_name, group_id, "Individual",
                trade_name, phone, email,
                city, street, province, address,
                warehouse_id, cost_center_id, price_list_id,
                doctor_id, doctor_frappe_name, tin, vat
            ))
            new_id = int(cur2.fetchone()[0])
            conn2.commit()
            conn2.close()
            ...
            # Removed _push_to_frappe call, schedule_frappe_sync_check handles it entirely.
    ...
    # -------------------------------------------------------------------------
    # (Redundant _push_to_frappe method removed to prevent race conditions.
    #  models.customer.schedule_frappe_sync_check handles all pushing/retries)
