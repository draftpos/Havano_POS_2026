...
class LaybyePaymentDialog(QDialog):
    ...
    def __init__(
        ...
        try:
            ...
            _defs = _gd() or {}
            self._base_currency = (_defs.get("server_company_currency") or "USD").upper().strip()
        except Exception:
            pass
        ...
        self.deposit_currency = self._base_currency
        self.delivery_date = ""
    ...
    def _build_ui(self):
        ...
        zwg_rate = _get_local_rate(base_ccy, "ZWG")
        rate_text = f"1 {base_ccy} = {zwg_rate:,.2f} ZWG" if base_ccy != "ZWG" else f"Currency: {base_ccy}"
        ...
        if self.discount_amount > 0:
            disc_lbl = QLabel(f"Discount: {self.discount_percent:.1f}%  (−{base_ccy} {self.discount_amount:.2f})")
    ...
    def _build_left(self):
        ...
        tot_val = QLabel(f"{base_ccy} {self.total:.2f}")
        ...
        self._dep_lbl = QLabel(f"{base_ccy} 0.00")
    ...
    def _refresh_totals(self):
        ...
        # Update deposit card
        base_ccy = getattr(self, "_base_currency", "USD")
        self._dep_lbl.setText(f"{base_ccy}  {paid_usd:.2f}")
