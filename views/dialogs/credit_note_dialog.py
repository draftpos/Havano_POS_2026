...
class CreditNoteDialog(QDialog):
    ...
    def _build_search_area(self) -> QWidget:
        ...
        self._search.returnPressed.connect(self._handle_enter)

        self._btn_enter = QPushButton("Enter")
        self._btn_enter.setFixedHeight(38)
        self._btn_enter.setFixedWidth(80)
        self._btn_enter.setCursor(Qt.PointingHandCursor)
        self._btn_enter.setStyleSheet(f"""
            QPushButton {{
        ...
        """)
        self._btn_enter.clicked.connect(self._handle_enter)

        row.addWidget(lbl)
        row.addWidget(self._search, 1)
        row.addWidget(self._btn_enter)
        vl.addLayout(row)
        ...
        self._ac_list.itemActivated.connect(self._on_ac_clicked)
        
        # Arrow key navigation
        from PySide6.QtGui import QShortcut, QKeySequence
        self._shortcut_down = QShortcut(QKeySequence(Qt.Key_Down), self)
        self._shortcut_down.setContext(Qt.WidgetWithChildrenShortcut)
        self._shortcut_down.activated.connect(self._handle_down_arrow)
        
        self._shortcut_up = QShortcut(QKeySequence(Qt.Key_Up), self)
        self._shortcut_up.setContext(Qt.WidgetWithChildrenShortcut)
        self._shortcut_up.activated.connect(self._handle_up_arrow)
    ...
    def _build_btns(self) -> QWidget:
        ...
        self._bcancel = QPushButton("Cancel")
        self._bcancel.setFixedHeight(44); self._bcancel.setFixedWidth(100)
        self._bcancel.setCursor(Qt.PointingHandCursor)
        self._bcancel.setFocusPolicy(Qt.NoFocus)
        self._bcancel.setStyleSheet(f"""
            QPushButton {{ background:{LIGHT}; color:{DARK_TEXT};
                           border:1px solid {BORDER}; border-radius:6px;
                           font-size:13px; font-weight:bold; }}
            QPushButton:hover {{ background:{BORDER}; }}
        """)
        self._bcancel.clicked.connect(self._handle_bottom_action)
    ...
    def _on_ac_clicked(self, item):
        ...
        self._sale_stub_id = sale_stub["id"]
        
        self._bcancel.setText("Enter")
        self._bcancel.setStyleSheet(f"""
    ...
    def _handle_bottom_action(self):
        if self._bcancel.text() == "Enter":
            if hasattr(self, "_sale_stub_id"):
                self._load_sale(self._sale_stub_id)
                self._bcancel.setText("Cancel")
                self._bcancel.setStyleSheet(f"""
                    QPushButton {{ background:{LIGHT}; color:{DARK_TEXT};
                                   border:1px solid {BORDER}; border-radius:6px;
                                   font-size:13px; font-weight:bold; }}
    ...
    def _handle_enter(self):
        if self._bcancel.text() == "Enter" and hasattr(self, "_sale_stub_id"):
            self._handle_bottom_action()
            return
        self._ac_list.setFixedHeight(0)
        self._ac_list.clear()
        query = self._search.text().strip().lower()
        ...
        for s in self._all_sales:
            if (s.get("invoice_no") or "").lower() == query:
                self._load_sale(s["id"])
                return
        # If no exact match, just show search results
        self._run_search()



    def _handle_down_arrow(self):
        if self._ac_list.count() > 0 and self._ac_list.height() > 0:
            if not self._ac_list.hasFocus():
                self._ac_list.setFocus()
                self._ac_list.setCurrentRow(0)
            else:
                r = self._ac_list.currentRow()
                if r < self._ac_list.count() - 1:
                    self._ac_list.setCurrentRow(r + 1)

    def _handle_up_arrow(self):
        if self._ac_list.count() > 0 and self._ac_list.height() > 0:
            if not self._ac_list.hasFocus():
                self._ac_list.setFocus()
                self._ac_list.setCurrentRow(self._ac_list.count() - 1)
            else:
                r = self._ac_list.currentRow()
                if r > 0:
                    self._ac_list.setCurrentRow(r - 1)
    ...
    def _load_sale(self, sale_id: int):
        ...
        self._set_pill(self._b_date,   str(full.get("invoice_date", "-")))
