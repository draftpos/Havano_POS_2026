...
class CustomerSearchPopup(QDialog):
    ...
    def _build(self):
        ...
        self._status_lbl = QLabel("Double-click or Enter to select")
        ...
        self._tbl.setHorizontalHeaderLabels(["Name", "Type", "Phone", "Price List"])
        hh = self._tbl.horizontalHeader()
        hh.setSectionResizeMode(0, QHeaderView.Stretch)
        hh.setSectionResizeMode(1, QHeaderView.Fixed)
        hh.setSectionResizeMode(2, QHeaderView.Fixed)
        hh.setSectionResizeMode(3, QHeaderView.Fixed)
        self._tbl.setColumnWidth(1, 95)
        self._tbl.setColumnWidth(2, 110)
        self._tbl.setColumnWidth(3, 150)
    ...
    def _populate(self, custs):
        ...
        self._tbl.setRowCount(len(custs))
        for r, c in enumerate(custs):
            pl_name = c.get("price_list_name") or "—"
            for col, val in enumerate([
                c.get("customer_name") or "", c.get("customer_type") or "",
                c.get("custom_telephone_number") or "", pl_name,
            ]):
    ...
    def _on_sync_clicked(self):
        ...
        try:
            ...
            self._status_lbl.setText(f"Error: {str(e)}")
            self._sync_btn.setEnabled(True)
            self._sync_btn.setText("Sync Cloud")
...
class _InlineSettingsDialog(QDialog):
    ...
    def _build(self):
        ...
        # Close button removed - use the native title bar ✕
        root.addWidget(hdr)
...
class QuantityPopup(QDialog):
    ...
    def _build(self):
        ...
        for r, row_keys in enumerate(keys):
            for c, key in enumerate(row_keys):
                ...
                if key == "Del":
                    btn.clicked.connect(self._edit.backspace)
                else:
                    btn.clicked.connect(lambda _, k=key: self._append_to_edit(k))
    ...
    def _append_to_edit(self, key):
        if self._edit.hasSelectedText():
            self._edit.clear()
        if key == ".":
            ...
            if not current:
                ...
                return
        self._edit.setText(self._edit.text() + key)

    def _confirm(self):
        try:
            val = float(self._edit.text().replace(",", ".").strip() or "1")
            try:
                from database.db import get_connection
                conn = get_connection(); cur = conn.cursor()
                cur.execute("SELECT setting_value FROM pos_settings WHERE setting_key='qty_decimal_places'")
                r = cur.fetchone()
                conn.close()
                dp = int(r[0]) if (r and r[0] is not None) else 4
                if dp < 1:
                    dp = 4
            except Exception:
                dp = 4
            self.entered_qty = round(val, dp)
        except ValueError:
...
class CompanyDialog(QDialog):
    ...
    def _build(self):
        ...
        add_btn.clicked.connect(self._add); del_btn.clicked.connect(self._delete)
        br.addWidget(self._status, 1); br.addWidget(add_btn); br.addWidget(del_btn)
        lay.addLayout(br)
...
class CustomerGroupDialog(QDialog):
    ...
    def _build(self):
        ...
        br = QHBoxLayout(); br.setSpacing(8)
        self._status = QLabel(""); self._status.setStyleSheet(f"font-size:12px;color:{SUCCESS};background:transparent;")
        add_btn=navy_btn("Add",height=34,color=SUCCESS,hover=SUCCESS_H); del_btn=navy_btn("Delete",height=34,color=DANGER,hover=DANGER_H)
        add_btn.clicked.connect(self._add); del_btn.clicked.connect(self._delete)
        br.addWidget(self._status,1); br.addWidget(add_btn); br.addWidget(del_btn)
        lay.addLayout(br)
...
class HoldRecallDialog(QDialog):
    def __init__(self, parent=None):
        ...
        btn_row.addWidget(recall_btn); btn_row.addWidget(delete_btn); btn_row.addStretch()
        layout.addLayout(btn_row)
...
class POSView(QWidget):
    ...
    def _add_product_to_invoice(self, name, price, part_no="", product_id=None):
        ...
        for r in range(self.MAX_ROWS):
            ...
            if match:
                ...
                # ── Find the last filled row and insert after it ───────────────
                last_filled = -1
                for scan in range(self.MAX_ROWS):
                    try:
                        n = self.invoice_table.item(scan, 1).text().strip()
                        if n:
                            last_filled = scan
                    except AttributeError:
                        pass
    ...
    def _print_receipt(self):
        ...
        lay.addWidget(txt, 1)
        br = QHBoxLayout()
        # Close button removed - use the native title bar X
        lay.addLayout(br)
        dlg.exec()
