...
def _load_accounts_for_company(company: str, cashier_id: int = None) -> list[dict]:
    try:
        from models.gl_account import get_all_accounts
        accounts = get_all_accounts()
        company_accts = [a for a in accounts if a.get("company") == company]
        accts_to_use = company_accts if company_accts else accounts
        ...
        if cashier_id:
            ...
            if urow and urow.get("allowed_payment_methods"):
                ...
                if pm_str != "ALL":
                    ...
                    # Map to GL accounts
                    cur.execute("SELECT name, gl_account FROM modes_of_payment WHERE enabled=1")
                    mops = fetchall_dicts(cur)
                    allowed_gls = set()
                    for m in mops:
                        if m.get("name", "").lower() in allowed_methods and m.get("gl_account"):
                            allowed_gls.add(m["gl_account"])
            conn.close()
            
            if allowed_gls is not None:
                accts_to_use = [a for a in accts_to_use if a.get("name") in allowed_gls or a.get("account_name") in allowed_gls]
...
class SplitPaymentDialog(QDialog):
    ...
    def __init__(self, parent=None, total: float = 0.0,
                 company: str = "", company_currency: str = "", cashier_id: int = None):
        ...
        if not company_currency:
            try:
                from models.company_defaults import get_defaults
                d = get_defaults() or {}
                company_currency = (d.get("server_company_currency") or "USD").upper().strip()
            except Exception:
        ...
        self.accepted_method   = "SPLIT"
        self.accepted_currency = self.company_currency
    ...
    def _build(self):
        ...
        hl.addStretch()
        rate_hint = self._lbl(
            f"Rate: 1 {self.company_currency} = {self._zig_rate:.2f} ZIG" if self.company_currency != "ZIG" else f"Currency: {self.company_currency}", "#8fa8c8", 11
        )
        ...
        for attr, label, color, is_rem in [
            ...
            usd_lbl = QLabel(f"{self.company_currency}  0.00")
    ...
    def _load_accounts(self):
        self._accounts = _load_accounts_for_company(self.company, cashier_id=self.cashier_id)
    ...
    def _recalc(self):
        ...
        # ── summary cards ─────────────────────────────────────────────────────
        self._paid_usd_lbl.setText(f"{base}  {total_paid_usd:.2f}")
        self._paid_zig_lbl.setText(f"ZIG  {paid_zig:.2f}")
        self._rem_usd_lbl.setText(f"{base}  {remaining_usd:.2f}")
        ...
        for row, (edit, curr, rate) in self._editors.items():
            ...
            if item is None:
                continue
            # Show remaining in the row's own currency + base currency equivalent
            if curr == "ZIG":
                item.setText(
                    f"ZIG  {remaining_zig:.2f}   /   {base}  {remaining_usd:.2f}"
                )
            elif curr != self.company_currency and rate > 0:
                local_val = remaining_usd / rate
                item.setText(
                    f"{curr}  {local_val:.2f}   /   {base}  {remaining_usd:.2f}"
                )
            else:
                item.setText(
                    f"{base}  {remaining_usd:.2f}   /   ZIG  {remaining_zig:.2f}"
                )
