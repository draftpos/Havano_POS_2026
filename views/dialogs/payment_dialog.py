...
#                  items: list = None, cashier_id: int = None, cashier_name: str = "System",
#                  subtotal: float = 0.0, total_vat: float = 0.0, discount_amount: float = 0.0,
#                  discount_percent: float = 0.0, shift_id: int = None, is_restaurant_order: bool = False):
#         super().__init__(parent)
#         self.is_restaurant_order = is_restaurant_order
#         self.total             = total
#         self.items             = items or []
...
#         # [OK] Add processing flag to prevent duplicate saves
...
#         self._split_btn: QPushButton | None = None
        
#         self._allow_split_bill = False
#         try:
#             from models.restaurant_order import get_restaurant_settings
#             rs = get_restaurant_settings()
#             self._allow_split_bill = bool(rs.get("allow_split_bill"))
#         except Exception:
#             pass
...
#         if self._allow_split_bill:
#             self._split_btn = _action_btn("Split Payment", ORANGE, "#a04a00", height=52)
#             self._split_btn.setIcon(qta.icon("fa5s.columns", color="white"))
#             self._split_btn.clicked.connect(self._open_split)
#             brow.addWidget(self._split_btn)
...
#         # [OK] Prevent multiple simultaneous saves
...
#         # [OK] Generate transaction hash for duplicate detection
#         transaction_hash = self._generate_transaction_hash()
        
#         # [OK] Check for duplicate before proceeding
...
#             # [OK] Create the sale with idempotency_key
#             sale = create_sale(
#                 items=sale_items,
#                 total=self.total,
...
#                 idempotency_key=transaction_hash,  # [OK] Pass transaction hash
#                 splits=splits,                     # [OK] Per-method breakdown for receipt Payment Details
#             )
...
#             # GATED KOT PRINTING: If not in "Order Mode" (Restaurant Mode), print KOTs now.
#             # In Order Mode, KOTs are handled during the ordering phase, not at payment.
#             try:
#                 from models.restaurant_order import get_restaurant_settings
#                 rs = get_restaurant_settings()
#                 from services.printing_service import PrintingService
#                 ps = PrintingService()
#                 if not rs.get("enabled") or not self.is_restaurant_order:
#                     # Either restaurant mode is off, or this is a normal retail sale.
#                     # Normal sales need their KOTs printed immediately.
#                     print(f"[PaymentDialog] Printing KOT for {'Normal' if not self.is_restaurant_order else 'Table'} sale.")
#                     ps.print_kitchen_orders_for_sale(sale)
#                 else:
#                     print("[PaymentDialog] Restaurant Order Mode is ENABLED. Skipping automatic KOT print at payment.")
#             except Exception as e:
#                 print(f"[PaymentDialog] KOT print gate failed: {e}")
#             # [OK] Record the transaction hash in database
#             try:
...
#             # [OK] Create Payment Entries for Syncing
...
#             if item_count == 0:
#                 print(f"[PaymentDialog] [!] No items found! Inserting {len(sale_items)} items directly...")
...
def _get_local_rate(from_currency: str, to_currency: str = "USD") -> float:
    """
    Return the exchange rate from_currency -> to_currency.
    Automatically normalizes rate direction for non-base currencies (ZIG, ZAR, AMD, etc.)
    so that multi-currency payment calculations (3+ currencies) never waffle or invert.
    """
    from_u = (from_currency or "").upper().strip()
    to_u = (to_currency or "").upper().strip()
    ...
    try:
        from models.company_defaults import get_defaults
        d = get_defaults() or {}
        b = str(d.get("server_company_currency") or "USD").strip().upper()
        if b:
            base_u = b
    except Exception:
        pass
    ...
    try:
        ...
        # 1. FOREIGN -> BASE (e.g. ZIG -> USD, ZAR -> USD, AMD -> USD)
        # Should return BASE units per FOREIGN unit (< 1.0). If r_val > 1.0, invert!
        if from_u != base_u and to_u == base_u:
            if r_val > 1.0 and from_u not in ("GBP", "EUR", "AUD", "NZD"):
                return 1.0 / r_val
            return r_val

        # 2. BASE -> FOREIGN (e.g. USD -> ZIG, USD -> ZAR, USD -> AMD)
        # Should return FOREIGN units per BASE unit (> 1.0). If r_val < 1.0, invert!
        if from_u == base_u and to_u != base_u:
            if r_val < 1.0 and to_u not in ("GBP", "EUR", "AUD", "NZD"):
                return 1.0 / r_val
            return r_val
...
def _load_payment_methods(company: str, cashier_id: int = None) -> list[dict]:
    """
    Load payment methods from modes_of_payment table (synced from Frappe).
    Only includes MOPs that have a non-empty gl_account (leaf accounts only -
    group accounts like "Cash In Hand - DC1134" are excluded because Frappe
    ...
    try:
        ...
        cur.execute("""
            ...
            ORDER BY display_order, mop_name
        """)
        rows = fetchall_dicts(cur)
        ...
        try:
            from models.company_defaults import get_defaults
            d = get_defaults() or {}
            base_ccy = str(d.get("server_company_currency") or "").strip().upper()
        except Exception:
            pass
        ...
        for row in rows:
            mop_name   = (row.get("mop_name")   or "").strip()
            gl_account = (row.get("gl_account") or "").strip()
            row_curr   = (row.get("currency")   or "").strip()

            if not row_curr or (base_ccy != "USD" and row_curr.upper() == "USD"):
                curr = base_ccy
            else:
                curr = row_curr
            ...
            try:
                ...
                from database.db import fetchone_dict as _fd
                _conn = _gc(); _cur = _conn.cursor()
                _cur.execute(
                    "SELECT account_type FROM gl_accounts WHERE name = ?",
                    (gl_account,)
            ...
            # If this is from the gl_accounts fallback, and the GL account is already mapped 
            # by an existing Mode of Payment, skip it so we don't show duplicates!
            if row.get("display_order") == 9999 and gl_account.lower() in seen_gl:
                continue

            seen.add(key)
            if gl_account:
                seen_gl.add(gl_account.lower())
...
def _numpad_btn(text, kind="digit"):
    ...
    styles = {
        "digit": (WHITE,   LIGHT,    DARK_TEXT),
        "quick": (ACCENT,  ACCENT_H, WHITE),
        "del":   (ACCENT,  ACCENT_H, WHITE),
        "clear": (DANGER,  DANGER_H, WHITE),
    }
    bg, hov, fg = styles.get(kind, styles["digit"])
    btn.setStyleSheet(
        ...
        f" border-radius:6px; font-size:15px; font-weight:bold; }}"
        f"QPushButton:hover {{ background:{hov}; }}"
        f"QPushButton:pressed {{ background:{ACCENT_H}; color:{WHITE}; }}")
    return btn
...
class PaymentDialog(QDialog):
    ...
    def __init__(self, parent=None, total: float = 0.0, customer: dict | None = None,
                 ...
                 subtotal: float = 0.0, total_vat: float = 0.0, discount_amount: float = 0.0,
                 discount_percent: float = 0.0, shift_id: int = None, is_restaurant_order: bool = False,
                 prefill_amounts: dict | None = None, waiter_name: str = "",
                 table_id: int = None, warehouse_id: int = None, cost_center_id: int = None,
                 dining_option: str = ""):
        super().__init__(parent)
        self.is_restaurant_order = is_restaurant_order
        self._prefill_amounts    = prefill_amounts or {}
        self.total             = total
        self.items             = items or []
        self.cashier_id        = cashier_id
        self.cashier_name      = cashier_name
        ...
        # Auto-resolve waiter from the open restaurant order when:
        #   - this is a restaurant order payment, AND
        #   - the caller didn't already supply a waiter_name
        if is_restaurant_order and not (waiter_name or "").strip():
            try:
                from models.restaurant_order import get_order_by_table, get_waiter_name
                _order = None
                if table_id:
                    _order = get_order_by_table(table_id)
                if _order and _order.get("waiter_id"):
                    self.waiter_name = get_waiter_name(_order["waiter_id"]) or ""
                    print(f"[PaymentDialog] Auto-resolved waiter_name='{self.waiter_name}' "
                          f"from table_id={table_id} order_id={_order.get('id')}")
            except Exception as _we:
                print(f"[PaymentDialog] Waiter auto-resolve skipped: {_we}")

        print(f"[DEBUG] PaymentDialog.__init__: waiter_name='{self.waiter_name}'")
        ...
        co_name = self._company.get("name", "") if self._company else ""
        self._methods: list[dict] = _load_payment_methods(co_name, cashier_id=self.cashier_id)
        ...
        try:
            ...
            _defs = _gd() or {}
            self._credit_sales_allowed = str(_defs.get("allow_credit_sales", "0")).strip() == "1"
            self._base_currency = str(_defs.get("server_company_currency") or "").upper().strip()
        except Exception:
            pass

        self.accepted_currency = self._base_currency
        ...
        # ── Pre-fill collected shares into their MOP rows ──────────────────
        if self._prefill_amounts:
            for mop_label, native_amount in self._prefill_amounts.items():
                if mop_label in self._method_rows:
                    _, ae, _ = self._method_rows[mop_label]
                    ae.setText(f"{native_amount:.2f}")
            # Recompute totals after populating
            self._on_text_changed()
    ...
    def _build_ui(self):
        ...
        title.setStyleSheet(
            f"color:{NAVY}; font-size:17px; font-weight:bold; background:transparent;")
        base_ccy = (getattr(self, "_base_currency", "") or "USD").upper()
        
        # Collect all unique non-base currencies from configured payment methods
        sec_currencies = []
        for m in getattr(self, "_methods", []):
            c = (m.get("currency") or "").strip().upper()
            if c and c != base_ccy and c not in sec_currencies:
                sec_currencies.append(c)

        rate_parts = []
        for ccy in sec_currencies:
            r = _get_local_rate(base_ccy, ccy)
            if r and float(r) > 0:
                rate_val = float(r)
                if rate_val < 1.0:
                    inv_val = 1.0 / rate_val
                    rate_parts.append(f"1 {base_ccy} = {inv_val:,.2f} {ccy}")
                else:
                    rate_parts.append(f"1 {base_ccy} = {rate_val:,.2f} {ccy}")

        if rate_parts:
            rate_text = "   •   ".join(rate_parts)
        else:
            rate_text = f"Base Currency: {base_ccy}"

        rate_pill = QLabel(rate_text)
        rate_pill.setStyleSheet(
            f"color:{NAVY}; font-size:11px; font-weight:bold; background:{LIGHT};"
    ...
    def _build_left(self):
        ...
        cap_due.setStyleSheet(
            ...
            f" letter-spacing:1.1px; background:transparent;")
        cap_due.setAlignment(Qt.AlignCenter)
        base_ccy = getattr(self, "_base_currency", "USD")
        due_usd = QLabel(f"{base_ccy}  {self.total:.2f}")
        due_usd.setStyleSheet(
            f"color:{DARK_TEXT}; font-size:18px; font-weight:bold;"
            f" font-family:'Courier New',monospace; background:transparent;")
        due_usd.setAlignment(Qt.AlignCenter)
        ...
        cap_chg.setStyleSheet(
            ...
            f" letter-spacing:1.1px; background:transparent;")
        cap_chg.setAlignment(Qt.AlignCenter)
        self._chg_usd_lbl = QLabel(f"{base_ccy}  0.00")
        self._chg_usd_lbl.setStyleSheet(
            f"color:{DARK_TEXT}; font-size:18px; font-weight:bold;"
            f" font-family:'Courier New',monospace; background:transparent;")
        self._chg_usd_lbl.setAlignment(Qt.AlignCenter)
        ...
        def _make_row(label, curr, is_oa=False):
            ...
            ae.focusInEvent = lambda e, m=label, orig=ae.focusInEvent: (
                self._activate_method(m, focus_field=False), orig(e))
            ae.textChanged.connect(lambda _, m=label: self._on_text_changed(m))
            ae.returnPressed.connect(self._on_enter_pressed)
        ...
        if self._credit_sales_allowed:
            ...
            vbox.addWidget(_hr())
            vbox.addSpacing(4)
            vbox.addWidget(_make_row(self._OA_LABEL, base_ccy, is_oa=True))
    ...
    def _build_right(self):
        ...
        self._print_btn = _action_btn("Print  (F5)", ACCENT, ACCENT_H, height=52)
        self._print_btn.setIcon(qta.icon("fa5s.print", color="white"))
        self._print_btn.clicked.connect(self._save)
        self._print_btn.setEnabled(False)
        brow.addWidget(self._print_btn)
    ...
    def _active_field(self) -> QLineEdit:
        if self._active_method in self._method_rows:
            return self._method_rows[self._active_method][1]
        # Return first available or None if empty
        try:
            return next(iter(self._method_rows.values()))[1]
        except StopIteration:
            return None
    ...
    def _method_info(self, label: str) -> tuple[str, float, str]:
        ...
        for m in self._methods:
            if m["label"] == label:
                curr = m["currency"]
                gl_acct = m.get("gl_account", "")
                if curr.upper() == base_ccy.upper():
                    return curr, 1.0, gl_acct
                r = _get_local_rate(curr, base_ccy)
                return curr, (r if r > 0 else 1.0), gl_acct
        return base_ccy, 1.0, ""
    ...
    def _on_enter_pressed(self):
        if self._print_btn and self._print_btn.isEnabled():
            self._save()

    def _get_paid_native(self, label: str) -> float:
        """Returns the amount exactly as entered by the user, capped at 99,999,999.99 max."""
        if label not in self._method_rows:
            return 0.0
        _, ae, _ = self._method_rows[label]
        try:
    ...
    def _on_text_changed(self, _label: str = ""):
        ...
        base_ccy = getattr(self, "_base_currency", "USD")
        chg_text = f"{base_ccy}  {chg_usd:.2f}"
        if len(active_methods) == 1:
            sole = active_methods[0]
            curr, usd_per_unit, _ = self._method_info(sole)
            if curr.upper() != base_ccy and usd_per_unit > 0:
                chg_native = max(
                    self._get_paid_native(sole) - (self.total / usd_per_unit),
                    0.0,
                )
        ...
        for label in self._method_rows:
            ...
            fg = SUCCESS if settled else DARK_TEXT
            curr, usd_per_unit, _ = self._method_info(label)
            if curr.upper() == base_ccy:
                text = f"{base_ccy}  {rem_usd:.2f}"
            else:
                rate_usd_to_native = _get_local_rate(base_ccy, curr)
                native = rem_usd * rate_usd_to_native
                text = f"{curr}  {native:,.2f}"
            due_lbl.setText(text)
            due_lbl.setTextFormat(Qt.PlainText)
    ...
    def _save(self):
        ...
        # We still generate the hash for the idempotency_key to prevent API double-sync,
        # but we no longer block the user locally with the "Duplicate Transaction" popup 
        # because the _processing_save flag already prevents double-clicks.
        ...
        try:
            ...
            for idx, item in enumerate(self.items):
                ...
                sale_item = {
                    ...
                    "order_1":      item.get("order_1"),
                    "order_2":      item.get("order_2"),
                    "order_3":      item.get("order_3"),
                    "order_4":      item.get("order_4"),
                    "order_5":      item.get("order_5"),
                    "order_6":      item.get("order_6"),
                }
                sale_items.append(sale_item)
                print(f"   Item {idx+1}: {sale_item['product_name']} qty={sale_item['qty']} "
                      f"price={raw_price}->{sale_item['price']} total={raw_total}->{sale_item['total']} "
            ...
            # [OK] Create the sale with idempotency_key
            from models.company_defaults import get_defaults
            _cd_info = get_defaults() or {}
            cur_store = getattr(self, "store", None) or _cd_info.get("server_warehouse") or _cd_info.get("warehouse") or _cd_info.get("company_name") or ""
            cur_profile = getattr(self, "pos_profile", None) or _cd_info.get("server_profile") or _cd_info.get("server_terminal_name") or ""
            cur_terminal = getattr(self, "terminal_id", None) or _cd_info.get("server_terminal_id") or ""
            ...
            sale = create_sale(
                ...
                customer_name=self._customer.get("customer_name", "") if self._customer else "",
                customer_contact=self._customer.get("mobile", "") if self._customer else "",
                company_name=cur_store or (self._company.get("name", "") if self._company else ""),
                store=cur_store,
                pos_profile=cur_profile,
                terminal_id=cur_terminal,
                payment_method=accepted_meth,
                kot="",
                currency="USD" if accepted_meth == "SPLIT" else curr,
                subtotal=self.subtotal,
                total_vat=self.total_vat,
            ...
            print(f"[DEBUG] PaymentDialog._save: Called create_sale with waiter_name='{self.waiter_name}'")
            ...
            try:
                ...
                if mode == "odoo":
                    print("[PaymentDialog] Odoo mode detected - triggering immediate push...")
                    from services.odoo.sale_upload_service import push_unsynced_sales_odoo
                    import threading
                    threading.Thread(target=push_unsynced_sales_odoo, daemon=True).start()
                elif mode in ("saas", "frappe"):
                    print(f"[PaymentDialog] {mode.upper()} mode detected - triggering immediate sales invoice push...")
                    from services.pos_upload_service import push_unsynced_sales
                    import threading
                    threading.Thread(target=push_unsynced_sales, daemon=True).start()
            except Exception as e:
                print(f"[PaymentDialog] Could not trigger immediate sales push: {e}")
            ...
            if self._print_btn:
                self._print_btn.setEnabled(True)
                self._print_btn.setText("Print  (F5)")
                self._print_btn.setIcon(qta.icon("fa5s.print", color="white"))
    ...
    def _open_split(self):
        ...
        try:
            from views.dialogs.split_payment_dialog import SplitPaymentDialog
            dlg = SplitPaymentDialog(self, total=self.total, company=co,
                                     company_currency=co_curr, cashier_id=self.cashier_id)
            if dlg.exec() == QDialog.Accepted:
                self.accepted_method       = "SPLIT"
                self.accepted_tendered     = sum(s["base_value"] for s in dlg.splits)
                self.accepted_change       = dlg.accepted_change
    ...
    def keyPressEvent(self, event):
        k = event.key()
        if k == Qt.Key_F5:
            if not self._processing_save and self._get_tendered() >= self.total - 0.005 and self._get_tendered() > 0:
                self._save()
            return
        if k == Qt.Key_F3:
            return
        if k in (Qt.Key_Return, Qt.Key_Enter):
            if not self._processing_save and self._print_btn and self._print_btn.isEnabled():
                self._save()
            return
        if k == Qt.Key_Escape:
            self.reject()
