...
def _friendly_error(e):
    ...
    if "REFERENCE constraint" in msg or "FK_" in msg or "foreign key" in msg.lower():
        return "Cannot delete â€” record is still linked to other data. Remove those links first."
...
# â”€â”€ Hardware Config Logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Hardware Config Logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
_HW_FILE = Path("app_data/hardware_settings.json")   # â† now matches sql_settings.json
_ORDER_STATIONS = [f"Order {i}" for i in range(1, 7)]
def _load_hw() -> dict:
    try:
...
# =============================================================================
# BASE â€” all section dialogs inherit from this
# =============================================================================
class _Base(QDialog):
    TITLE  = "Settings"
    W, H   = 740, 540
...
# =============================================================================
# Section Dialogs (Companies, Customer Groups, etc.) â€” unchanged
# =============================================================================
...
# â”€â”€ Users dialog â€” separate file, no circular dependency â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
try:
...
# â”€â”€ Helpers â€” defined locally to avoid importing from main_window â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _friendly_db_error(e):
    msg = str(e)
    if "REFERENCE constraint" in msg or "FK_" in msg or "foreign key" in msg.lower():
        return "Cannot delete â€” record is still linked to other data."
    if "UNIQUE" in msg or "duplicate key" in msg.lower():
        return "A record with that name already exists."
    if "Cannot insert the value NULL" in msg:
        return "A required field is missing."
...
class CustomerGroupDialog(QDialog):
    ...
    def _reload(self):
        ...
        for g in groups:
            ...
            parent_name = next((x["name"] for x in groups if x["id"]==g.get("parent_group_id")),"â€”")
...
class CustomerDialog(QDialog):
    ...
    def _build(self):
        ...
        self._search=QLineEdit(); self._search.setPlaceholderText("Search by name, trade name or phoneâ€¦"); self._search.setFixedHeight(34)
...
# =============================================================================
# HardwareDialog â€” Hardware Settings (Enhanced)
# =============================================================================
# =============================================================================
# HardwareDialog â€” Hardware Settings (Fixed & Working)
# =============================================================================
class HardwareDialog(_Base):
    ...
    def _build(self, lay):
        ...
        self._kitchen_enabled_chk.setToolTip(
            "When on, every sale fans out grouped items to the Order 1â€“6 printers below.\n"
...
class SettingsDialog(QDialog):
    ...
    def _build(self):
        ...
        # â”€â”€ MASTER DATA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ HARDWARE & PRINTING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # Pharmacy Mode toggle moved to Company Defaults â†’ Payment Settings card.
...
# =============================================================================
# ToggleSwitch â€” Proper iOS-style animated toggle widget
# =============================================================================
...
class ToggleSwitch(QCheckBox):
    ...
    Draws its own track + thumb via paintEvent â€” no stylesheet tricks.
    """
    ...
    def paintEvent(self, _event):
        ...
        # Interpolate track color OFF â†’ ON
        t = self._anim_pos
...
# =============================================================================
# POS BUSINESS RULES â€” NAVY BLUE ERP (FIXED CLICK & SLIDE)
# =============================================================================
from PySide6.QtWidgets import (
...
# â”€â”€ Sliding Toggle (Fixed Click Logic) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class SlidingToggle(QCheckBox):
    ...
    def paintEvent(self, e):
        ...
        # Track Background (Gray to Navy Blue: #1a5fb4)
...
class POSRulesDialog(QDialog):
    ...
    def _build(self):
        ...
        # â”€â”€ Header Action Bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        header = QWidget()
        ...
        title.setStyleSheet("font-size:11px; font-weight:bold; color:#1a5fb4; letter-spacing:1.5px;")
        ...
        self.save_btn.setStyleSheet("""
            QPushButton { 
                background:#1a5fb4; color:white; border-radius:4px; 
        ...
        self.exit_btn.setStyleSheet("""
            QPushButton { 
                background:transparent; color:#1a5fb4; border:1px solid #1a5fb4; 
                border-radius:4px; font-weight:bold; font-size:10px;
            }
            QPushButton:hover { background:#eef2f7; color:#1a5fb4; }
        """)
        ...
        # â”€â”€ List Body â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        body = QWidget()
        bl = QVBoxLayout(body)
        ...
        self.status_lbl = QLabel("â— SYSTEM CONFIGURATION")
        self.status_lbl.setStyleSheet("color:#1a5fb4; font-size:9px; font-weight:bold; opacity:0.6;")
    ...
    def _rule_row(self, key, label, desc):
        ...
        title = QLabel(label)
        title.setStyleSheet("color:#1a5fb4; font-size:11px; font-weight:bold;")
        sub = QLabel(desc)
        sub.setStyleSheet("color:#1a5fb4; font-size:10px; opacity:0.6;")
