...
class UnfiscalizedDialog(QDialog):
    ...
    def _load_history(self):
        ...
        for sale in rows:
            ...
            curr = (defs.get("server_company_currency") or defs.get("currency") or "").strip()
            sym = {"USD": "$", "ZAR": "R", "ZWG": "ZiG ", "ZIG": "ZiG ", "EUR": "€", "GBP": "£"}.get(curr.upper(), curr) if curr else "$"
            total_item = QTableWidgetItem(f"{sym}{total_val:,.2f}")
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        self._hist_table.setHorizontalHeaderLabels([
            "Date", "Invoice No", "Customer", f"Total ({sym})", "Fiscal Code", "Verification Code", ""
        ])
    ...
    def _v_cell(self, val, bold=False):
        ...
        curr = (defs.get("server_company_currency") or defs.get("currency") or "").strip()
        sym = {"USD": "$", "ZAR": "R", "ZWG": "ZiG ", "ZIG": "ZiG ", "EUR": "€", "GBP": "£"}.get(curr.upper(), curr) if curr else "$"
        it = QTableWidgetItem(f"{sym}{val:,.2f}")
