...
#         self._status_lbl.setText("Connecting to apk.havano.cloud...")
...
#         havano_lnk = _QL('<a href="https://apk.havano.cloud" style="color:#0d1f3c;font-size:11px;text-decoration:none;font-weight:bold;">apk.havano.cloud</a>')
...
#                     # If the inline search popup is open, let its own handler take it
...
#         self._product_grid.setContentsMargins(1, 1, 1, 1)
...
#         # Clamp name so the price line is never pushed off the button.
#         # Two lines: first line up to 18 chars (elided with …), second line = price.
#         MAX_NAME = 18
#         display_name = pname if len(pname) <= MAX_NAME else pname[:MAX_NAME - 1] + "…"
#         label = f"{display_name}\n${price:.2f}" if price else display_name
...
#                 currency = getattr(dlg, "accepted_currency", "USD")
#                 sale = create_sale(
#                     items=items,
#                     total=total,
#                     tendered=tendered,
...
#         """Update the Unsynced badge count from the local DB."""
#         try:
...
class CreditNoteDialog(QDialog):
    """Search-only dialog. Enter/click -> items load straight into POSView."""
    ...
    def _run_search(self):
        ...
        if not matches:
            self._ac.setFixedHeight(0); self.setFixedSize(600, 170); return
        exact = [s for s in matches if (s.get("invoice_no") or "").lower() == ql]
        if exact:
            self._load_and_close(exact[0]); return
        if len(matches) == 1:
            self._load_and_close(matches[0]); return
        for s in matches:
            label = (
                f"{s.get('invoice_no', '?')}"
    ...
    def _load_and_close(self, stub: dict):
        ...
        inv   = full.get('invoice_no', '')
        cust  = full.get('customer_name') or 'Walk-in'
        total = full.get('total', 0)
        nitems = len(full.get('items', []))
        reply = QMessageBox.question(
            self, 'Load Invoice for Return?',
            f'Invoice:  {inv}\n'
            f'Customer: {cust}\n'
            f'Items:    {nitems}\n'
            f'Total:    ${float(total):.2f}\n\n'
            'Load this invoice into the main table for return?',
            QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply != QMessageBox.Yes:
            return
