class AdminDashboard(QWidget):
    def __init__(self, parent_window=None, user=None):
        ...
        self._build()
        # NOTE: Do NOT call _load_data() here.
        # switch_to_dashboard() in main_window.py schedules _load_data() after
        # switching the stack widget. Calling it here too causes two loaders
        # to appear simultaneously on first Dashboard open.
    ...
    def _build_kpi_row_1(self):
        ...
        for key, label, icon, color in [
            ...
            ("avg_profit", "Avg Profit / Inv", "fa5s.divide", SUCCESS),
        ]:
    ...
    def _render_stock(self, products: list):
        ...
        # 2. Render only up to 200 products to prevent UI freeze
        render_limit = 200
        for p in products[:render_limit]:
            qty = float(p.get("stock", 0) or 0)
            cost = float(p.get("cost_price", 0) or 0)
            sell = float(p.get("price", 0) or 0)
            val_cost = qty * cost
        ...
        # 3. Add a footer row if there are more items
        if len(products) > render_limit:
            r = self._stock_tbl.rowCount()
            self._stock_tbl.insertRow(r)
            it = QTableWidgetItem(f"... and {len(products) - render_limit} more products (use search to filter)")
            it.setTextAlignment(Qt.AlignCenter)
            it.setForeground(QColor(MUTED))
            font = QFont(); font.setItalic(True)
            it.setFont(font)
            self._stock_tbl.setItem(r, 0, it)
            self._stock_tbl.setSpan(r, 0, 1, 8)
            self._stock_tbl.setRowHeight(r, 32)
        ...
        self._stock_count_lbl.setText(f"{len(products)} product{'s' if len(products) != 1 else ''} found")
    ...
    def _load_data(self):
        try:
            ...
            self._dashboard_loader = SleekLoaderOverlay(parent=self)
            self._dashboard_loader.set_status("Loading Dashboard...")
            self._dashboard_loader.show_loading()
        except Exception:
            self._dashboard_loader = None
            
        from PySide6.QtCore import QTimer
        QTimer.singleShot(50, self._step_1_stock)

    def _step_1_stock(self):
        try:
            self._load_stock_data()
        except Exception as e:
            print("Error loading stock data:", e)
            
        if hasattr(self, '_dashboard_loader') and self._dashboard_loader:
            self._dashboard_loader.set_status("Loading Top Items...")
            
        from PySide6.QtCore import QTimer
        QTimer.singleShot(10, self._step_2_top_items)

    def _step_2_top_items(self):
        try:
        ...
        if hasattr(self, '_dashboard_loader') and self._dashboard_loader:
            self._dashboard_loader.set_status("Loading Recent Sales...")
            
        from PySide6.QtCore import QTimer
        QTimer.singleShot(10, self._step_3_sales)

    def _step_3_sales(self):
        try:
        ...
        if hasattr(self, '_dashboard_loader') and self._dashboard_loader:
            self._dashboard_loader.set_status("Finalizing...")
            
        from PySide6.QtCore import QTimer
        QTimer.singleShot(10, self._step_4_alerts)

    def _step_4_alerts(self):
        try:
        ...
        if hasattr(self, '_dashboard_loader') and self._dashboard_loader:
            try:
                self._dashboard_loader.hide_loading()
            except Exception:
                pass
    ...
    def _load_financial_kpis(self):
        ...
        try:
            ...
            try:
                ...
                r = cur.fetchone()
                daily_sales = float(r[0] or 0)
                daily_inv = int(r[1] or 0)
                ...
                daily_prof = daily_sales - daily_cost
                avg_profit = (daily_prof / daily_inv) if daily_inv > 0 else 0.0
            except Exception:
                avg_profit = 0.0
    ...
    def _load_stock_alerts(self):
        ...
        try:
            if hasattr(self, "_all_stock"):
                low = [p for p in self._all_stock if float(p.get("stock", 0) or 0) <= 5]
            else:
                from models.product import get_all_products
...
class POSView(QWidget):
    ...
    def _do_nav_sync_products(self):
        ...
        # Using hardcoded hex instead of constants since new_d might not import WHITE/ACCENT directly the same way
    ...
    def _add_product_to_invoice(self, name, price, part_no="", product_id=None, stock=None, uom=""):
        ...
        if self._is_pharmacy_product_lookup(product_id, part_no):
            _allow_cashier = False
            try:
                from database.db import get_connection as _gc
                _c = _gc(); _cur = _c.cursor()
                _cur.execute("SELECT allow_cashier_pharmacy_sales FROM company_defaults")
                _row = _cur.fetchone()
                _c.close()
...
# ── Options dropdown ──────────────────────────────────────────────────────
    ...
    def _build_invoice_table(self):
        ...
        dynamic_labels = ["Item No.", "Item Details", f"Amount ({sym})", "Qty", "UOM", "Disc", "TAX", f"Total ({sym})"]
