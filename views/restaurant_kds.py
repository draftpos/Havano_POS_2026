...
Unified/Order Monitor mode: compact cards only - no item breakdown.
"""
...
class OrderCard(QFrame):
    ...
    def _build(self):
        ...
        # ── Public / unified mode - compact summary, NO item list ─────────────
        if self.mode == "public":
            sub = QHBoxLayout()
            sub.setSpacing(8)
            ...
            """)
            lay.addWidget(pill)
            # Compact card ends here - no items, no footer buttons
            return

        # ── Kitchen / dispatch sub-header ─────────────────────────────────────
        sub = QHBoxLayout()
        sub.setSpacing(8)
        w  = (self.data.get("waiter_name") or "System").title()
        wl = QLabel(f"▸ {w}")
...
class KDSWindow(QWidget):
    ...
    def _build_unified(self):
        ...
        # Outer container: two columns + centre divider
        cw = QWidget()
        cw.setStyleSheet("background: transparent;")
        cl = QHBoxLayout(cw)
        cl.setContentsMargins(0, 0, 0, 0)
        cl.setSpacing(0)  # spacing is provided by the divider widget's margins
        ...
        divider.setFixedWidth(25)   # total width including side margins
    ...
    def _pop_unified(self, orders, new_ids):
        ...
        if prep:
            ...
            for o in prep:
                # mode="public" -> compact card, no items breakdown
