...
class ManageUsersDialog(QDialog):
    ...
    def _build(self):
        ...
        w_rl, self._f_role   = _combo_field("Role *",  ["cashier", "admin", "Pharmacist"])
        ...
        # Restaurant
        rw7, self._perm_close_table = _perm_row("Allow Close Table", "Can finalize/close restaurant tables")
        rw8, self._perm_prebill     = _perm_row("Allow Pre-bill",    "Can print proforma receipts")
        rw9, self._perm_pay_kot     = _perm_row("Allow Pay KOT",     "Can process KOT payments")
        rw10, self._perm_edit_kot    = _perm_row("Allow Edit KOT",    "Can modify active restaurant orders")
        rw11, self._perm_cancel_kot  = _perm_row("Allow Cancel Order","Can cancel restaurant orders")
        rw12, self._perm_auto_logout = _perm_row("Auto Logout",       "Log out staff after every order")

        for rw in [rw1, rw2, rw3, rw4, rw5, rw6, rw7, rw8, rw9, rw10, rw11, rw12]:
            c4l.addWidget(rw)
    ...
    def _on_row_click(self, row, _col):
        ...
        if role == "admin":
            self._f_role.setCurrentIndex(1)
        elif role == "pharmacist":
            self._f_role.setCurrentIndex(2)
        else:
        ...
        # Restaurant
        self._perm_close_table.setChecked(bool(u.get("allow_close_table", False)))
        self._perm_prebill.setChecked(    bool(u.get("allow_prebill",     False)))
        self._perm_pay_kot.setChecked(    bool(u.get("allow_pay_kot",     False)))
        self._perm_edit_kot.setChecked(   bool(u.get("allow_edit_kot",    False)))
        self._perm_cancel_kot.setChecked( bool(u.get("allow_cancel_kot",  False)))
        self._perm_auto_logout.setChecked(bool(u.get("auto_logout",       False)))
        # Discount limits
    ...
    def _save_user(self):
        ...
        # Restaurant
        perm_close_table = int(self._perm_close_table.isChecked())
        perm_prebill     = int(self._perm_prebill.isChecked())
        perm_pay_kot     = int(self._perm_pay_kot.isChecked())
        perm_edit_kot    = int(self._perm_edit_kot.isChecked())
        perm_cancel_kot  = int(self._perm_cancel_kot.isChecked())
        perm_auto_logout = int(self._perm_auto_logout.isChecked())
            ...
            if self._editing_id is None:
                ...
                try:
                    ...
                    for col in ["allow_discount", "allow_receipt",
                                ...
                                "allow_cancel_kot", "allow_pay_kot", "allow_close_table",
                                "allow_prebill", "allow_edit_kot", "auto_logout"]:
                        try:
                            cur.execute(f"""
                                IF NOT EXISTS (
                    ...
                    """, (username, role,
                          ...
                          perm_cancel_kot, perm_pay_kot,
                          perm_close_table, perm_prebill,
                          perm_edit_kot, perm_auto_logout,
...
class OptionsDialog(QDialog):
    ...
    def _do_sync_products(self):
        ...
        self._sync_status_lbl.setText("Full Sync from Server...")
...
class POSRulesDialog(QDialog):
    ...
    def _add_rule_row(self, layout, key, label, desc, default):
        ...
        try:
            ...
            chk.setChecked((str(row[0]).strip().lower() in ("1", "true", "yes")) if row else default)
        except Exception:
...
class UnsyncedPopup(QDialog):
    ...
    def _load_all(self):
        ...
        for kind in self._TAB_META:
            if is_saas and kind == "PAY":
                continue
            if kind not in self._tables: continue
        ...
        for kind in self._TAB_META:
            ...
            self._tab_widget.setTabText(tab_idx, label + suffix)
            tab_idx += 1

    def _load_tab(self, kind: str) -> int:
        """Load rows for one tab. Returns row count."""
        if kind not in self._tables: return 0
            ...
            try:
                ...
                if kind == "SI":
                    ...
                    try:
                        ...
                        print(f"[retry] CUST sync error: {e}")
            except Exception as e:
...
class _BadgeWorker(QThread):
    ...
    def run(self):
        ...
        # Always emit - even if all counts stayed 0 - so the badge is refreshed
...
class POSView(QWidget):
    ...
    def _do_nav_sync_products(self):
        ...
        self._sync_dlg.setStyleSheet("QDialog { background: #FFFFFF; border: 2px solid #3498db; border-radius: 8px; } QLabel { font-weight: bold; color: #2c3e50; font-size: 13px; }")
        ...
        close_btn.setIcon(qta.icon("fa5s.times", color="#7f8c8d"))
    ...
    def _open_payment(self):
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            # ── Clear invoice for next customer (after fiscal wait if needed) ──
            _sale_id_for_fiscal = (
                sale_id_result
                or (sale.get("id") if sale else None)
            )

            def _after_fiscal_payment(fiscalized: bool):
                if not fiscalized:
                    print(f"[POSView] Fiscal wait timed out for sale {_sale_id_for_fiscal} - proceeding anyway")
                self._new_sale(confirm=False)

            # Gate on the sale's own fiscal_status rather than importing the
            # fiscal service - avoids silent failures if the service isn't ready.
            _needs_fiscal_wait = False
            if _sale_id_for_fiscal:
                try:
                    from models.sale import get_sale_by_id as _gsbi
                    _s = _gsbi(_sale_id_for_fiscal)
                    _fs_status = (_s.get("fiscal_status", "") if _s else "")
                    _needs_fiscal_wait = _fs_status in ("pending", "failed", "")
                    print(f"[POSView] fiscal_status={_fs_status!r} needs_wait={_needs_fiscal_wait}")
                except Exception as _fe:
                    print(f"[POSView] Could not read fiscal_status: {_fe}")

            if _needs_fiscal_wait:
                self._start_fiscal_wait(_sale_id_for_fiscal, _after_fiscal_payment)
            else:
                self._new_sale(confirm=False)
        ...
        for ln, it in enumerate(items, start=1):
            ...
            if is_pharm and not dosage:
                ...
                QMessageBox.warning(
                    ...
                    f"Please select the row and press F1 to input the dosage."
                )
                return
    ...
    def _refresh_unsynced_badge(self):
        ...
        if getattr(self, "_badge_worker_running", False):
            # Safety: if the flag has been stuck for >15 s, reset it
            import time as _time
            stuck_since = getattr(self, "_badge_worker_start", 0)
            if _time.time() - stuck_since < 15:
                return
            # Reset the dead-lock
        ...
        import time as _time
        self._badge_worker_start = _time.time()
    ...
    def _apply_badge_counts(self, si_count, cn_count, so_count, pay_count, cust_count, fiscal_count):
        """Update the Q-badge and Z-badge UI - always visible, never shrinks."""

        # Determine if we are in offline mode
        try:
        ...
        # ── Q-badge: hidden in offline (nothing to sync), always shown otherwise ──
        ...
        # ── Z-badge: hidden in offline, always shown otherwise ──────────────────
        if hasattr(self, "_z_badge"):
            if _is_offline:
                self._z_badge.setVisible(False)
            else:
                z_btn = self._z_badge
                z_btn.setVisible(True)
                z_btn.setMinimumWidth(60)
                ...
                if fiscal_count == 0:
                    ...
                    z_btn.setToolTip(
                        ...
                        "Click to view and retry failed fiscalizations"
                    )
                    z_btn.setStyleSheet(f"""
                        QPushButton {{
                            background-color: {bg}; color: {WHITE}; border: none;
        ...
        if self._get_pos_rule("block_zero_stock", default=False):
            ...
            if product_id:
                try:
                    ...
                    pass
            if item_stock is None and stock is not None:
                item_stock = float(stock)
            ...
            if item_stock <= 0:
                self._warn_popup(
                    "Insufficient Stock",
                    f"<b>{name}</b> is out of stock ({item_stock:g} in stock).<br>"
    ...
    def _collect_invoice_items(self) -> list[dict]:
        ...
        for r in range(self.invoice_table.rowCount()):
            ...
            pharm = None
            try:
                part_cell = self.invoice_table.item(r, self.COL_PART_NO)
                if part_cell:
                    pharm = part_cell.data(self.PHARMACY_META_ROLE)
            except Exception:
                pharm = None
            ...
            items.append({
                ...
                "tax_amount": total - (total / (1 + (tax_rate / 100))) if tax_rate > 0 else 0,
    ...
    def _save_sale(self):
        ...
        try:
            ...
            # ── Clear invoice after fiscal wait if enabled ────────────────
            _sale_id_for_fiscal = sale.get("id") if sale else None

            def _after_fiscal_save(fiscalized: bool):
                if not fiscalized:
                    print(f"[POSView] Fiscal wait timed out for sale {_sale_id_for_fiscal} - proceeding anyway")
                self._new_sale(confirm=False)
            ...
            if _needs_fiscal_wait:
                self._start_fiscal_wait(_sale_id_for_fiscal, _after_fiscal_save)
            else:
                self._new_sale(confirm=False)
        except Exception as e:
...
# ── Options dropdown ──────────────────────────────────────────────────────
    ...
    def _build_invoice_table(self):
        ...
        sym = self._get_currency_symbol()
        dynamic_labels = ["LN", "Item No.", "Item Details", f"Amount ({sym})", "Qty", "UOM", "Disc", "TAX", f"Total ({sym})"]
    ...
    def _init_row(self, r, part_no="", details="", qty="",
                  amount="", uom="", disc="", tax="", total="", ln=""):
        # vals order must match COL_* constants
        vals = [ln, part_no, details, amount, qty, uom, disc, tax, total]
    ...
    def _get_pos_rule(self, key: str, default: bool = True) -> bool:
        ...
        try:
            ...
            cur.execute(
                "SELECT setting_value FROM pos_settings WHERE setting_key=?", (key,))
            row = cur.fetchone(); conn.close()
            return (str(row[0]).strip().lower() in ("1", "true", "yes")) if row else default
        except Exception:
            return default
    ...
    def _build_invoice_footer(self):
        ...
        # ── Fiscal status indicator (shown during post-sale fiscalization wait) ──
        self._lbl_fiscal_status = QLabel("")
        self._lbl_fiscal_status.setStyleSheet(
            f"color: {ORANGE}; font-size: 11px; font-weight: bold; background: transparent; min-width: 200px;")
        self._lbl_fiscal_status.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._lbl_fiscal_status.setVisible(False)
        layout.addWidget(self._lbl_fiscal_status)
    ...
    # =========================================================================
    # FISCALIZATION WAIT - post-sale 3+3 second poller
    # =========================================================================
    def _start_fiscal_wait(self, sale_id: int, on_complete):
        """
        After a sale, poll fiscalization status for up to 6 seconds total:
          - Phase 1 (0–3 s):  silent wait, poll every 500 ms.
                              If fiscalized -> call on_complete(fiscalized=True).
          - Phase 2 (3–6 s):  show "⏳ Waiting for fiscalization..." in footer.
                              If fiscalized -> call on_complete(fiscalized=True).
          - After 6 s:        call on_complete(fiscalized=False) and clear label.

        on_complete(fiscalized: bool) is always called exactly once on the
        Qt main thread, so it is safe to do UI work inside it.
        """
        # Stop any previously running fiscal wait for an older sale
        self._stop_fiscal_wait()

        self._fiscal_wait_sale_id   = sale_id
        self._fiscal_wait_elapsed   = 0        # milliseconds elapsed
        self._fiscal_wait_callback  = on_complete
        self._fiscal_wait_phase2    = False    # True once we've entered the warning phase

        POLL_MS   = 500    # how often we check the DB
        PHASE1_MS = 3000   # silent window
        TOTAL_MS  = 6000   # give up after this

        def _poll():
            self._fiscal_wait_elapsed += POLL_MS

            # ── Check DB for fiscal status ─────────────────────────────────
            try:
                from models.sale import get_sale_by_id
                s = get_sale_by_id(self._fiscal_wait_sale_id)
                status = s.get("fiscal_status", "") if s else ""
            except Exception:
                status = ""

            if status == "fiscalized":
                self._clear_fiscal_status()
                cb = self._fiscal_wait_callback
                self._fiscal_wait_callback = None
                if cb:
                    cb(fiscalized=True)
                return   # timer already stopped inside _clear_fiscal_status

            # ── Phase 2: show the waiting message ─────────────────────────
            if self._fiscal_wait_elapsed >= PHASE1_MS and not self._fiscal_wait_phase2:
                self._fiscal_wait_phase2 = True
                if hasattr(self, "_lbl_fiscal_status"):
                    self._lbl_fiscal_status.setText("⏳ Waiting for fiscalization...")
                    self._lbl_fiscal_status.setVisible(True)

            # ── Time out ──────────────────────────────────────────────────
            if self._fiscal_wait_elapsed >= TOTAL_MS:
                self._clear_fiscal_status()
                cb = self._fiscal_wait_callback
                self._fiscal_wait_callback = None
                if cb:
        ...
        self._fiscal_poll_timer.setInterval(POLL_MS)
        self._fiscal_poll_timer.timeout.connect(_poll)
        self._fiscal_poll_timer.start()

    def _stop_fiscal_wait(self):
        """Cancel any running fiscal poll timer without firing the callback."""
        t = getattr(self, "_fiscal_poll_timer", None)
        if t is not None:
            t.stop()
            t.deleteLater()
            self._fiscal_poll_timer = None
        self._fiscal_wait_callback = None
        self._clear_fiscal_status()

    def _clear_fiscal_status(self):
        """Hide and blank the footer fiscal status label."""
        t = getattr(self, "_fiscal_poll_timer", None)
        if t is not None:
            t.stop()
            t.deleteLater()
            self._fiscal_poll_timer = None
        if hasattr(self, "_lbl_fiscal_status"):
            self._lbl_fiscal_status.setText("")
            self._lbl_fiscal_status.setVisible(False)
...
""")
    ...
    def _compute_grid_dims(self, any_image: bool) -> tuple:
        """Compute adaptive COLS/ROWS from the actual widget size.

        Strategy:
          • Minimum card width  = 75 px  (readable on small screens)
          • Minimum card height = 55 px  (no images) / 80 px (with images)
          • COLS = available_width  // MIN_CARD_W   clamped to [4, 14]
          • ROWS = available_height // MIN_CARD_H   clamped to [2, 6]
        This makes the grid fill every screen from 1024-wide up to 4K.
        """
        GAP = 2
        MIN_W = 75          # px - minimum card width
        MIN_H_IMG  = 80     # px - minimum card height when images present
        MIN_H_NOIMG = 55    # px - minimum card height, text-only

        gw = self._product_grid_widget.width()
        gh = self._product_grid_widget.height()
        if gw < 200: gw = 800   # sensible fallback before first paint
        if gh < 100: gh = 240

        cols = max(4, min(14, gw // (MIN_W + GAP)))
        min_h = MIN_H_IMG if any_image else MIN_H_NOIMG
        rows = max(2, min(6, gh // (min_h + GAP)))
        return rows, cols
    ...
    def _grid_turn_page(self, direction: int):
        ...
        ROWS, COLS = self._compute_grid_dims(any_img)
        per_page    = ROWS * COLS
    ...
    def _render_product_page(self):
        """Render the current page of products - adaptive columns/rows."""
        ...
        ROWS, COLS = self._compute_grid_dims(any_image_all)
        per_page    = ROWS * COLS
        total       = len(self._current_products)
        total_pages = max(1, (total + per_page - 1) // per_page)
        ...
        # Adaptive cell size - fills the actual widget area at any resolution
        GAP  = 2
        gw   = max(400, self._product_grid_widget.width())
        gh   = max(200, self._product_grid_widget.height())

        cell_w = max(60, (gw - (COLS + 1) * GAP) // COLS)
        cell_h = max(40, (gh - (ROWS + 1) * GAP) // ROWS)

        # Font scales with card size so tiny cards still look clean
        font_pt = max(7, min(11, cell_w // 10))
        font_size = f"{font_pt}pt"
        BTN_STYLE = f"""
            QToolButton {{
        ...
        # Let each row/col expand to fill available space evenly
        for r in range(ROWS):
            self._product_grid.setRowStretch(r, 1)
            self._product_grid.setRowMinimumHeight(r, cell_h)
        for c in range(COLS):
            self._product_grid.setColumnStretch(c, 1)
    ...
    def _open_shift_chooser(self):
        """Nav-bar SHIFT pill -> opens ShiftReconciliationDialog if active, or StartShiftDialog if no shift."""
        try:
            from models.shift import get_active_shift
            if get_active_shift():
    ...
    def _ensure_default_customer(self):
        """
        Guarantees that 'Cash Customer' is set as the active customer 
        whenever no customer is currently selected.

        Rules
        ─────
        • Called on startup (via QTimer.singleShot) and before every payment.
        • 'Cash Customer' customer is assumed to be created securely around login.
        ...
        try:
            ...
            if default:
                ...
                if self.parent_window:
                    self.parent_window._set_status(
                        f"Customer: {default.get('customer_name', 'Cash Customer')}"
                    )
    ...
    def _open_manage_bundles_dialog(self):
        from views.dialogs.manage_bundles_dialog import ManageBundlesDialog
        dlg = ManageBundlesDialog(self)
        dlg.exec()
    ...
    def _run_startup_sync(self):
        ...
        class _StartupSyncWorker(QThread):
            def run(self_inner):                          # noqa: N805
                ...
                log.info("[startup-sync] [OK] All startup syncs complete.")
...
class ReprintDialog(QDialog):
    ...
    def _so_search(self):
        ...
        for o in matches:
            ...
            bal  = float(o.get("balance_due") or 0)
            dt   = o.get("order_date", "")
